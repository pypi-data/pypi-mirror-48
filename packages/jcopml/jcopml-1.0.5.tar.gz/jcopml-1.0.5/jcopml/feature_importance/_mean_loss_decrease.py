...
def _get_feature_importance(model):
    steps = [step[0] for step in model.best_estimator_.steps]
    if steps[-1] in ['dt', 'rf', 'xgb']:
        return model.best_estimator_.named_steps[steps[-1]].feature_importances_
    else:
        raise Exception("Tree based model is not in the pipeline")
