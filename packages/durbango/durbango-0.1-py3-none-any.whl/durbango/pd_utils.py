...
def convert_dates(date_ser, date_format=None, errors='raise'):
    '''returns a pd.Timestamp series. A performance hack'''
    if date_ser.dtype != 'O':
        return date_ser
...
def check_columns(df, required_columns):
    ...
    '''
    if isinstance(required_columns, basestring):
...
def next_n_day_returns(ser, n_days=1, compare_func=np.subtract):
    '''Series of forward looking returns
    Args:
        ser: pd.Series of closing prices
        n_days: (numeric) number of days to look forward, defaults to 1
        compare_func: argument for comparing future and past returns must take two args
    Returns:
        pd.DataFrame
    >>> from klearn.ingest.timeseries import price_df
    >>> px_df = price_df(['SPY', 'AAPL'])
    >>> ydf = px_df.apply(next_n_day_returns)  # n=5
    '''
    back_shift = 1
    forward_shift = back_shift - n_days
    return compare_func(ser.shift(forward_shift), ser.shift(back_shift))


def count_df(ser):
    '''return pd.DataFrame({'n': ser.value_counts(), 'frac': ser.value_counts(normalize=True)})'''
    counts = pd.DataFrame({'n': ser.value_counts(), 'frac': ser.value_counts(normalize=True)})
...
def _md_row_fmt(iterable):
    '''Given an iterable, format as entries of row of markdown table'''
    return '|{}|'.format('|'.join([str(x) for x in iterable]))
...
def render_df_in_markdown(df, incl_index=True):
    '''Given a dataframe, output in raw markdown (for inclusion in gitlab markdown file, etc)

    Args:
        df: (pd.DataFrame) the dataframe to be rendered
        incl_index: (boolean) index will not be included if `False`

    Returns:
        str representation of raw markdown
    '''
    # Force values to dtype string and reset index if req'd
    df_as_str = (df.reset_index() if incl_index else df).astype(str)

    # Format the header and values, and add separator between header and values
    hdr = _md_row_fmt(df_as_str.columns)
    hdr_sep = _md_row_fmt(['---' for _ in df_as_str.columns])
    content = '\n'.join([_md_row_fmt(z) for z in df_as_str.values.tolist()])

    return '\n'.join([hdr, hdr_sep, content])
    ...
    '''Get the outlier trading frequency:
        Args:
            ...
            expected_periods: (list of tuples) of average # of periods,
                e.g. [('month', 2), ('year', 5)]. Derived from flow df
    '''
...
def currency_forward_symbol_to_symbol(fwd_symbol):
    '''return the underlying currency from forward contract symbol'''
    pat = r'\_[a-zA-Z]*\_fwd\_[0-9]*m'
    return re.sub(pat, '', fwd_symbol)
...
def safe_read_msgpack(path):
    '''Read msgpack file only if the file exists. Hack to get around the pandas bug (returns a list
    of jibber-jabber for non-existent files
    ...
    if not os.path.exists(path):
        raise ValueError('File "{}" oesn\'t exist'.format(path))
...
def map_to_num(ser):
    '''Replace a columns existing content with frequency rank. Flow Use Case: renaming client'''
...
def weighted_avg_of_coupon(df, epsilon=1e-6, weight_col='y'):
    '''calculates weighted average of coupon using y column as weights'''
    return weighted_avg(df, 'coupon', weight_col, epsilon)


def calc_fractional_difference(time_ser, derivative_degree):
    '''Compute the fractional difference for a given differentiation degree (could be non-integer)

    We want to compute a derivative that holds some memory of the past:
        1. Integrals hold memory of the entire interval they are applied on.
        2. Derivatives hold memory only on an infinitesimally small area around a point.
        3. Integrals and Derivatives are inverse operators.
        4. The gamma function can be utilized to compute integrals a non-integer number of times
        5. We can combine 3 and 4 to get the desired, non-integer differentiation of any real degree

    Applying to discrete time series:
        1. We can approximate the gamma function with a convolution
        2. The fast fourier transform helps speed up the convolution while retaining ALL of the
            original data points.
        3. Additional explanations are encouraged.

    wiki link:  https://en.wikipedia.org/wiki/Fractional_calculus
    We implemented this:  http://qed.econ.queensu.ca/working_papers/papers/qed_wp_1307.pdf

    Args:
        time_ser:  (pd.Series) A time series
        derivative_degree: (float) the degree of the derivative.  If derivative_degree=0 it
            outputs the original time_ser. If the derivative_degree=1 the output is the same as diff

    Returns:
        (pd.Series) The fractional difference at each point
    '''
    sample_size = time_ser.size
    points_numbers = np.arange(1, sample_size)
    fractional_coeffs = (
        np.hstack([[1], np.cumprod((points_numbers - derivative_degree - 1) / points_numbers)])
    )
    #  for FFT we need a length to be a power of two.  this is the smallest power of two
    #  that is greater than 2 * sample_size - 1. We need a size of 2 * sample_size - 1 for the conv
    convolution_size = int(2 ** np.ceil(np.log2(2 * sample_size - 1)))
    convolution_padding = np.zeros(convolution_size - sample_size)
    coeffs_for_fft = np.hstack([fractional_coeffs, convolution_padding])
    points_for_fft = np.hstack([time_ser.values, convolution_padding])
    fourier_transform_convolution = np.fft.fft(coeffs_for_fft) * np.fft.fft(points_for_fft)
    all_dx = np.fft.ifft(fourier_transform_convolution)
    relevant_real_dx = np.real(all_dx[0: sample_size])
    return pd.Series(relevant_real_dx, index=time_ser.index)
...
def make_score_table(cv, scorer=output_df_scorer):
    '''helper to run output_df_scorer on test and train'''
    mask = cv.output_df.index.get_level_values(0).isin(cv.train_dates)
    odf_tr = cv.output_df.loc[mask]
    odf_valid = cv.output_df.loc[~mask]
    tab = scorer(odf_tr).to_frame('train').assign(valid=scorer(odf_valid))
    return tab
...
def score_metrics_by_size(pred_df, threshold, col_name):
    ...
    metrics_df = pd.concat([above_threshold_metrics, below_threshold_metrics],
                           axis=1)
