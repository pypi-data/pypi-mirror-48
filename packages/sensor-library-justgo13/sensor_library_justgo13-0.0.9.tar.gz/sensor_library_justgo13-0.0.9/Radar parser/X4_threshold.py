    ...
    with open(filename, "rt") as csvfile:
        ...
        for i in range(len(data)):  # Goes through the 180 by n CSV file
            ...
            for j in range(len(data[i])):
                ...
                mean[j] += complex(
                    data[i][j])  # Adds up numbers for each 180 column to get to get a mean for each column

        for i in range(len(data)):

            for j in range(len(data[i])):
                data[i][j] = abs(data[i][j] - mean[j] / len(
                    data))  # Each complex number is subtracted by it's respective column mean to even it out and the magnitude is taken since it is complex
