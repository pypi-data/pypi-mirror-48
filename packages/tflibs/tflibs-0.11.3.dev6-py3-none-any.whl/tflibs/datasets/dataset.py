...
class BaseDataset:
        ...
        def process_wrapper(coll, length, thread_idx):
            ...
            for elem in tqdm(coll, total=length, position=thread_idx):
                ...
                for processed_e in processed:
                    # Build feature proto
                    nested_feature = {k: feature_spec.feature_proto(k, [processed_e[k]]) for k, feature_spec in
                                      feature_specs if k in processed_e}
                    # Flatten nested dict
    ...
    def read(self, split=None, num_parallel_reads=16, num_parallel_calls=16, cache=True):
        ...
        def dataset_fn():
            ...
            def parse(record):
                return {k: feature_spec.parse(k, record) for k, feature_spec in feature_specs}
