...
#     showCrystals = pyqtSignal(bool, name=SHOW_CRYSTAL_SIGNAL)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.tools.calibrationRadioButton.toggled.connect(self.handleViewChangeToCalibration)
        self.tools.exposureRadioButton.toggled.connect(self.handleViewChangeToExposure)
...
class FilterBox(QWidget):
    ...
    def getImageDisplayFilters(self):
        displayFilters = {}
        for filter in self.filterWidgets:
            logger.debug("DisplayFilter %s" % filter)
            if filter.filter.name == MIN_VISIBLE_FILTER:
                displayFilters[MIN_VISIBLE_FILTER] = self._getDisplayFilterVals(filter)
            elif  filter.filter.name == MAX_VISIBLE_FILTER:
                displayFilters[MAX_VISIBLE_FILTER] = self._getDisplayFilterVals(filter)
...
#                                             self.handleSigLevelsChanged)
    ...
    def getFilteredData(self):
        filteredData = np.copy(self.imageStack[self.imageNumber, :, :])
...
#         logger.debug ("image.shape %s" % ((image.shape,)))
    ...
    def handleSigLookupTableChanged(self, object):
        '''
        '''
        logger.debug(METHOD_ENTER_STR % object)
        imageNumber = self.slider.value()
        lut = self.histWidget.item.getLookupTable( \
                                    img=self.imageStack[imageNumber,:,:])
        logger.debug("new Lookup Table: %s" % ((lut,)))
          
    def handleSigLevelsChanged(self, object):
        '''
    ...
    def handleSigLevelChangeFinished(self, object):
        '''
