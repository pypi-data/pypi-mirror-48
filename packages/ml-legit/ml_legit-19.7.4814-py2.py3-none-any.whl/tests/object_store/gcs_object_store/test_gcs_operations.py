...
from missinglink.legit.gcs_utils import GCSDownloadDirectDownload, do_delete_all, Uploader, Downloader, s3_moniker, \
    ...
    _clear_cached_objects, _get_boto_local_client, _get_azure_retry
...
class TestGCSOperations(BaseTest):
    ...
    def test_download_no_auth_method_with_bucket(self, mock_download_to_file):
        def actual_test():
            ...
            mock_download_to_file.expects_call().with_args(arg.isinstance(GCSDownloadDirectDownload.WrapperStream)).calls(write_data)
    ...
    def test_gcs_download_with_bucket_into_target(self, mock_download_to_file):
        def actual_test():
            ...
            mock_download_to_file.expects_call().with_args(arg.isinstance(GCSDownloadDirectDownload.WrapperStream))
    ...
    def test_s3_download_with_bucket_into_target(self):
        ...
        mock_download_fileobj = fudge.Fake().is_callable().with_args(self.bucket_name, self.object_name_1, fake_target_fileobj, Callback=None)
    ...
    def test_s3_download(self):
        ...
        mock_download_fileobj = fudge.Fake().is_callable().with_args(self.bucket_name, self.object_name_1, arg.isinstance(six.BytesIO), Callback=None).calls(fake_download_fileobj)
    ...
    def test_s3_download_moniker_in_object_name(self):
        ...
        mock_progress_callback = fudge.Fake().is_callable()

        for i in range(len(download_result)):
            mock_progress_callback = mock_progress_callback.with_args(i + 1).next_call()

        mock_download_fileobj = fudge.Fake().is_callable().with_args(self.bucket_name, self.object_name_1, arg.isinstance(six.BytesIO), Callback=mock_progress_callback).calls(fake_download_fileobj)
