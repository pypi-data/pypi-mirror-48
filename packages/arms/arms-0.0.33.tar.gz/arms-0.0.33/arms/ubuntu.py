...
def register_runner(domain):
    ...
    assert exe('gitlab-runner register -n --name {domain} --tag-list deploy --executor shell --maximum-timeout 3600'
               ' -u {gitlab_url} -r {gitlab_token} '.format(domain=domain, gitlab_url=gitlab_url, gitlab_token=gitlab_token))
