...
class Mysql(object):
    ...
    def like_update(self, table, where, values={}):
        """
            like update
        """
        if not isinstance(where, dict):
        ...
        self.connect()
        cursor = self.conn.cursor()
        sql = self.sqlGen.like_update(table=table, where=where, values=values)
        self.debug(sql)
        cursor.execute(sql)
        self.conn.commit()
        cursor.close()
    ...
    def like_query(self, table, where={}):
        """like query"""
        res_arry = []
        self.connect()
        cursor = self.conn.cursor()
        sql = self.sqlGen.like_query(table=table, where=where)
        self.debug(sql)
        cursor.execute(sql)
        for data in cursor.fetchall():
            res_arry.append(data)
...
if __name__ == '__main__':
    ...
    table = 'test'
    # insert
    ret = db.insert(table, {'val': str(time.time())})
    print(ret)
    # delete
    db.delete(table, {'id': '<26'})
    # update
    db.update(table, {'id': 26}, {'val': '666'})
    # query
    res = db.query(table)
    print(res)
