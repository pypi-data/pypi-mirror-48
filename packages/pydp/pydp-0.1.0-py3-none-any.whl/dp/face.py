...
def show_camera_face_window(camera_data, warning_faceids=['zhu', 'yjy', 'yan']):
    """创建视频人脸监控windows窗口"""
    # create window
    face_imgs, face_labels = [], []
    for i in range(MAX_FACE):
        face_imgs.append(sg.Image(filename=CUR_PATH + '/data/image/camera-kuang.png', size=(80, 80), key='face' + str(i)))
        face_labels.append(sg.Text('', size=(14, 1), key='label' + str(i)))
    layout = [
        [sg.Image(filename=CUR_PATH + '/data/image/camera-pic.png', size=(600, 400), key='camera')],
        face_imgs,
        face_labels,
...
if __name__ == '__main__':
    ...
    # 创建人脸ID
    faceid = face.register_faceid(CUR_PATH + '/data/image/test-face.jpg', 'test', faceid_path=Face.FACE_ID_PATH)
    print(faceid)
    # 启动摄像头人脸识别
    face.get_camera_face(camera_data=face.camera_data, callback=show_camera_face_window)
