...
def is_proper_time():
    ...
    if not do_not_bother['enable']:
        return True
    if 'since' not in do_not_bother or 'till' not in do_not_bother:
        return True
    since = do_not_bother['since']
    till = do_not_bother['till']
        ...
        while self._running is True:
            ...
            if state == "PASSIVE":
                if status > 0:  # key word found
                    ...
                    if audio_recorder_callback is not None and status == 1 and is_proper_time():
                        state = "ACTIVE"
                    continue
