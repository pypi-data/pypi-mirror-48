...
class Defaults(object):
    ...
    def defaults(self, solution, configuration=None):
        ...
        if solution.project_list[0].platform.is_windows():
            self.environmentvariables = ['BURGER_SDKS']

            # Add the default system directories
            if solution.project_list[0].project_type != ProjectTypes.library or \
        ...
        elif solution.project_list[0].platform.is_macos():
            if solution.project_list[0].project_type != ProjectTypes.library or \
                    solution.name != 'burgerlib':
            ...
            if solution.project_list[0].platform.is_macos_carbon():
            ...
            elif solution.project_list[0].platform.is_macos_classic():
                self.systemsearchpaths = ['MSL', 'MacOS Support']
...
def generate(solution):
    ...
    for project in solution.project_list:
        ...
        for configuration in project.configuration_list:
            if configuration.platform.is_windows():
                configuration.platform = PlatformTypes.win32
    ...
    for configuration in solution.project_list[0].configurations:
        ...
        platform = solution.project_list[0].platform
        ...
        if solution.project_list[0].platform.is_windows():
            ...
            target.settinglist.append(
                MWProject_X86(
                    solution.project_list[0].project_type,
        ...
        if alllists or liblist:
            ...
            for item in alllists:
                ...
                # Add to file group
                codewarriorprojectfile.addtogroups(solution.project_list[0].platform, configuration.name, parts)

            filelist = sorted(filelist, key=unicode.lower)
            for item in filelist:
                target.filelist.append(FILE(solution.project_list[0].platform, configuration.name, item))
                target.linkorder.append(FILEREF(solution.project_list[0].platform, None, item))
            ...
            for item in liblist:
                ...
                codewarriorprojectfile.addtogroups(
                    solution.project_list[0].platform, configuration.name, [
                        'Libraries', item])
    ...
    if cwfile is not None and solution.project_list[0].platform.is_windows():
