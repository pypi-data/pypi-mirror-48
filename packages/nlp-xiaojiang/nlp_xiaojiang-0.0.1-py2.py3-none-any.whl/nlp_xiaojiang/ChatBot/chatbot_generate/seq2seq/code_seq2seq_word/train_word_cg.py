...
from nlp_xiaojiang.utils.mode_util.seq2seq.data_utils import batch_flow_bucket as batch_flow
from nlp_xiaojiang.utils.mode_util.seq2seq.model_seq2seq import SequenceToSequence
from nlp_xiaojiang.utils.mode_util.seq2seq.thread_generator import ThreadedGenerator

from nlp_xiaojiang.conf.path_config import chatbot_data_cg_emb_anti_word
from nlp_xiaojiang.conf.path_config import chatbot_data_cg_xyw_anti_word
from nlp_xiaojiang.conf.path_config import model_ckpt_cg_anti_word
from nlp_xiaojiang.utils.mode_util.seq2seq.word_sequence import WordSequence

sys.path.append('..')


def tet(bidirectional, cell_type, depth,
         attention_type, use_residual, use_dropout, time_major, hidden_units):
    """测试不同参数在生成的假数据上的运行结果"""

    emb = pickle.load(open(chatbot_data_cg_emb_anti_word, 'rb'))

    x_data, y_data, ws = pickle.load(
        open(chatbot_data_cg_xyw_anti_word, 'rb'))
