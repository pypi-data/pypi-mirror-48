...
See Sample.py for more details of usage.
"""
...
class Data:
    def __init__(self):
        #self.comparison=""  #comparison title, NOT used current version
        #self.outcome=""     #outcome text, NOT used current version
        self.datatype=''    #data type:'CATE','CONT','DIAG'
    ...
    #input:formatted lines
    #output: studies[[],...]
    #affect values：self.studies,self.subgroup,self.comparison,self.outcome,self.nototal
    def getdata(self,lines):
        ...
        if self.datatype=='CATE':
            ...
            def _getnum(s):
                return float(s)
        elif self.datatype=='DIAG':
            stylen=5
            def _getnum(s):
                return int(s)
        else:
            Err="error.(no datatype setting:'CATE', 'CONT' or 'DIAG')"
            raise Exception(Err)
        ...
        for line in lines:
            ...
            elif line[0:12].lower()=='<comparison>':
    ...
    def readfile(self,fname='studies.txt'):
        try:
            f = open(fname,"r")
            lines = f.readlines()   #read all lines, ps: read single line by f.readlines()
        except :
...
class Meta:
    def __init__(self):
        #self.name='Intervention' #干预('Intervention')，诊断('Diagnostic'),网状('Network')
        self.datatype=''       #'CATE': binary/categorical/dichotomous data; and 'CONT': continuous data; and 'DIAG': Diagnostic test data
        ...
        #self.comparison=''     #comparison title
...
def chkdtype(datatype):
    ...
    a_diag=["DIA","DIAG","DIAGNOSTIC","DIAGNOSTICS","DIAGNOSIS"]
    if dt in a_cate:
        dt="CATE"
    elif dt in a_cont:
        dt="CONT"
...
#Q检验（异质性检验/卡方检验）估计p值使用
#结合I^2使用反映异质性通常：
# Q检验p>0.1 认为异质性较小（此时I^2<50%）
# Q检验p<0.1 认为有异质性（此时I^2>50%）
#It's just guess a range of P value from a table, NOT exactly calculate
def Stat_guess_CSP(cs,k):
    #import pdb;pdb.set_trace()
    #from metabase import lmtbl_chisquare as lmtbl
...
#I^2
#反映异质性的量，通常：
# I^2>50%认为有异质性（此时可参考Q检验的p值，p>0.1）
# I^2<50%认为异质性较小（此时Q检验的p<0.1）
def Stat_Isquare(Q,k):
    if Q==0:
        return 0
    return max(100*(Q-k+1)/Q,0)

#z test(t/u test)符合正态分布
#合并效应量的检验，输出结果是z和p
...
'''
M-H法
计数资料，固定模型，合并效应量（不能检验），适合计算OR，RR，RD
'''
#Functions for M_H method
#Q Test,反映异质性的中间量，用于计算I^2；其分布符合卡方分布，因此可以根据卡方表估计p值（Q检验）
#input:EFFS=[[es,w],...]:效应量（LnOR/LnRR/RD/MD）,效应量的标准误
...
def MH_total_OR (studies,models='Fixed'):
    ...
    w_q,tw_q,te_q=0,0,0    #用于计算随机模型下的Q值
...
#Q Test,反映异质性的中间量，用于计算I^2；其分布符合卡方分布，因此可以根据卡方表估计p值（Q检验）
#input:EFFS=[[effs,w],...]:效应量（LnOR）,w; ttles=lnOR
#output: Q and p (range string, NOT exactly)
...
#########################################
'''
Inverse variance 倒方差法
计数或计量资料，固定模型，效应量合并及检验
'''
#Weight
#input:se
def IV_Weight(se):
...
#Q Test,反映异质性的中间量，用于计算I^2；其分布符合卡方分布，因此可以根据卡方表估计p值（Q检验）
#input:EFFS=[[es,se],...]:效应量（LnOR/LnRR/RD/MD）,效应量的标准误
#output: Q and p (range string, NOT exactly)
def IV_Q(effs,ttles):
    Q=0;p=0
    for idx in range(len(effs)):
...
'''
DerSimonnian Laird法
计数或计量资料，随机模型，效应量合并(LnOR,LnRR,RD,MD,SMD)及检验
'''
...
'''
绘图模块
forest, funnel, Sensitivity single, Sensitivity two
...
#results[0]:total; [0]'Total', [1]OR, [2]weight, [3]LCI, [4]UCI, [5]N, [6]SE{Ln(OR)}, [7]Q, [8]p, [9]I2 ...
#results[i]:studies; [0]study name, [1]effect size, [2]weight, [3]LCI, [4]UCI, [5]n,[6]quality ...
def Fig_Forest (size,dpi,es_w_ci, titletxt="Meta-analysis Results (by Pymeta)",no_ttl=False):
    ...
    myfig = plt.figure(num="Forest plot by Pymeta", linewidth=1, figsize=size, dpi=dpi)  #Frameon=False
    ...
    xlim=[];y_k=0;subgrp=[]  #y_k:y轴偏移量；subgrp:亚组
    ...
    plt.plot([0,0], [0,len(es_w_ci)+3], 'black')     #draw y-axis
    ...
    for i in range(1,len(es_w_ci)):
        ...
        if stdname[0:5]=="<sub>":  #this line is a subgroup
            ...
            y=[len(es_w_ci)-i+y_k+0.2,
               ...
               len(es_w_ci)-i+y_k+0.2]
            if (es_w_ci[i][9]<50) :
                plt.fill(x,y,color="blue", lw=1)  #filled: I2<50 异质性较小
            else :
                plt.plot(x,y, 'blue', lw=1)       #empty: I2>50 异质性较大
        ...
        #shadow X line
        #lncolor,lnstyle=("blue","-") if (es_w_ci[i][4]<1 or es_w_ci[i][3]>1) else ("blue","--") #if p>0.05 then fade it
    ...
    if no_ttl==True:
        ...
        y=[2.3,2,1.7,2,2.3]
        if (es_w_ci[0][9]<50) :
            plt.fill(x,y,color="black", lw=1)  #filled: I2<50 异质性较小
        else :
            plt.plot(x,y, 'black', lw=1)       #empty: I2>50 异质性较大
