...
@click.argument('type', type=click.STRING, default='widget', required=True)
@click.option('--type', default='widget',
    help='Choose type of your project(oauth, widget)')
@click.argument('structure', type=click.STRING, default='widget', required=True)
@click.option('--structure', default='default',
    help='Choose structure type(default, inside)')
def init(name, type, structure):
    if type.lower() not in ['widget', 'oauth', 'crm']:
        raise Exception("Invalid type, type muste oauth or widget")
    ...
    if type.lower() == 'widget':
        os.popen(f'cp -R {file_manager.integration_dir}/integration_files/widget/* {name}')
    elif type.lower() == 'oauth':
        os.popen(f'cp -R {file_manager.integration_dir}/integration_files/oauth/* {name}')
    elif type.lower() == 'crm':
        while True:
            print("================")
            ...
            if auth_type.lower() == 'basic':
                os.popen(f'cp -R {file_manager.integration_dir}/integration_files/widget/* {name}')
                os.popen(f'cp -R {file_manager.integration_dir}/integration_files/crm/basic* {name}')
                break
            if auth_type.lower() == 'oauth':
                os.popen(f'cp -R {file_manager.integration_dir}/integration_files/oauth/* {name}')
                os.popen(f'cp -R {file_manager.integration_dir}/integration_files/crm/oauth* {name}')
                break
            else:
                print(Base.WARNING, "WARNING chose valid auth_type(basic, oauth):", Base.END)
    else:
        raise Exception("Invalid type")
    os.popen(f'cp -R {file_manager.integration_dir}/integration_files/methods/ {name}')
    
    print(Base.OKBLUE, f"-=-=-: INSTALL DEPENDIES FOR PROJECT: {name} :-=-=-", Base.END)
    install_dependies()
    
    print(Base.OKGREEN, "-=-=-: FINISH INSTALL DEPENDIES :-=-=-", Base.END)
    sleep(4)
    print(Base.HEADER,f"-=-=-: STARTING CREATE django project :-=-=-", Base.END)
    file_manager.rewrite_settings()
    sleep(6)
