...
class RequestHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        smol.logger.log(__class__, f"Got request from {self.client_address[0]}")
        parsed = smol.http.parse(self.data)
        ...
        if(route_match):
            ...
            for key, value in server_options.items():
                response[key] = value
            response["content"] = function({"post": kwargs, "ip": self.client_address[0],"request": parsed})
            self.request.sendall(smol.http.build_response(response).encode("utf-8"))
        else:
...
class Server(object):
    def __init__(self, port=2920, host="0.0.0.0"):
        self.port = port
        self.host = host
        self.router = smol.router.Router()
