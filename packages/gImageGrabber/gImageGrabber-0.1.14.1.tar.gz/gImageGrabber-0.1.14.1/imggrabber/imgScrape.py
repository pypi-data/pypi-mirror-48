    ...
    try:
        ...
        try:
            ...
            driver = webdriver.Firefox(
                executable_path=path+"driver/geckodriver")
...
def saveImages(data, name, onlyType='', startingnos=0):
    if onlyType is '':
        onlyType = ' '
    ...
    if onlyType == ' ':
            ...
            for i, link in enumerate(data[type]):
                if type == '':
                    ...
                    savepath = os.path.join(
                        directory, typeDirec, '{:06}.jpg'.format(i+startingnos))
                    print(link, "save as", str(i+startingnos)+".jpg")
                    try:
                    ...
                    savepath = os.path.join(
                        directory, type, '{:06}.{}'.format((i+startingnos), type))
                    try:
                        ulib.urlretrieve(link, savepath)
                    except (ulibError.HTTPError, ulibError.URLError) as e:
                        print(str(e))
                        pass
                    print(link, "save as", str(i+startingnos)+"."+type)
        ...
        for i, link in enumerate(data[onlyType]):
            ...
            savepath = os.path.join(
                directory, onlyType, '{:06}.{}'.format((i+startingnos), onlyType))
            print(link+str(i+startingnos)+" save as "+"."+onlyType)
            try:
                ulib.urlretrieve(link, savepath)
            except (ulibError.HTTPError, ulibError.URLError) as e:
                print(str(e))
