...
class FluentPlot:
    ...
    def _draw(self, obj, ax):
        if self._iterate:
            for item in obj:
                self._method(ax, *self._make_args(item), **self._make_kwargs(item))
            ...
            self._method(ax, *self._make_args(obj), **self._make_kwargs(obj))


    def on(self, ax: Axes, width: Optional[int] = None, height: Optional[int] = None) -> 'FluentPlot':
        if ax is not None:
            self._default_ax = ax
    ...
    def call_ax(self, ax_selector: Callable[[Axes],Callable]) -> 'FluentPlot':
        self._method = lambda ax, *args, **kwargs: ax_selector(ax)(*args,**kwargs)
        return self
