...
class Path(type(pathlib.Path())) :
	...
	def archive(self, dst_dir=None, timestamp=False, delete_original=False) :
		...
		if self.is_dir() :
			...
			cc_pathlib.proc.run('tar', '--create', '--file', tar_pth, self.name, cwd=self.parent)
			cc_pathlib.proc.run('lzip', '--best', tar_pth)
			if delete_original :
				self.delete()
		elif self.is_file() :
			cc_pathlib.proc.run('lzip', '--best', self)
	...
	def delete(self, content_only=False) :
		if not self.is_dir() :
			# print("{0} does not exists: nothing done".format(pth))
			return
	...
	def _load_archive(self, fmt, encoding) :
		...
		if fmt == '.gz' :
			...
			cmd = ['lzip', '--decompress', '--stdout', str(self)]
	...
	def _save_archive(self, data, fmt, encoding='utf-8', opt=None) :
		...
		if fmt == '.gz' :
			...
			cmd = ['lzip', '--best', '--force', '--output', str(self.with_suffix(''))]
			ret = subprocess.run(cmd, input=b_data)
		else :
