...
class NetworkScanner:
    ...
    def add_locations_to_device_list(self, device_list):
        """
        ...
        for local in device_list:
            local["location"] = self.ip2loc("")
            if("connections" in list(local.keys())):
                for remote in local["connections"].keys():
                    local["connections"][remote]["location"] = self.ip2loc(remote)
