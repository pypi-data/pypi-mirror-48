...
class Region(NamedBaseModel):
    ...
    def __init__(self, start, end, context, direction=FORWARD, name=None, id=None):
        ...
        self.__span = [context.t(start), context.t(end)]
        if direction == Region.REVERSE:
            self.__end, self.__start = self.__span
        else:
            self.__start, self.__end = self.__span
    ...
    def rp(self):
        ...
        return max(self.__span)

    @property
    def lp(self):
        """The left most point. The minimum index in the region."""
        return min(self.__span)

    @lp.setter
    def lp(self, v):
        """Sets the left most point"""
        self.__span[argmin(self.__span)] = self.context.t(v)

    @rp.setter
    def rp(self, v):
        """ Sets the right most point """
        self.__span[argmax(self.__span)] = self.context.t(v)
    ...
    def copy(self):
        ...
        return self.__class__(
            self.lp, self.rp, self.context, direction=self.direction, name=self.name
        )
