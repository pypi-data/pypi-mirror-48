...
class AdminModelPaginator(pagination.PageNumberPagination):
    # def get_api_sort_interface_url(self):
    #     model_id = [value for item, value in self.request.parser_context['kwargs'].items() if 'id' in item]
    #     try:
    #         if model_id:
    #             return self.request.build_absolute_uri(reverse(
    #                 str(self.request.resolver_match.view_name).replace(self.request.resolver_match.url_name,
    #                                                                    'sort-interface'), args=[model_id[0]]))
    #         else:
    #             return self.request.build_absolute_uri(reverse(
    #                 str(self.request.resolver_match.view_name).replace(self.request.resolver_match.url_name,
    #                                                                    'sort-interface')))
