    ...
    def Sn2_networkxG(self, bandlimit=0):
        ...
        for i in range(en):
            if self.sedege[i].lastbandwidth > bandlimit:
                g.add_edge(self.sedege[i].nodeindex[0], self.sedege[i].nodeindex[1],index=self.sedege[i].index,lastbandwidth= self.sedege[i].lastbandwidth,bandwidth=self.sedege[i].bandwidth,classflag=self.sedege[i].classflag,open=self.sedege[i].open)
        return g
