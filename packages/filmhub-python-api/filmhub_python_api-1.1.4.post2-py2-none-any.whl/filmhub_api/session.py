...
class Session(object):
	...
	def __init__(self, domain=None, username=None, api_key=None, pwd=None, hostname=None, port=None, proxy=None, verbose=False, dev=False):
		...
		self.verbose("Creating FilmHUB Python API session (v%s)"%Session.__version__)
		if domain is None:
	...
	def login(self, revive_session_key=None):
		...
		if self._api_key:
			headers = {
				"Authorization":"FHCredentials %s"%(Session.base64_encode('{"domain":"%s","username":"%s","api_key":"%s"}'%(self._domain, self._username, self._api_key)))  
			}
		else:
			headers = {
				"Authorization":"FHCredentials %s"%(Session.base64_encode('{"domain":"%s","username":"%s","pwd":"%s"}'%(self._domain, self._username, Session.base64_encode(self._pwd))))
			}
	...
	def rest(self, method, hostname, uri, data, timeout=None, ssl=True, port=None, quiet=False, headers=None):
		...
		if headers is None:
			if uri.find("registry/")!=0:
				...
				headers = {'Authorization':"FHSession %s"%(Session.base64_encode('{"domain":"%s","username":"%s","session_key":"%s"}'%(self._domain, self._username, self._session_key)))}
			else:
				headers = {}
		headers['FHDevice'] = "PythonAPI v%s @ %s %s(%s)"%(Session.__version__, sys.platform, Session.get_hostname(), os.name)
	...
	# Help
	def help(self):
		print("Please have a look at the Python API reference: https://sites.google.com/filmhub.cloud/python-api")
