...
class BasePipe(object):
    def __init__(self, command_line):
        ...
        self.__cmd_args = ' '.join(self.__cmd_args)
    ...
    def start(self):
        self.popen_inst = Popen(self.__cmd_args, stdout=PIPE, stderr=PIPE, shell=True)
    ...
    def kill(self, signal=15):
        ...
        cmd_kill = 'kill -%s %s' % (str(signal), self.proc_pid)
        os.system('ps -ef|grep python')
        #os.system(cmd_kill)
        os.kill(self.proc_pid, signal)
        self.wait()
        os.system('ps -ef|grep python')
