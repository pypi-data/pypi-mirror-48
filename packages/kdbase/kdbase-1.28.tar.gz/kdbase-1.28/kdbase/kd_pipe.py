...
class BasePipe(object):
    ...
    def start(self):
        ...
        for child in self.__proc.children():
            sys.stderr.write('real pid:%s' % (str(child.pid)))
            self.proc_pid = child.pid
        sys.stdout.write('父进程id: %s\n' % str(self.__pid))
        sys.stdout.write('子进程id: %s\n' % str(self.proc_pid))
        #设置stdout为非堵塞状态
    ...
    def kill(self, signal=15):
        ...
        os.system('sudo kill -%s %s' % (str(signal), str(self.__p_pid)))
        os.system('sudo kill -%s %s' % (str(signal), str(self.proc_pid)))
        self.wait()
    ...
    def get_rss_memory(self):
        pidmem = 0
        pidfile = os.path.join("/proc/", str(self.proc_pid), "status") 
        ...
        if not verbose:
            ...
            try:
                ...
                while self.popen_inst.poll() is None:
                    ...
                    sys.stderr.write('pid:%s host:%s\n' % (self.proc_pid, socket.gethostname()))
                    sys.stderr.write(str(self.__child_oom)+'\n')
                    sys.stderr.write(str(total_memory)+'\n')
                ...
                if len(buff_out_list) != 0:
                    sys.stderr.write(''.join(buff_out_list))
...
def test1():
    p = AdvPipe('env -i bash -c \'export CUDA_VISIBLE_DEVICES=99 && /opt/mr_binary/test_lane_client_v5.3.2 400205723 400026669_20190116160342369 3 000000020\'', '10', '5')
