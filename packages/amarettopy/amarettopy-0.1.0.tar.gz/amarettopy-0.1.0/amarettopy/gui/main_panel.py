...
class MainPanel(Tk.Frame):
    ...
    def add_tab(self, event=None):
        ...
        isUnsafeMode = int(os.getenv('UNSAFE_AMARETTO', 0)) == 1
        if isUnsafeMode:
            note.add(InitTab(self.amaretto, note), text = "Init")
        note.add(ConfigTab(self.amaretto, 'servo.yml', True, note), text = "ServoParam")
        note.add(ConfigTab(self.amaretto, 'system.yml', False, note), text = "SystemParam")

        note.add(LogTab(self.amaretto, isUnsafeMode, note), text = "Log")
