...
class HashEncoder(Encoder):
    def __init__(self, fit_variables: list, n_min_features: int = 2,
                 n_attempt_max: int = 3, n_attempt_tot_max: int = 30,
                 verbose: bool = False):
    ...
    def __get_hashing(self, df: pandas.DataFrame, variable: str):
        ...
        while (lost != 0 or not non_trivial) \
            ...
            keys_list = list(
                map(lambda x: str(x) + str(numpy.random.rand())[2:8], uniques))
        ...
        if lost != 0 or not non_trivial:
            raise HashingError('Hashing failed on variable: {}. Lost: {}. '
                               'Trivial: {}.'
                               .format(variable, lost, not non_trivial))
