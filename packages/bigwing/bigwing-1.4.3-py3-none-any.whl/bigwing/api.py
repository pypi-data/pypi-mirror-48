...
class BigwingAPIProcessor(metaclass=ABCMeta) :
    ...
    def summary(self) :
        ...
        print("- 성공 건수 : ",sum(self.data.status == "OK"))
        print("- 실패 건수 : ",sum(self.data.status != "OK"))
        print("- 성공율 : {}%".format(round(sum(self.data.status == "OK")/self.data.shape[0]*100,1)))
...
class Bigwing_Geocoder(BigwingAPIProcessor) :
    ...
    def __fetch(self, keyword):
        ...
        if (status == 'OK') & (len(resp['results']['juso']) != 0):
    ...
    def run(self, limit=True):
        ...
        limit이 True일경우, status가 "OK"인 행데이터는 Skip하고 연속진행
        :return: 없음
        '''
        ...
        data = self.data.copy()
        if (limit == True) & ("status" in data.columns):
            data = data[data["status"] != "OK"]
        data_size = len(data)
        succeed_cnt = 0
        if data_size != 0:
            for idx, (keyword, keyword2) in enumerate(zip(data[self.col], data[self.col2])):
                # 변환 및 저장
                ...
                if values[0] == "OK":
                    ...
                    values = self.__fetch(keyword2)
                    if values[0] == "OK":
                        succeed_cnt += 1

                for value in values[1:]:
                    self.data.loc[self.data[self.col] == keyword, value[0]] = value[1]
                self.data.loc[self.data[self.col] == keyword, "status"] = values[0]
                # 결과 출력
