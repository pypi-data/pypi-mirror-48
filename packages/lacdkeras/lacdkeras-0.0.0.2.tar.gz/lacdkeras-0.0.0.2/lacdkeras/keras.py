...
class keras_model():
    ...
    def set_model(self,Input_size,output_size,neuron_count):
        ...
        self.model.add(Dense(output_dim = output_size,activation='softmax'))
        ...
        self.train_history = model.fit(self.x_train, self.y_train,      #輸入 與 輸出
        ...
        #最後輸出測試資料跟訓練資料的正確率!
        score = self.model.evaluate(x_Train_norm,One_hot_Y_Train_Data)
        print ('\nTrain Acc:', score[1])
        score = self.model.evaluate(x_Test_norm,One_hot_Y_Test_Data)
        print ('\nTest Acc:', score[1])
        #開始使用修正完的參數做預測，並將"預測"的結果放置在classes裡面
        classes = model.predict_classes(x_Test_norm, batch_size=batch_size)
