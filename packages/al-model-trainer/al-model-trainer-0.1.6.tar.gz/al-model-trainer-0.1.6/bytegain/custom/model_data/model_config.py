...
class ModelConfig(object):
    ...
    def from_schema_info_file(path):
        ...
        for table_info in schema_info.join_tables.values():
...
class ModelConfigRowHandler(object):
    ...
    def make_sequence_example(self, rows, bucket_data, instance, prediction_context_page_path, instance_id):
        ...
        feature_values['telescope'] = [tf.train.Feature(int64_list=tf.train.Int64List(value=[0, 0])) for i in xrange(min(len(rows), TELESCOPE_TO_LENGTH))]
        ...
        feature_lists = tf.train.FeatureLists(feature_list={k: tf.train.FeatureList(feature=v) for k, v in feature_values.iteritems()})
