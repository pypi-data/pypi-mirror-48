...
class ColumnTracker(object):
    ...
    def get_top_values(self, min_percent):
        ...
        for key, value in sorted(self._counts.iteritems(), key=lambda (k,v): (v,k), reverse = True):
            if value >= min_count:
...
class TableTracker(object):
    ...
    def _add_column(self, column_name, value):
        ...
        self._columns[column_name].add_entry(unicode(value, "UTF-8") if type(value) == str else value)
    ...
    def _add_columns(self, iteritems, prefix = None):
        for column_name, value in iteritems:
            ...
            if type(value) == dict:
                self._add_columns(value.iteritems(), prefix = full_name)
    ...
    def add_entry(self, values, has_userid):
        ...
        if self._is_tracks:
            ...
            self._add_columns(values["context"].iteritems())
        else:
            self._add_columns(values.iteritems())
    ...
    def print_pretty(self, min_percent):
        ...
        for key, column in sorted(self._columns.iteritems(), key=lambda (k,v): (v._total_count, k), reverse = True):
...
def create_table_info(schema, table_tracker):
    ...
    for column_tracker in table_tracker._columns.values():
