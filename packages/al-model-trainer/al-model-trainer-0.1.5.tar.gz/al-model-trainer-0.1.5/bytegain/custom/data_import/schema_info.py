...
class SchemaInfo(object):
    ...
    def sorted_join_tables(self):
        return sorted(self._join_tables.values(), key = lambda (v): (v._rows), reverse = True)
    ...
    def print_pretty(self):
        ...
        for join_table in sorted(self.join_tables.values(), key = lambda (v): v.row_count, reverse=True):
    ...
    def remove_columns(self, columns):
        for column_full in columns:
            ...
            if tablename == "*":
                tables = [self._main_table, self._page_table] + self._join_tables.values()
            else:
    ...
    def translate_schema_tablenames(self, new_schema):
        ...
        new_join_table = {}
        for table in self._join_tables.values():
...
class ColumnInfo(object):
    ...
    def read_stats(self, items):
        self.null_fraction = items.next()
        if "char" in self.datatype:
            self.col_length = items.next()
            if self.col_length is None:
                self.col_length = 0
        if self.is_numeric():
            self.average = items.next()
            self.variance = items.next()
    ...
    def print_pretty(self):
        ...
        if len(self.name) > 30:
            ...
            name = self.name
        base = u"  %30s(%s): " % (name, dt)
        if self.null_fraction is None:
            return "%42s: No Info" % (base)

        if "char" in self.datatype:
            base = u"%42s %3d%% %3d" % (base, self.null_fraction * 100, self.col_length)
        else:
            base = u"%42s %3d%%" % (base, self.null_fraction * 100)
        ...
        if self.is_groupable():
            ...
            for group in self._groups:
                fraction = group[1]
                if fraction > 0.001:
                    grouping.append(u"%s: %2d%%" % (group[0], fraction * 100))
