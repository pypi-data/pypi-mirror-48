...
class BaseModelSpec(object):
    ...
    def download(self, dir_path=None, for_prod=True):
        ...
        wofs = Wofs(
            aws_access_key_id=os.environs["AWS_ACCESS_KEY_ID"],
            aws_secret_access_key=os.environs["AWS_SECRET_ACCESS_KEY"]
        )
    ...
    def upload(self, dir_path=None):
        ...
        if dir_path is None:
            dir_path = GCS_BUCKET + '/' + self.model_name
        wofs = Wofs(gcs_project=GCS_PROJECT)
...
def save_results_as_csv(filename, id_header, prediction_header, results):
    with open(filename, "w") as f:
        out = csv.writer(f)
        out.writerow([id_header, prediction_header])
        for result in results:
            out.writerow([result['id'], result['prob']])
