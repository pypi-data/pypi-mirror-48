...
class TestBatchIterator(unittest.TestCase):
    def test_batch_reader(self):
        ...
        for i in range (0, iters):
            expected = range(i * batch_size, (i +1) * batch_size)
            actual = batch.next()
            self.assertEquals(actual, expected)

        remainder = batch.next()
        expected = range(iters * batch_size, 1000)
        self.assertEquals(remainder, expected)
    ...
    def test_batch_reader_with_limit(self):
        ...
        remainder = batch.next()
        expected = range(limit / batch_size * batch_size, limit)
        self.assertEquals(remainder, expected)
...
class TestIterableDataset(unittest.TestCase):
    def test_iterable_test_mode(self):
        ...
        for prev_id in id_array:
            data,label,id = enqueued.next()
            self.assertEquals(prev_id, id)
