...
class MakeDict(DoFnWithCounters):
    ...
    def _make_dict(self, row):
        import csv
        values = csv.reader([row.encode('utf-8')], delimiter='|', escapechar='\\').next()
        values = [unicode(val, 'utf-8') for val in values]
        row_dict = {}
...
def make_sessions(driver_input, session_generator_config, dataflow_args, output_sessions, tmp_dir):
    ...
    _ = (data | beam.CoGroupByKey()
            ...
            | 'ignore empty and large sessions' >> beam.Filter(lambda (user, _events): 0 < len(_events) < 1000)
