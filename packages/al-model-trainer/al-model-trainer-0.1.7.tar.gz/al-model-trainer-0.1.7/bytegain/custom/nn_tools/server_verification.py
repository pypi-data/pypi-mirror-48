...
def get_input_rows(config, fields, sortby="random()"):
    ...
    with read_from_db.create_connection(args.redshift_url) as connection:
        all_rows = []
...
def process_search(db_rows, train_rows, row_handler):
    new_train_rows = {}
    for k, v in train_rows.iteritems():
        # print v
    ...
    rh_keys.add('unit_id')
    for id, user_db_rows in db_rows.iteritems():
        # print "Id: %s" % id
        if count == args.count:
            break
...
def process_lease(input_rows, train_rows, row_handler):
    ...
    if misses > 1:
        ...
        for k, v in miss_values.iteritems():
            print "%s: %s" % (k, str(["%s=%s" % (x, y) for x, y in v.iteritems()]))
...
if args.local_model:
    ...
    classifier = None
    connection = httplib.HTTPSConnection(args.server)

train_rows = get_train_predictions(input_rows.keys(), model_spec.get_validation_file())
