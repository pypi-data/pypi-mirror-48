...
def find_repo_helper(error_on_no_identity=False):
...
@click.option(
    ...
    "--filter-by",
    multiple=True,
    help="Functions to filter by.  Takes two arguments, the name of a function and the query value.",
    type=(str, str),
)
@click.option(
    "--long-ids",
...
def list_(sort_by, filter_by, long_ids, reverse_sort, max_title_length, all):
    """
...
def label(issue, label, delete):
    ...
    try:
        new_id = func(repo_info["root"], repo_info["name"], issue["id"], set(label))
    except data.HiveDataException as e:
        click.echo(colored("ERROR: %s" % e.args[0], "red"), err=True)
        sys.exit(1)
    ...
    if delete:
        click.echo(
            colored(
                "Removed the following labels from issue %s:" % (issue["id"],), "green"
            )
        )
    else:
        click.echo(
            colored("Added the following labels to issue %s:" % (issue["id"],), "green")
        )
    for l in label:
        click.echo(colored(l, attrs=["reverse"]))
