...
class Session:
    ...
    async def handle_select(self, wspg_msg: Dict[str, Any]) -> Optional[JsonLiteral]:
        ...
        return await self._select(
            ...
            _slice, chronicle_note)

    async def _select(self, relation_sql: psycopg2.sql.Composable, query_rules: Optional[QueryRulesRuleSetParser], returning_columns: Optional[List[str]], order: Optional[List[Dict[str, str]]], limit: Optional[int], _slice: Optional[Dict], chronicle_note: Optional[str]) -> Optional[JsonLiteral]:
        fetch_result = True
        ...
        return await self._execute_sql(select_sql, fetch_result, chronicle_note)
    ...
    async def handle_insert(self, wspg_msg: Dict[str, Any]) -> Optional[JsonLiteral]:
        ...
        return await self._insert(relation_sql, rows, returning_columns, chronicle_note)

    async def _insert(self, relation_sql: psycopg2.sql.Composable, rows: List[Dict], returning_columns: Optional[List[str]], chronicle_note: Optional[str]) -> Optional[JsonLiteral]:
        ...
        return await self._execute_sql(insert_sql, fetch_result, chronicle_note)
    ...
    async def handle_update(self, wspg_msg: Dict[str, Any]) -> Optional[JsonLiteral]:
        ...
        return await self._update(relation_sql, updates, query_rules, returning_columns, chronicle_note)

    async def _update(self, relation_sql: psycopg2.sql.Composable, updates: Dict, query_rules: Optional[QueryRulesRuleSetParser], returning_columns: Optional[List[str]], chronicle_note: Optional[str]) -> Optional[JsonLiteral]:
        updates_sqls = []
        ...
        return await self._execute_sql(update_sql, fetch_result, chronicle_note)
    ...
    async def handle_delete(self, wspg_msg: Dict[str, Any]) -> None:
        ...
        await self._delete(relation_sql, query_rules, chronicle_note)

    async def _delete(self, relation_sql: psycopg2.sql.Composable, query_rules: Optional[QueryRulesRuleSetParser], chronicle_note: Optional[str]) -> None:
        if query_rules:
            where_sql = psycopg2.sql.SQL("WHERE {}").format(query_rules)
        else:
            where_sql = psycopg2.sql.SQL("")
        ...
        await self._execute_sql(delete_sql, False, chronicle_note)
    ...
    async def _execute_sql(self, sql: psycopg2.sql.Composable, fetch_result: bool, chronicle_note: Optional[str]) -> Optional[JsonLiteral]:
        try:
            async with self.endpoint.db_pool.acquire() as connection:
                async with connection.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
                    ...
                    return await self._execute_sql_on_cursor(cursor, sql, fetch_result)
