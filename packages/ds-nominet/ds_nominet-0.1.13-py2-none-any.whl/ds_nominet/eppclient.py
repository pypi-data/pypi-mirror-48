...
#            time.sleep(1)
    ...
    def domain_create(self, domain, user, password, contactid):
        return self._send_xml('domain_create', DOMAIN=domain, USER=user, PASSWORD=password, CONTACTID=contactid, DOMAIN_PERIOD=2)

    def domain_check(self, domain, user):
        return self._send_xml('domain_check', DOMAIN=domain, USER=user)

    def domain_info(self, domain, user, password):
        return self._send_xml('domain_info', DOMAIN=domain, USER=user, PASSWORD=password)

    def contact_update(self, domain):
        return self._send_xml('contact_update')

    def contact_check(self, user, contactid):
        return self._send_xml('contact_check', USER=user, CONTACTID=contactid)

    def contact_info(self, user, contactid):
        return self._send_xml('contact_info', USER=user, CONTACTID=contactid)

    def contact_create(self, user, password, contactid, name, company, street1, street2, city, country, postcode, country_code, phone, email):
        return self._send_xml('contact_create', USER=user, PASSWORD=password, CONTACTID=contactid, NAME=name, COMPANY=company, STREET1=street1, STREET2=street2, CITY=city, COUNTRY=country, POST_CODE=postcode, COUNTRY_CODE=country_code, PHONE=phone, EMAIL=email)
