...
class PhotoInline(admin.StackedInline):
    ...
    def photo(self, obj):
        ...
        return mark_safe("""<div class="thumb">%s</div>""" % html)
    photo.allow_tags = True
    photo.short_description = 'Podgląd'
...
class PhotoAdmin(ModelAdmin):
    ...
    def photo_thumb(self, obj):
        html = obj.image.render(100)
        return mark_safe("""<span class="thumb">%s</span>""" % html)
    photo_thumb.allow_tags = True
    photo_thumb.short_description = _("Zdjęcie")

    def photo_preview(self, obj):
        url = obj.image.get_url()
        html = obj.image.render(100)
        return mark_safe("""<span class="thumb"><a href="%s">%s</a></span>""" % (url, html))
    photo_preview.allow_tags = True
    photo_preview.short_description = _("Podgląd")

    def size(self, obj):
        _size = obj.get_size()
        return "%sx%s" % _size if _size else _("(Brak)")
    size.short_description = _("Rozmiar")

    # actions
    def change_watermark(self, request, queryset):
        title = _("Zmień znak wodny")
    ...
    change_watermark.short_description = _("Zmień znak wodny")
    ...
    def change_watermark_action(self, request):
        ...
        self.message_user(request, _("Znak wodny został zmieniony pomyślnie"))
