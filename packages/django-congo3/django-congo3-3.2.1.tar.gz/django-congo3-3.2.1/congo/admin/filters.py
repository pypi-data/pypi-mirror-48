    ...
    def choices(self, cl):
        ...
        for lookup, title in self.lookup_choices:
            yield {
                ...
                'query_string': cl.get_query_string({self.lookup_kwarg: str(lookup)}, [self.lookup_kwarg_isnull]),
                'display': title,
            }
        yield {
...
class ContentTypeFilter(SimpleNoneListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        return [(content_type.id, str(content_type)) for content_type in content_types]
