...
def get_sha1(*args, **kwargs):
    ...
    result = hashlib.sha1(text.encode('utf-8')).hexdigest()[:lenght]
...
def get_md5(*args, **kwargs):
    ...
    upper = kwargs.get('upper', True)
    result = hashlib.md5(text.encode('utf-8')).hexdigest()[:lenght]
    return result.upper() if upper else result.lower()

def encrypt(clear_text, secret_key = None):
...
def decrypt(cipher_text, secret_key = None):
    if secret_key is None:
        secret_key = settings.SECRET_KEY[:32]
    aes = AES.new(secret_key)
