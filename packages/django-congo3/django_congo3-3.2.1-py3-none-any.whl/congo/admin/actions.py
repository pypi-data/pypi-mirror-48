...
def change_date_range(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, _("Zakres dat zmieniony: %s") % rows_updated)
change_date_range.short_description = _("Zmień zakres dat na 100 lat")

def make_visible(modeladmin, request, queryset):
    rows_updated = queryset.update(is_visible = True)
    modeladmin.message_user(request, _("Zmieniono na widoczne: %s") % rows_updated)
make_visible.short_description = _("Zmień wybrane na widoczne")

def make_invisible(modeladmin, request, queryset):
    rows_updated = queryset.update(is_visible = False)
    modeladmin.message_user(request, _("Zmieniono na niewidoczne: %s") % rows_updated)
make_invisible.short_description = _("Zmień wybrane na niewidoczne")

def make_active(modeladmin, request, queryset):
    rows_updated = queryset.update(is_active = True)
    modeladmin.message_user(request, _("Zmioniono na aktywne: %s") % rows_updated)
make_active.short_description = _("Zmień wybrane na aktywne")

def make_inactive(modeladmin, request, queryset):
    rows_updated = queryset.update(is_active = False)
    modeladmin.message_user(request, _("Zmieniono na nieaktywne: %s") % rows_updated)
make_inactive.short_description = _("Zmień wybrane na nieakywne")
