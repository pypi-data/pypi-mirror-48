...
def render_pagination_size_widget(context):
    ...
    if 'request' in context:
        ...
        getvars_key_list = list(getvars.keys())
        if len(getvars_key_list) > 0:
            payload['getpagingvars'] = list(zip(getvars_key_list, list(getvars.values())))
        else:
...
def get_search_form(context):
    ...
    if 'request' in context:
        ...
        if 'q' in getvars:
            ...
            del getvars['q']
            return {'getvars':list(getvars.items()), 'searchterm': searchterm}
        else:
...
def paginator(context, adjacent_pages = 3):
    ...
    page_nums = list(range(max(1, datagrid.page.number - adjacent_pages), min(datagrid.paginator.num_pages, datagrid.page.number + adjacent_pages) + 1))
