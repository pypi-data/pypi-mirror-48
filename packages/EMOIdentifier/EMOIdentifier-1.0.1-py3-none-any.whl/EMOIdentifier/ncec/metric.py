...
#	This method reads list of producers present in the block under consideration.
...
def getAffinityFlag(affinity,target):
...
#				check if vertex 'i' supplies data only to the statements of the block rooted at 'target'
				if i < v and graph.isSource(i,adj)and config.ctrlParentMap[i] == config.ctrlParentMap[target]: # i.e., 'i' is a source vertex
					count = 0						# ctrlParentMapt condition is added to prevent source inclusion of outer exc sources
...
#	dCount = float(len(dEdgeHash))		# It was being used in version 3.0; now we will compute accumulative 
	...
	if config.flag['NoPrEx']:	#	This flag on means that extract zero producer blocks also. 
		if pCount == 0 and accuDCount>0:
			affinity = 1/accuDCount
			config.log.write("\n\t\t Block :" + str(target) + "Affinity :" + str(affinity) + "\n")
			return getAffinityFlag(affinity,target)
		elif accuDCount == 0 :	#	pCount == 0 or accuDCount == 0 was the condition earlier ; modified on Oct 30th, 2018
