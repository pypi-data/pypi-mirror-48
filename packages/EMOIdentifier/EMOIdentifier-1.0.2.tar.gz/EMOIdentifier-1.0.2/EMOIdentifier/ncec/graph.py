...
def extractCtrlBlock(target, adj, ctrList):
	adj = contractCtrlBlock(target,adj)
...
def contractCtrlBlock(target, adj):	#	CHECK THIS CODE IF THE NESTED CTRL BLOCKS ARE CONTRACTING
	tmplist = []	#	It is used for temprary purpose
	size = len(adj)
	for v in range(0,size):
		if adj[target][v] ==1:
			edge_contraction(target,v,target,adj)
...
def edge_contraction(i,j,dom,adj):	
...
#	code below is merging two vertices/segments and deleting 'poor' segment
	...
	if poor in config.segList:
		...
		if not config.segment.has_key(poor):
			config.log.write("***Error: (poor not found) ; graph.py: edge_contraction() called from "+ inspect.stack()[1][3] +"\n")
			l = config.segment[dom]
		else:	
