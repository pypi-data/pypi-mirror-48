...
def caltechdata_edit(token,ids,metadata={},files={},delete={},production=False,auth=None):
    ...
    if auth:
        strv = auth[0]+":"+auth[1]
        basic = base64.b64encode(strv.encode('utf-8')).decode("ascii")
        headers = {
        'Authorization' : 'Basic %s, Bearer %s' % (basic,token),
        'Content-type': 'application/json'
    }
    ...
    for idv in ids:
        ...
        if files:
            ...
            # upload new
            print(files)
            fileinfo = [send_s3(f, token, production, auth) for f in files]
...
def caltechdata_add(token,ids,metadata={},files={},production=False,auth=None):
    '''Adds file'''
