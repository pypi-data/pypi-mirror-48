...
class InstanceV(DockerV):
    ...
    def get_menus(self, database):
        ...
        menus = self.execute_query(db_config, sql)
        for menu in menus:
            res.update({
                menu['xml_id']: menu
            })
    ...
    def menu_tree(self, menu_id, database):
        ...
        db_config.update({'db_name': database})
        db_config = utils.odoo2postgres(db_config)
        tree = self.execute_query(db_config, sql.format(menu=menu_id))
        if tree:
            res = tree[0]
        return res
    ...
    def deactivate_database(self, db_name, extra_config=False):
        """Executes the deactivation queries found in the deactivate.jinja file
