...
cdef class HMMTrainer:
    ...
    cdef void init_align_dist(self) except *:
        ...
        uniform_dist = np.zeros([max_len_src, 2*max_len_src-1], np.float64)
        ...
        msg = "word alignment distribution matrix size: {} [max_src_len+1] x {} [-max_src_len:max_src_len] x {} [bytes] = {:,d} [bytes]"
        logger.info(msg.format(max_len_src,max_len_src*2-1,uniform_dist.itemsize,uniform_dist.nbytes))
    ...
    cdef void train(self, int iteration_limit) except *:
        ...
        self.setup()
        logger.info("----")
        logger.info("start training HMM Model")
    ...
    cdef void train_step(self) except *:
        if self.model.trans_dist is None:
            ...
            HMMTrainer.expect_step(self)
            HMMTrainer.maximize_step(self)
