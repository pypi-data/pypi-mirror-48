...
class Token(pygame.sprite.DirtySprite):
    ...
    def __init__(self, position, position_y = None):
        self.board = None
        super().__init__()
        self.costume = None
        # private
        ...
        elif type(position) == int and type(position_y) == int:
            self.position : board_position = board_position.BoardPosition(position, position_y)
        else:
            raise TypeError("position argument for token {0} is neither (int, int nor tuple")
