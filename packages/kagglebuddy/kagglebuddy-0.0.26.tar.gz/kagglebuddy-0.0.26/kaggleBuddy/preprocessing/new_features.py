...
def create_groupby_features(df, group_columns_list, method_dict, add_to_original_data=False, verbose=1, verbose_detail="create stats features", suffix=''):
...
def create_w2v_features(data, prefix, col_groupby, col_vector, size, window):
    """ group by [col_groupby] and apply list on [col_vector], then do W2V on col_vector

    """
    df = data[[col_groupby, col_vector]]
    type_org = df[col_vector].dtype
    df[col_vector] = df[col_vector].astype(str)
    df[col_vector].fillna('NAN', inplace=True)
    df = df.groupby(col_groupby, as_index=False)[col_vector].agg({'list': (lambda x: list(x))}).reset_index(drop=True)

    doc_list = list(df['list'].values)
    w2v = Word2Vec(doc_list, size=size, window=window, min_count=1)
    vocab_keys = list(w2v.wv.vocab.keys())
    dict_w2v = {}
    for key in vocab_keys:
        dict_w2v[key] = w2v.wv[key]

    df_w2v = pd.DataFrame(dict_w2v).T.reset_index()
    df_w2v.columns = [col_vector] + [prefix + 'w2v_%s_dim_%d' % (col_vector, x) for x in range(size)]
    df_w2v[col_vector] = df_w2v[col_vector].astype(type_org)
