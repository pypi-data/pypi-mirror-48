...
from onepanel.utilities.gcp_cs.file_sync import FileSynchronizer as FileSynchronizerGCP, \
    ThreadedFileSynchronizer as ThreadedFileSynchronizerGCP
...
class UploadUpdateThread(threading.Thread):
    def __init__(self, thread_id, thread_name, vc):
    ...
    def run(self):
        ...
        while True:
            ...
            # TODO - this needs to be redone so we keep track of it locally....not request s3 side.
            update_upload_url = '/update_upload'
            upload_data = {
                'bytesCurrent': 404,  # int(file_info_cloud_provider['data']['total_bytes']),
                'filesCurrent': 404,  # int(file_info_cloud_provider['data']['total_files']),
            }
...
class UploadDataset:
    ...
    def check_upload(self):
        ...
        if directory_stats['num_files'] < 1:
            return False
    ...
    def upload(self, version, quiet=False):
        provider_storage_push_to_dir = version['data']['version']['path']
        CloudStorageUtility.sync_files(self.account_uid, 'datasets', self.dataset_uid, self.directory,
                                       provider_storage_push_to_dir, quiet=quiet)

    def upload_finished(self, directory_stats):
        upload_data = {
            'bytesCurrent': directory_stats['size'],
            'filesCurrent': directory_stats['num_files'],
        }
    ...
    def run(self, vc, account_uid, dataset_uid, directory=None, quiet=False):
        """
        :param vc:
        ...
        try:
            ...
            self.upload_started()
            self.upload(upload_info['version'], quiet)
            self.upload_finished(upload_info['directory_stats'])
...
class InformedUploadDataset(UploadDataset):
    ...
    def upload_started(self):
        UploadDataset.upload_started(self)

        self.status_uploader = UploadUpdateThread(1, 'updater', self.vc)
        self.status_uploader.start()

    def upload_finished(self, directory_stats):
        UploadDataset.upload_finished(self, directory_stats)
        self.status_uploader.exitFlag = True
...
@click.pass_context
@login_required
def datasets_push(ctx, message, name, update_version, quiet, background, watch, in_background):
    _datasets_push(ctx, message, name, update_version, quiet, background, watch, in_background)


def _datasets_push(ctx, message, name, update_version, quiet, background, watch, in_background):
    if background:
        cmds = sys.argv
        cmds.append('--in-background')
        run_in_background(cmds)
        return
    ...
    dataset_update.run(vc, vc.account_uid, vc.dataset_uid, quiet=quiet)
...
def sync(ctx, skip_check, master=FileSynchronizer.LOCAL, threads=10):
    ...
    if cloud_provider == 'GCP':
        ...
        synchronizer = ThreadedFileSynchronizerGCP(synchronizer, threads, [FileSynchronizer.print_status(cwd_length, master)])
        ...
        synchronizer = ThreadedFileSynchronizer(synchronizer, threads, [FileSynchronizer.print_status(cwd_length, master)])
