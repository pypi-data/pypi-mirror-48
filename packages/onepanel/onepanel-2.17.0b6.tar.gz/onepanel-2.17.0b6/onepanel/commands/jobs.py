...
class JobViewController(APIViewController):
    ...
    def update_config(self, project_uid=None, account_uid=None, caller_is_sdk=False):
        if not project_uid and not self.initial_project_uid:
            print('Error: Project ID cannot be blank.')
            if caller_is_sdk:
        ...
        if not account_uid and not self.initial_account_uid:
            print('Error: Account ID cannot be blank.\nRun \'onepanel login\' to login.')
            if caller_is_sdk:
                return False
            else:
                sys.exit(1)
...
@click.option(
    ...
    type=str,
    required=True,
    help='Machine type ID. Run "onepanel machine-types list" for a list of IDs.'
)
@click.option(
    '-e', '--environment',
    type=str,
    required=True,
    help='Environment ID. Run "onepanel environments list" for a list of IDs.'
)
@click.option(
    '-s', '--storage',
    type=str,
    required=True,
    help='Volume type ID. Run "onepanel volume-types list" for a list of IDs.'
)
@click.option(
    '-c', '--code-repository',
