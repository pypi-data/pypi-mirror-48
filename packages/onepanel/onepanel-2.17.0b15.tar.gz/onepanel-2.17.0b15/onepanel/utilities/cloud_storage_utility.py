...
class CloudStorageUtility:
    ...
    @staticmethod
    def get_utility(account_uid=None, entity_type=None, entity_uid=None):
        util = None
        cloud_provider = os.getenv('CLOUD_PROVIDER', 'AWS')
        if cloud_provider == 'GCP':  # TODO
            util = GCPUtility()
        elif cloud_provider == "AWS":
            if CloudStorageUtility.cached_credentials_provider is None:
    ...
    def sync_files(account_uid, entity_type, entity_uid, filepath, s3_prefix, s3_wrapper=None,
                   ...
                   quiet=False, include=None, prefix=None):
        if s3_wrapper is None:
            s3_wrapper = CloudStorageUtility.get_utility(account_uid, entity_type, entity_uid)
