...
class DatasetIdentifier:
    ...
    def __init__(self, path):
        ...
        :param path: account_uid/datasets/dataset_uid[/version][/subpath].
                     if version is none, 'current' is assumed, which is the most up to date.
        :type path str
        """
        values = path.split('/')
        ...
        if len(values) > 3:
            self._version = values[3]

        self._subpath = None
        if len(values) > 4:
            self._subpath = '/'.join(values[4:])
...
class UploadDataset:
    ...
    def upload(self, version, quiet=False):
        ...
        CloudStorageUtility.sync_files(self.account_uid, 'datasets', self.dataset_uid, self.directory,
                                       provider_storage_push_to_dir, quiet=quiet, ignore_file_states=ignore_file_states)
