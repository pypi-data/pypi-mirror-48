...
class RAWG(metaclass=utils.Singleton):
    ...
    def pagination_generator(self, url, results_name="results", next_name="next"):
    ...
    def game_request(self, slug, additional_param="") -> dict:
        ...
        :rtype: dict
        """
        url = "https://rawg.io/api/games/{slug}".format(slug=slug)
        return self.get_request(url)

    def search(self, query, num_results=5, additional_param=""):
        """searches for a game and returns a list of :class:`~rawgpy.game.Game`
    ...
    def game(self, slug):
        """uses the :attr:`~rawgpy.RAWG.game_request` method to get a specific game
    ...
    def user_request(self, slug):
        """Returns a user json

        :param slug: the users slug
        :type slug: str
        """
        return self.get_request("https://rawg.io/api/users/{slug}".format(slug=slug))
    ...
    def login(self, email, password):
        ...
        response = self.post_request(
            "https://rawg.io/api/auth/login", {"email": email, "password": password})
    ...
    def user_games(self, slug, status=None):
        ...
        """
        if not status:
            return self.pagination_generator("https://rawg.io/api/users/{}/games".format(slug))
        else:
            return self.pagination_generator("https://rawg.io/api/users/{}/games?statuses={}".format(slug, status))
    ...
    def game_suggestions(self, slug):
        ...
        :type slug: str
        """
        return self.pagination_generator("https://rawg.io/api/games/{}/suggested".format(slug))

    def review_data(game_id: int, level, reactions=None, add_to_library=False, text="", post_twitter=False, post_facebook=False):
    ...
    def review_game(self, game_id: int, level, reactions=None, add_to_library=False, text="", post_twitter=False, post_facebook=False):
        ...
        :type post_facebook: bool, optional
        """
        self.post_request("https://rawg.io/api/reviews",
    ...
    def game_collections(self, game_slug):
        ...
        return self.pagination_generator(
            "https://rawg.io/api/games/{}/collections".format(game_slug))

    def collection_games(self, slug):
        """generator that yields the collections games json"""
        return self.pagination_generator(
            "https://rawg.io/api/collections/{}/feed".format(slug))

    def collection_request(self, slug):
        """Returns the collection json

        :param slug: the colection slug
        :type slug: str
        """
        return self.get_request("https://rawg.io/api/collections/{}".format(slug))

    def current_user_request(self):
        """Returns the currently authenticated user json
        """
        json = self.get_request("https://rawg.io/api/users/current")
        if json.get("detail", "").startswith("Authentication credentials"):
            raise AttributeError(json["detail"])
        return json

    def current_user(self):
        """Returns the currently authenticated :class:`~rawgpy.user.User`
        """
    ...
    def patch_game(self, slug, data):
        ...
        :type data: dict
        """
        return self.patch_request("https://rawg.io/api/games/{}".format(slug), data)
