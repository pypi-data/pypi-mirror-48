...
class Basis:
    ...
    .. note ::
        Any given :class:`Plane` has an infinite number of associated `Basis`
        constructions.
...
class PlanarPolygon:
    ...
    @property
    def points(self):
        return [p for polygon in self._polygons()
                for p in polygon.points]

    def _polygons(self):
        return [polygon for exterior in [True, False]
                for polygon in self.project(exterior)]

    def segments(self):
        return [s for polygon in self._polygons()
