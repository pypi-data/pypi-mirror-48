...
def get_dependencies(file):
    ...
    """
    # for python v3.5 downwards compatibility -> str(file)
...
def get_modules_dependencies(modules=None, file=None, base_dir=False):
    ...
    for m in modules: # size around 10.
        ...
        if addon_path not in addon_module_paths:
            raise ValueError(" %s is not a valid module" % m)
        try:
...
if __name__ == "__main__":
    ...
    s = resolve_dependencies(modules=['account', 'crm', 'delivery'],base_dir='/Users/yinet/Desktop/odoo/addons')
