...
Release 1.5.4 is a bug-fix release addressing a few bugs present in 1.5.3.
It also includes some changes to make the behavior more compatible with 1.6.x.

Platform changes:
^^^^^^^^^^^^^^^^^

* Json structures can be built with numpy scalars and arrays of supported scalar
  types.
* Plots in Table viewer shows current resampling.
* Future compatibility: nodes with parameters containing unknown parameter types
  (such as json which was added in 1.6.0) can be configured.
* Fixed incorrect index reported on exception for several list nodes on
  Python 2.

Node changes
^^^^^^^^^^^^
* Improved legend handling in :ref:`Figure Compressor`. Behavior should now be
  the same as in 1.6.
* Fixed exception converting some columns using masked arrays
  in :ref:`org.sysess.sympathy.data.table.converttablecolumns`.

* :ref:`Calculators<Calculator>`:

  * Allow plugins to hide items from the tree of common functions.
  * Fixed preview not working in Python 2.
  * Fixed dragging of results not woring in Python 2.

* Fixed missing plots in output from :ref:`Report Apply<Report Apply Tables>`.
