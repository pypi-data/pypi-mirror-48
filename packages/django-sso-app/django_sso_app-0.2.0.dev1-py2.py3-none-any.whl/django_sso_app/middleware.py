...
class SsoMiddleware(MiddlewareMixin):
    ...
    header = settings.DJANGO_SSO_APIGW_HEADER
    # Username for anonymous user
    anonymous_username = getattr(settings, "DJANGO_SSO_ANONYMOUS_USERNAME",
                                 "anonymous")
    ...
    def process_request(self, request):
        ...
        try:
            ...
            if consumer_username == self.anonymous_username:
                ...
                return
            encoded_jwt = request.COOKIES["jwt"]
        except KeyError:
            ...
            if user_profile.sso_id == sso_id:
                if request.user.is_active:
                    if settings.DJANGO_SSO_REPLICATE_PROFILE:
                        # Check if sso_rev changed and, if so, fetch profile from
        ...
        user = auth.authenticate(consumer_username=consumer_username,
