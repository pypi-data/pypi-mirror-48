...
class Document(metaclass=DocumentMeta):
    ...
    def __init__(self, *args, saved=False, **kwargs):
        ...
        for key, value in kwargs.items():
            if not saved:
                if not value is None and type(value) in _staticTypes:
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self, 'id') and self.id and key is "_id":
    ...
    def __setitem__(self, key, value):
        self._saved = False
        if not self._unsavedItems.get(key, None):
    ...
    def _filtered_attributes(self, ignore_settings=False) -> dict:
        ...
        if hasattr(self, 'id') and self.id and ignore_settings is False:
            # Check to see if preserve_id is set
            if self._preserve_ids or self._restore_id:
                attr['_id'] = attr['id']
