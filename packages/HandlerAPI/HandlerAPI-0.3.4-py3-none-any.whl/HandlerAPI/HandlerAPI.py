...
class HandlerAPI:
    def __init__(myself, url, api_data, api_commit)
        myself.url = url
        myself.api_data = api.data
        myself.api_commit = api_commit

    def parsed_api(url):
        response = requests.get(myself.url)
        data = response.text
    ...
    #     arr = [] #array
        ...
        for i in range (len(myself.api_data)):
            temp = myself.api_data[i]['user']['login'] #temp untuk menyimpan nama developer
            count+=1
    ...
    def count_state_commit(myself.api_data):
    #     arr = [] #array
        data = {} #dictionary{key:value}
        count = 0
        user_state = {}
        ...
        for i in range (len(myself.api_data)):
            temp = myself.api_data[i]['author']['login'] #temp untuk menyimpan nama developer
            count+=1
    ...
    #     temp = {}
        locArr = []
        for i in range(len(myself.api_commit)):
            a = myself.api_commit[i]['weeks']
            arrA = []
            for j in range(len(myself.api_commit[i]['weeks'])):
                tempDict = myself.api_commit[i]['weeks'][j]
                arr = []
        ...
        for i in range(len(loc_tot)):
            developer = myself.api_commit[i]['author']['login']
