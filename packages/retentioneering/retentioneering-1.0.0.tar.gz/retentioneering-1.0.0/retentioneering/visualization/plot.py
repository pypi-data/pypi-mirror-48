...
def _make_json_data(data, node_params, layout_dump, thresh=.05, width=500, height=500, **kwargs):
    ...
    data = data[data.weight.abs() >= thresh].copy()
...
def __save_plot__(func):
    ...
    def save_plot_wrapper(*args, **kwargs):
        ...
        vis_object.get_figure().savefig(name)
        return res
    return save_plot_wrapper


@__save_plot__
def step_matrix(diff, plot_name=None, **kwargs):
    """
    ...
    heatmap = sns.heatmap(diff, annot=True, cmap="BrBG", center=0)
...
def cluster_tsne(data, clusters, target, plot_name=None, **kwargs):
    ...
    if hasattr(data.retention, '_tsne'):
        ...
        tsne2 = data.retention.learn_tsne(target, **kwargs)
    tsne = tsne2.values
    if np.unique(target).shape[0] > 10:
