...
class Camera(Thread):
    ...
    def __init__(self, config, **kwargs):
        ...
        if os.environ.get("OUTPUT_TIF", "true") in ("false", "off", "n", "0"):
            if "tif" in self.output_types:
                self.output_types.remove("tif")
            if "tiff" in self.output_types:
                self.output_types.remove("tiff")
        if os.environ.get("OUTPUT_JPG", "true") in ("false", "off", "n", "0"):
            if "jpg" in self.output_types:
                self.output_types.remove("jpg")
            if "jpeg" in self.output_types:
                self.output_types.remove("jpeg")
