...
#   array[8] is pause rate (number of pauses / seconds of silent duration)
#   array[9] is speaking-to-total ratio (ratio of speaking duration to total duration)
#   array[10] is pronunciation probability score
def analysis_array(m, p):
    ...
    array.append(gender(m, p, objects))
    array.append(mood(m, p, objects))

    syllables = objects[1].strip().split()[0];
    array.append(objects[1].strip().split()[0]) # syllables

    pauses = objects[1].strip().split()[1];
    array.append(objects[1].strip().split()[1]) # pauses

    total_duration = objects[1].strip().split()[5];
    array.append(total_duration) # total duration

    speaking_duration = objects[1].strip().split()[4]
    array.append(speaking_duration) # speaking duration

    silent_duration = str(float(total_duration) - float(speaking_duration));
    array.append(silent_duration) # silent duration

    array.append(str(float(syllables) / float(speaking_duration))) # speech rate
    array.append(str(float(pauses) / float(silent_duration))) # pause rate
    array.append(str(float(speaking_duration) / float(total_duration))) # speaking-to-total

    array.append(pronunciation(m, p, objects))
