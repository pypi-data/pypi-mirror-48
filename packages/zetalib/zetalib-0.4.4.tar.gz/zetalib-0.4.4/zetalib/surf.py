...
try:
    if resource_exists('zetalib', 'crunch.so'):
        common.libcrunch = ctypes.CDLL(resource_filename('zetalib', 'crunch.so'))
except Exception as e:
    logger.warning('The C extension crunch.so could not be loaded. '
                'Computations of topological zeta functions will use '
                'the slower Python implementation.')
    # To debug uncomment:
    #logger.warning(e)
        ...
        with gzip.open(dfile, 'rb') as gzfile:
            ...
            while True:
                ...
                try:
                    ...
                    logger.debug("file %d reached EOF", dfile)
                if not raw:
                    break # EOF
...
def multicrunch(surfsums, varname=None):
    ...
    R = QQ[varname]
    s = R.gen(0)
    g = R(prod((a*s-b)**e for ((a,b),e) in six.iteritems(cand)))
    m = g.degree()
