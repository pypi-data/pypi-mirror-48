...
class SubobjectDatum(ZetaDatum):
    ...
    def reduce(self, preemptive=False):
        chops = self.strategy.prechopper(self.toric_datum, list(range(len(self.toric_datum.rhs)))) if preemptive else self.strategy.chopper(self.toric_datum, self.toric_datum._coll_idx)
        for C in chops:
