...
class ToricDatum(object):
    ...
    def __init__(self, ring, integrand, cc, initials, polyhedron, depth=0):
        ...
        idx = [i for i in range(len(cc)) if (cc[i][1] != Integer(0))] # and (not cc[i][0].is_constant())]
        ...
        self.cc = list(zip(self.lhs, self.rhs))
    ...
    def _purge_multiples(self):
        ...
        mins = minimal_elements(list(range(len(self.rhs))), lte)
    ...
    def simplify(self):
        ...
        while true:
            ...
            idx_mon = [i for i in range(len(T.cc)) if (T.initials[i] is not None) and T.initials[i].is_monomial()]
            idx_nonmon = [i for i in range(len(T.cc)) if not i in idx_mon]
            ...
            for i in idx_nonmon:  # or 'xrange(len(T.cc))', to be safe
                ...
                idx_red = [ j for j in range(len(mon))
    ...
    def order(self):
        ...
        I = list(range(len(self.rhs))) if not self._coll_idx else self._coll_idx
    ...
    def __str__(self):
        ...
        width = max([len([i for i in f.exponents()[0] if i > 0] ) for f in self.lhs]) * 4 + 2
        for i in range(len(self.cc)):
