...
cdef class HashAdder:
    ...
    # Why the cdef method cause the error:
    # AttributeError: 'zetalib.addmany.HashAdder' object has no attribute 'values'
    #cdef values(HashAdder self):
    def values(HashAdder self):
        return itervalues(self.D)

    def keys(HashAdder self):
        return iterkeys(self.D)
...
cdef class DictPolynomial:
    ...
    def __iadd__(DictPolynomial self, DictPolynomial other):
        cdef uint32_t key
        for key, value in iteritems(other.D):
    ...
    def polynomial(DictPolynomial self, ring):
        ...
        return ring.sum(value.eval() * y**(key & 0xffff) * x**(key >> 16) for key, value in iteritems(self.D))
        ...
        for i in range(0, len(indices), BUCKET_SIZE):
            ...
            if not pid:
                ...
                try:
                    ...
                    print('Unhandled exception in forked process:', e, file=sys.stderr)
                    sys.stderr.flush()
                    os._exit(1)
            else:
...
def _optimise_combine(filenames):
    ...
    HA.simplify()
    rays_of = lambda HA: list(set(x for v in HA for x in v))
