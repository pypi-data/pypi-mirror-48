...
def installation_swa(envTag):
    ...
    os.system("sudo docker-compose -f /var/www/docker-compose.yml run -d --service-ports --name max_swa swa")
...
def installation_minicampus(envTag):
    ...
    os.system("sudo docker-compose -f /var/www/docker-compose.yml run -d --service-ports --name max_minicampus minicampus")
...
def installation_backend(envTag):
    ...
    os.system("sudo docker-compose -f /var/www/docker-compose.yml run -d --service-ports --name max_backend backend")
...
def installation_assessment(envTag):
    ...
    os.system("sudo docker-compose -f /var/www/docker-compose.yml run -d --service-ports --name max_assessment assessment")
...
def getUserInput(miniCampus, dbusername, dbpassword,buildEnv):
    ...
    subprocess.Popen('export NODE_ENV=%s' %buildEnv)
    subprocess.Popen('export MINICAMPUS_CODE=%s' %miniCampus)
    subprocess.Popen('export MONGO_INITDB_ROOT_USERNAME=%s' %dbusername)
    subprocess.Popen('export MONGO_INITDB_ROOT_PASSWORD=%s' %dbpassword)

    subprocess.Popen('echo $NODE_ENV')
    ...
    try:
        ...
        subprocess.Popen('unset NODE_ENV')
        subprocess.Popen('unset MINICAMPUS_CODE')
        subprocess.Popen('unset MONGO_INITDB_ROOT_USERNAME')
        subprocess.Popen('unset MONGO_INITDB_ROOT_PASSWORD')
