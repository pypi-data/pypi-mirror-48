...
def tickers(SIC_codes=[], index=None, company_identifiers=[], entire_universe=False, NAICS_codes=[]):
    """Return a list of tickers in the peer-group"""
    companies = _companies(SIC_codes, index, company_identifiers, entire_universe, NAICS_codes=NAICS_codes)
...
# 0 for annual data, 1,2,3,4 for quarterly data
    period = 0
    info = ['entity_name','sic_code','sic_category','City','Country','State','Zip','naics','is_ifrs','sec_html_url','earnings_release_url','proxy_url','FiscalYearEndDate','primary_currency']
    standardized_data(company_identifiers=['emgc'], 
                                  metrics=['is_ifrs'], 
                                  start_year = year, 
                                  end_year = year,
                                  period_type='annual',
