...
from smrt.core.check_numba import nb
...
"""        
        ...
        assert sensor.mode == 'P'  #  The active model does not work because the diagonalization expect a partir forme for the matrix, which is not the case yet. This may be a bug in the fft calculation or a feature.
        ...
        # original that should work: 
        intensity = intfct(mu)
        # we try to remove the "old" code
        #i = np.argsort(mu)
    ...
    def dort(self, m_max=0, special_return=False):
        ...
        eigenvalue_solver = [_EigenValueSolver(self.emmodels[l].ke,
                                              self.emmodels[l].ks,
    ...
    def dort_modem_banded(self, m, n_stream, eigenvalue_solver, mu, weight, outmu,
        ...
        if len(n_stream) >= 2:
            nband = npol * max(np.max(2 * n_stream[1:] + n_stream[:-1]),
                               np.max(n_stream[1:] + 2 * n_stream[:-1]))
            #print("gain:", nband / (3 * npol * np.max(n_stream)))
        ...
        #bBC = _BoundaryCondition(nboundary, nband)
        bBC = np.zeros((2*nband+1, nboundary))
        ...
        for l in range(0, self.nlayer):
            ...
            transt = np.exp(-np.maximum(beta, 0) * self.thickness[l])[np.newaxis, :]  # positive beta, reference at the bottom
            transb = np.exp(np.minimum(beta, 0) * self.thickness[l])[np.newaxis, :]   # negative beta, reference at the top
            ...
            Rtop_l = self.interfaces[l].specular_reflection_matrix(self.sensor.frequency, self.permittivity[l], epslm1, mu[l, 0:nsl], npol)  # snow-snow

            # fill the matrix
            todiag(bBC, il_topl, j, np.array(Ed - Rtop_l @ Eu) * transt)  # this line perform matrix multiplication between Rtop_l and Eu. Make sure that reflection_matrix return matrix!

            if debug_compute_BC:
                BC[il_topl:il_topl+nslnpol, j:j+nsl2npol] = np.array(Ed - Rtop_l @ Eu) * transt   # a mettre en (l,l), theta<0 et * transt  # a mettre en (l,l)
            ...
            if l < self.nlayer - 1:
                ...
                todiag(bBC, il_top[l+1], j, - np.array(Tbottom_lp1 @ Ed[0:ns_, :]) * transb)
                if debug_compute_BC:
                    BC[il_top[l+1]:il_top[l+1]+ns_, j:j+nsl2npol] = - np.array(Tbottom_lp1 @ Ed[0:ns_, :]) * transb  # a mettre en (l+1,l)
            ...
            if m == 0 and self.temperature is not None and self.temperature[l] > 0:
                if Rtop_l is 0:
                    ...
                    b[il_topl:il_topl+nslnpol, :] -= ((1.0 - mulone(Rtop_l)) * self.temperature[l])[:, np.newaxis]  # a mettre en (l)
                # the mulone comes from the isotropic emission of the black body

                if l < self.nlayer - 1 and self.temperature[l] > 0:
                    b[il_top[l+1]:il_top[l+1]+ns_, :] += (mulone(Tbottom_lp1) * self.temperature[l])[:, np.newaxis]     # a mettre en (l+1)
            ...
            # compute reflection coefficient between l and l-1
            if l < self.nlayer-1:
                Rbottom_l = self.interfaces[l].specular_reflection_matrix(self.sensor.frequency, self.permittivity[l], self.permittivity[l+1], mu[l, 0:nsl], npol)  # snow-snow
            elif self.snowpack.substrate is not None:
                Rbottom_l = self.snowpack.substrate.specular_reflection_matrix(self.sensor.frequency, self.permittivity[l], mu[l, 0:nsl], npol)  # snow-sub
            ...
            # fill the matrix
            if Rbottom_l is 0:
                todiag(bBC, il_bottoml, j, Eu * transb)
            else:
                todiag(bBC, il_bottoml, j, np.array(Eu - Rbottom_l @ Ed) * transb)

            if debug_compute_BC:
                BC[il_bottoml:il_bottoml+nslnpol, j:j+nsl2npol] = np.array(Eu - Rbottom_l @ Ed) * transb  # a mettre en (l,l), theta >0

            if l > 0:
                ns_ = min(nslnpol, nslm1npol)
                Ttop_lm1 = self.interfaces[l].coherent_transmission_matrix(self.sensor.frequency, self.permittivity[l], self.permittivity[l-1], mu[l, 0:(ns_//npol)], npol)  # snow-snow
                todiag(bBC, il_bottom[l-1], j, - np.array(Ttop_lm1 @ Eu[0:ns_, :]) * transt)
                if debug_compute_BC:
                    BC[il_bottom[l-1]:il_bottom[l-1]+ns_, j:j+nsl2npol] = - np.array(Ttop_lm1 @ Eu[0:ns_, :]) * transt  # a mettre en (l-1)
            ...
            if m == 0 and self.temperature is not None and self.temperature[l] > 0:
                if Rbottom_l is 0:
                    ...
                    b[il_bottoml:il_bottoml+nslnpol, :] -= ((1.0 - mulone(Rbottom_l)) * self.temperature[l])[:, np.newaxis]  # a mettre en (l)
                if l > 0:
                    b[il_bottom[l-1]:il_bottom[l-1]+ns_, :] += (mulone(Ttop_lm1) * self.temperature[l])[:, np.newaxis]  # a mettre en (l-1)
            ...
            if m == 0 and l == self.nlayer-1 and self.snowpack.substrate is not None and \
                ...
                Ttop_sub = self.snowpack.substrate.emissivity_matrix(self.sensor.frequency, self.permittivity[l], mu[l, 0:nsl], npol)  # sub-snow
                b[il_bottoml:il_bottoml+nslnpol, :] += (mulone(Ttop_sub) * self.snowpack.substrate.temperature)[:, np.newaxis]
        ...
        if special_return == "BC":
            ...
            return bBC.as_matrix(), b

        if special_return == "testeq":
            # test
            x = scipy.linalg.solve(BC.as_matrix(), b, overwrite_a=True, overwrite_b=False)
            x2 = scipy.linalg.solve_banded((nband, nband), bBC.as_matrix(), b, overwrite_ab=True, overwrite_b=False)
        ...
        I0up_m = Rbottom_air_down @ intensity_down_m + (Ttop_0 @ I1up_m)[:npol*n_stream0]
...
#     for j in range(0, m):  # upper diag of df
#         ldiag = min(n, m-j)
#         bmat[u+oi-oj-j, j+oj:j+oj+ldiag] = df[j:j+(m+1)*ldiag:m+1]

#     for i in range(1, n):  # lower diag of df
#         ldiag = min(n-i, m)
#         bmat[u+oi-oj+i, 0+oj:0+oj+ldiag] = df[i*m:i*m+(m+1)*ldiag:m+1]
...
#     jdiag0 = int(oj + (m-1))
#     idiag0 = int(u + oi - jdiag0)

#     #ab[u + i - j, j] == a[i,j]
#     lendiag = bmat.shape[1]
#     mlendiag = m * (lendiag - 1)

#     for i in range(0, n):  # upper diag of df
#         ind = (idiag0 + i) * lendiag + jdiag0
#         bmat.flat[ind:ind+mlendiag:lendiag-1] = dmat[i, ::-1]

def todiag(bmat, oi, oj, dmat):
    _todiag(bmat, int(oi), int(oj), dmat)
...
if nb:
    #_todiag = nb.jit((nb.float64[:, :], nb.int32, nb.int32, nb.float64[:, :]), 
    #                    nopython=True, cache=True)(_todiag)
    _todiag = nb.jit(nopython=True, cache=True)(_todiag)



class _BoundaryCondition(object):

    def __init__(self, nboundary, u):

        self.u = u
        self.nbands = 2 * u + 1
        self.nboundary = nboundary

        self.data = np.zeros(self.nbands * nboundary)
        #self.data = np.zeros((self.nbands , nboundary))        #np.zeros((2*nband+1, nboundary))  # we use banded Boundary condition matrix

    #@jit(nopython=True)
    def assign_block(self, ij, dmat):
        #"""insert the small dense dmat matrix in the diagonal bmat matrix"""
        oi, oj = ij
        ...
        jdiag0 = oj + (m-1)
        idiag0 = self.u + oi - jdiag0

        #ab[u + i - j, j] == a[i,j]
        lendiag = self.nboundary
        mlendiag = m * (lendiag - 1)

        for i in range(0, n):  # upper diag of df
            ind = (idiag0 + i) * lendiag + jdiag0
            self.data[ind:ind+mlendiag:lendiag-1] = dmat[i, ::-1]

    def as_matrix(self):
        return self.data.reshape((self.nbands, self.nboundary))
...
class _EigenValueSolver(object):
    ...
    def solve(self, m, compute_coherent_only):
        ...
        # compute invmu
        invmu_s = 1.0 / self.mu
        invmu_s = np.repeat(invmu_s, npol)
        mu_i = np.concatenate((self.mu, -self.mu))  # first upwelling, second downwelling
        ...
        if self.ft_even_phase is None or compute_coherent_only:
            ...
            A = self.ft_even_phase(m, self.mu, mu_i)

        if A is 0:
            # the solution is trivial
            beta = invmu_s * np.repeat(self.ke(mu_s), npol)
            Eu = np.eye(n, n)
            Ed = np.zeros((n, n))
        else:
            ...
            A[np.diag_indices(n)] += np.repeat(self.ke(self.mu), npol)
            A = invmu_s[:, np.newaxis] * A

            # The A matrix in pi2018 is the opposite of the matrix in Stamnes 81, but 
            # we use the mu_s>0 part of the matrix, both signs cancell.
            Adiag = A[0:n, 0:n]
            Aoffdiag = A[0:n, n:]
            ...
            difA = Adiag-Aoffdiag
            Ap = difA @ sumA
            #Am = (Adiag+Aoffdiag) * (Adiag-Aoffdiag)

            #assert np.allclose(A[0:n, 0:n], -A[n:,n:])
            # if not np.allclose(A[0:n, n:], -A[n:,0:n]):
            #     print(A[0:n, n:])
            #     print(A[n:,0:n])
            #     assert False
...
# """)
            ...
            betap, Ep = scipy.linalg.eig(Ap, overwrite_a=True)
            #betam, Em = scipy.linalg.eig(Am, overwrite_a=True)
            ...
            betap = np.sqrt(betap.real)
            #betam = np.sqrt(betam.real)
            ...
            # if True:
            #     idx = np.argsort(betap)[::-1]
            #     betap = betap[idx]
            #     Ep = Ep[:, idx]

            # idx = np.argsort(betam)[::-1]
            # betam = betam[idx]
            # Em = Em[:, idx]

            # in principle there is a - in this equation, but since we cset betap to be -betap in the exp, this minus must be removed here.
            Em = sumA @ Ep * np.reciprocal(betap)[np.newaxis, :]
        ...
        return np.hstack((betap, -betap)), np.hstack((Eu, Ed)), np.hstack((Ed, Eu))

    def solve2(self, m, compute_coherent_only):
        # solve the homogeneous equation for a single layer and return eigne value and eigen vector
        # :param m: mode
        # :param compute_coherent_only
        # :returns: beta, E, Q
