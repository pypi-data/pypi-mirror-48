...
from ..core.check_numba import nb
...
class IBA(object):
    ...
    **Usage Example:**
    ...
    def ft_even_phase(self, m, mu_s, mu_i, npol=None):
        ...
        if (not hasattr(self, "cached_mu")) or (not (np.array_equal(self.cached_mu[0], mu_s) and np.array_equal(self.cached_mu[1], mu_i))) \
            or (len(self.cached_phase) < m):
            self.precompute_ft_even_phase(mu_s, mu_i, max(m, self.m_max), npol)
        return self.cached_phase[m]


    # !!!!!!!!!!!!!!!!!!!!!!!!!!
    # The returned array is now with the order mu_s, mu_i, phi. It must be changed in the other EMMODEL !!!!!!!!!!!!!!!!!!!!!!!!!
    # !!!!!!!!!!!!!!!!!!!!!!!!!!
...
"""
        ...
        np.divide(cos_i1, sinT, where=nonnullsinT, out=cos_i1)  #cos_i1[nonnullsinT] /= sinT[nonnullsinT]
        np.clip(cos_i1, -1.0, 1.0, out=cos_i1)

        cos_i2 = costi_sint - cost_sinti * cos_pd
        np.divide(cos_i2, sinT, where=nonnullsinT, out=cos_i2)  #cos_i2[nonnullsinT] /= sinT[nonnullsinT]
        ...
        nullsin_t = sin_t.squeeze() < 1e-6
        nullsin_ti = sin_ti.squeeze() < 1e-6
        cos_i1[nullsin_t, nullsin_ti, :] = 1.
        cos_i2[nullsin_t, nullsin_ti, :] = cos_pd
        ...
        # This will only affect the sin2phi calculations: all others are cos and/or squared
    ...
    def precompute_ft_even_phase(self, mu_s, mu_i, m_max, npol):
        ...
        if any(u == 1 for u in mu_i) and npol > 2:
        ...
        self.cached_phase = [np.empty((pm_size[m] * len(mu_s), pm_size[m] * len(mu_i))) for m in range(m_max + 1)]
        self.cached_mu = mu_s, mu_i

        p = self.phase(mu_s, mu_i, dphi, npol)
        decomposed_p = np.fft.fft(p, axis=4)
...
# if nb:
#     #Lmatrix = nb.jit((nb.float64[:, :, :], nb.float64[:, :, :], nb.tuple(nb.int32, nb.int32)), 
#     #                    nopython=True, cache=True)(Lmatrix)
#     Lmatrix = nb.jit(nopython=True, cache=True)(Lmatrix)
