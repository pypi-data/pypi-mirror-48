...
# Depending on teh install method, this may or may not also be the parent or our 'share' directory.
# There's got to be a better way, but this should work:

if head_dir.endswith('-packages'):
    head_dir = os.path.normpath(head_dir + '/../../../')
...
if qt_module_name == 'PySide':
    ...
    SvgView = QtWebKit.QGraphicsWebView
    ...
    name_from_dialog = lambda x: x[0]
    SvgView = QtWebEngineWidgets.QWebEngineView
    ...
    name_from_dialog = lambda x:x
    SvgView = QtWebEngine.QWebEngineView
else:
