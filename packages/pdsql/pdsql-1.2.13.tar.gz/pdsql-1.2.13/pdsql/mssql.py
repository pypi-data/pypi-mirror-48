...
def rd_sql(server, database, table=None, col_names=None, where_in=None, where_op='AND', geo_col=False, from_date=None, to_date=None, date_col=None, rename_cols=None, stmt=None, con=None, schema=None):
    """
    ...
    if geo_col & (stmt is None):
        df = rd_sql_geo(server=server, database=database, table=table, col_stmt=col_stmt, where_lst=where_lst, schema=schema)
            ...
            with engine.begin() as conn:
                if where_temp:
                    for key, value in where_temp.items():
                        ...
                        df.to_sql(temp_tab, con=conn, if_exists='replace', index=False, chunksize=1000, schema=schema)
                df = pd.read_sql(stmt1, con=conn, schema=schema)
        else:
            if where_temp:
                for key, value in where_temp.items():
                    df = pd.DataFrame(data=value, columns=[key])
                    temp_tab = '#temp_'+key
                    df.to_sql(temp_tab, con=con, if_exists='replace', index=False, chunksize=1000, schema=schema)
            df = pd.read_sql(stmt1, con=con, schema=schema)
        if rename_cols is not None:
...
def rd_sql_ts(server, database, table, groupby_cols, date_col, values_cols, resample_code=None, period=1, fun='mean', val_round=3, where_in=None, where_op='AND', from_date=None, to_date=None, min_count=None, con=None, schema=None):
    """
    ...
    ## Create connection to database and execute sql statement
    df = pd.read_sql(sql_stmt1, con, schema=schema)
...
def del_table_rows(server, database, table=None, pk_df=None, stmt=None, schema=None):
    """
...
#    del_where_list = sql_where_stmts(**kwargs)
    if isinstance(stmt, str):
        ...
        pk = rd_sql(server, database, stmt=pk_stmt, schema=schema).name
...
#        del_rows_stmt = "DELETE FROM " + table + " WHERE " + " AND ".join(del_where_list)
    ...
    with engine.begin() as conn:
        if isinstance(pk_df, pd.DataFrame):
            pk_df.to_sql(name=temp_tab, con=conn, if_exists='replace', chunksize=1000, schema=schema)
        conn.execute(del_rows_stmt)


def update_table_rows(df, server, database, table, on=None, index=False, append=True, schema=None):
    """
    ...
    if on is None:
        pk_stmt = get_pk_stmt.format(db=database, table=table)
        pk = rd_sql(server, database, stmt=pk_stmt, schema=schema).name.tolist()
...
#    up_stmt = "update " + table + " set " + ", ".join(up_list) + " from " + table + " inner join " + temp_tab + " on " + " and ".join(on_list)
    ...
    with engine.begin() as conn:
        ...
        df.to_sql(temp_tab, con=conn, if_exists='replace', index=index, chunksize=1000, schema=schema)
...
def rd_sql_geo(server, database, table, col_stmt, where_lst=None, schema=None):
    """
...
def update_from_difference(df, server, database, table, on=None, index=False, append=True, mod_date_col=False, schema=None):
    """
    ...
    old1 = rd_sql(server, database, table, df1.columns.tolist(), where_in=where_dict1, schema=schema)
    ...
    if not both1.empty:
        ...
        update_table_rows(both1, server, database, table, on=on, append=append, schema=schema)
