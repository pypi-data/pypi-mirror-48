...
def x_within_tol(char_1, char_2, x_tolerance):
...
def extract_words(chars,
    ...
    for idx_a in range(num_chars):
        for idx_b in range(idx_a + 1, num_chars):
            if sorted_chars[idx_b]["doctop"] > sorted_chars[idx_a]["doctop"] + y_tolerance:
                break
            elif x_within_tol(sorted_chars[idx_a], sorted_chars[idx_b], x_tolerance):
                G.add_edge(idx_a, idx_b)
                
    word_sets = list(nx.connected_components(G))
    words = []
    for word in word_sets:
        chars = [sorted_chars[char_idx] for char_idx in word]
        chars = sorted(chars, key=lambda char: (char["doctop"], char["x0"]))

        new_word = objects_to_rect(chars)
        new_word["text"] = "".join(char["text"] for char in chars)
