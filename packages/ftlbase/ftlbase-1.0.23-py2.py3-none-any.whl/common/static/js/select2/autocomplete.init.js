...
;(function ($) {
    ...
    function initialize() {
        ...
        // var element = $(this).filter('not[id*="__prefix__"]');

        // element = $(this).get(0);

        if ((initialized.indexOf(element) >= 0) ||
            ($(this).parent().children("span").filter(".select2").length > 0) || // Se já tem span é porque já foi executada a inicialização
            ($(this).parent().filter(".sr-only").length > 0) ||
            ($(this).parents('div[style*="display: none"]').length > 0)) { // Se a div é invisível, então não inicializa
            return
        }
    ...
    $(document).bind('DOMNodeInserted', function(e) {
        $(e.target).find('[data-autocomplete-light-function]:not([id*="__prefix__"])').each(initialize);
    });
