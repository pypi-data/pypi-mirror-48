...
class SubSection(object):
    ...
    def start_token_idx(self):
        return self.section.tokens[0].index

    @property
    def end_token_idx(self):
        return self.section.tokens[-1].index
    ...
    def title(self):
        ...
        if self.sentences[0].char_end == newline_idx:
...
class Section(object):
    ...
    def _init_from_json(self, jstr):
        ...
        for i, (start, end, constituency) in enumerate(
            ...
            self.sentences.append(sentence)
        if 'subsections' in j:
            for start_sent_idx, end_sent_idx in zip(j['subsections']['start'],
                                                    j['subsections']['end']):
    ...
    def dumps(self):
        ...
        for subsection in self.subsections:
            subsection_starts.append(subsection.start_sentence_idx)
            subsection_ends.append(subsection.end_sentence_idx)
        j['subsections'] = dict()
    ...
    def get_subsections_containing_span(self, span: Span):
        results = []
        for subsection in self.subsections:
            if subsection.start_token_idx <= span.start \
                    and subsection.end_token_idx >= span.end:
