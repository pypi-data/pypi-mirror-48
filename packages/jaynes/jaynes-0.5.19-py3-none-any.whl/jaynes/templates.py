...
def ssh_remote_exec(user, ip_address, script_path, port=None, pem=None,
    ...
    if remote_script_dir:  # `upload` mode
        ...
        if profile:
            launch = f"""ssh {options} {user}@{ip_address} {port_} {pem_} 'sudo su - {profile}; {sudo_} bash {remote_path}' """
        else:
            launch = f"""ssh {options} {user}@{ip_address} {port_} {pem_} '{sudo_} bash {remote_path}'"""
        return send_file, launch
    else:  # `pipe` mode
        if profile:
            launch = f"""ssh {options} {user}@{ip_address} {port_} {pem_} 'sudo su - {profile}; {sudo_} bash -s' < {script_path}"""
        else:
