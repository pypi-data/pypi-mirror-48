...
class Codecs:
	...
	kUSRF = fourcharcode(_kae.keyASUserRecordFields)
	...
	def __init__(self):
		...
		self.decoders = {fourcharcode(k): v for k, v in {
				_kae.typeNull: self.unpacknull,
				_kae.typeBoolean: self.unpackboolean,
				_kae.typeFalse: self.unpackboolean,
				_kae.typeTrue: self.unpackboolean,
				_kae.typeSInt32: self.unpacksint32,
				_kae.typeIEEE64BitFloatingPoint: self.unpackfloat64,
				_kae.typeUTF8Text: self.unpackunicodetext,
				_kae.typeUTF16ExternalRepresentation: self.unpackunicodetext,
				_kae.typeUnicodeText: self.unpackunicodetext,
				_kae.typeLongDateTime: self.unpacklongdatetime,
				_kae.typeAEList: self.unpackaelist,
				_kae.typeAERecord: self.unpackaerecord,
				_kae.typeAlias: self.unpackfile,
				_kae.typeFSS: self.unpackfile,
				_kae.typeFSRef: self.unpackfile,
				_kae.typeFileURL: self.unpackfile,
				_kae.typeType: self.unpacktype,
				_kae.typeEnumeration: self.unpackenumeration,
		}.items()}
	...
	def unpack(self, desc):
		...
		rec = desc.coerceToDescriptorType_(fourcharcode(_kae.typeAERecord))
		if rec:
			rec = self.unpackaerecord(rec)
			rec[AEType(_kae.pClass)] = AEType(struct.pack('>I', desc.descriptorType()))
			return rec
		# return as-is
		return desc
	...
	def packfloat(self, val):
		return self._packbytes(_kae.typeFloat, struct.pack('d', val))
	
	def packbytes(self, val):
		return self._packbytes(_kae.typeData, val)
	...
	def packdatetime(self, val):
		...
		return self._packbytes(_kae.typeLongDateTime, struct.pack('q', sec))
	...
	def packdict(self, val):
		...
		for key, value in val.items():
			if isinstance(key, AEType):
				if key.code == _kae.pClass and isinstance(value, AEType): # AS packs records that contain a 'class' property by coercing the packed record to the descriptor type specified by the property's value (assuming it's an AEType)
					desctype = value
				else:
	...
	def unpackfile(self, desc):
		url = bytes(desc.coerceToDescriptorType_(fourcharcode(_kae.typeFileURL)).data()).decode('utf8')
