...
def _connect(user, instance, args):
    ...
    command = 'ssh-add {key_path}; ssh -A {tunnel} {host} -o ConnectTimeout={timeout}'.format(**config)
...
def _jump_connect(user, ssh_instance, jump_server, args):
    ...
    command = 'ssh-add {key_path}; ssh -A {tunnel} {host} -o ConnectTimeout={timeout} -t ssh {ssh_instance.private_ip_address}'.format(**command_tuple)
...
def create_parser():
    ...
    parser.add_argument('--region', help='AWS region (us-east-1 if no user default).', default='default')
