...
// DivideGradLeft
__global__ void divide_grad_left(float *b, float *grad, float *a_grad,
                                 ...
                                 int *broadcast_shape, int *shape_size_)
{
    ...
    a_grad[a_ind] = a_grad[a_ind] + (grad[tmp_idx] / b[b_ind]);
}

// DivideGradRight
__global__ void divide_grad_right(float *a, float *b, float *grad, float *b_grad,
                                  int *pad_shape_a, int *pad_shape_b, 
                                  int *broadcast_shape, int *shape_size_)
{
    int idx = blockIdx.x;
    ...
    // get tmp_idx, a_ind, b_ind
    b_grad[b_ind] = b_grad[b_ind] + (-grad[tmp_idx] * a[a_ind] / (b[b_ind] * b[b_ind]));
}
""")

divide_kernel = kernel.get_function('divide')
divide_grad_left_kernel = kernel.get_function('divide_grad_left')
divide_grad_right_kernel = kernel.get_function('divide_grad_right')


class DivideOp(OpKernel):
    ...
    @property
    def res_shape(self):
        return self._res_shape


    def divide(self, a, b, output):
        divide_kernel(a, b, output,
                      self._gpu_pad_shape_a, self._gpu_pad_shape_b, 
                      self._gpu_broadcast_shape, self._gpu_shape_size,
                      grid=(self.grid, 1, 1), block=(1, 1, 1))


    def divide_grad_left(self, b, grad, a_grad):
        divide_grad_left_kernel(b, grad, a_grad,
                                self._gpu_pad_shape_a, self._gpu_pad_shape_b, 
                                self._gpu_broadcast_shape, self._gpu_shape_size,
                                grid=(self.grid, 1, 1), block=(1, 1, 1))


    def divide_grad_right(self, a, b, grad, b_grad):
        divide_grad_right_kernel(a, b, grad, b_grad,
                                 self._gpu_pad_shape_a, self._gpu_pad_shape_b, 
                                 self._gpu_broadcast_shape, self._gpu_shape_size,
                                 grid=(self.grid, 1, 1), block=(1, 1, 1))
