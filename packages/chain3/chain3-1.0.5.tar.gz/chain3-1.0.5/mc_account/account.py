...
from mc_keys.exceptions import (
    ValidationError,
)
...
from mc_account.datastructures import (
    AttributeDict,
)
from mc_account.internal.signing import (
...
class Account(object):
    ...
    def create(self, extra_entropy=''):
        '''
        Creates a new private key, and returns it as a :class:`~mc_account.local.LocalAccount`.
        ...
        .. code-block:: python
            ...
            >>> from mc_account import Account
    ...
    def setKeyBackend(self, backend):
        '''
        Change the backend used by the underlying mc-keys library.

        *(The default is fine for most users)*

        :param backend: any backend that works in
            `mc_keys.KeyApi(backend) <https://github.com/ethereum/mc-keys/#keyapibackendnone>`_
    ...
    def signHash(self, message_hash, private_key):
        ...
        :meth:`w3.mc.sign() <chain3.mc.Eth.sign>`
        you can use :meth:`~mc_account.messages.defunct_hash_message`.
        ...
        :rtype: ~mc_account.datastructures.AttributeDict

        .. code-block:: python

            >>> msg = "I♥SF"
            >>> from mc_account.messages import defunct_hash_message
        ...
        (v, r, s, mc_signature_bytes) = sign_message_hash(key, msg_hash_bytes)
        return AttributeDict({
            'messageHash': msg_hash_bytes,
            'r': r,
            's': s,
            'v': v,
            'signature': HexBytes(mc_signature_bytes),
        })
    ...
    def signTransaction(self, transaction_dict, private_key):
        ...
        :meth:`w3.mc.sendRawTransaction() <chain3.mc.Eth.sendRawTransaction>`.

        Create the transaction dict for a contract method with
        `my_contract.functions.my_function().buildTransaction()
        <http://chain3py.readthedocs.io/en/latest/contracts.html#methods>`_
        ...
        .. code-block:: python
            ...
            {'hash': HexBytes('0x6893a6ee8df79b0f5d64a180cd1ef35d030f3e296a5361cf04d02ce720d32ec5'),
             ...
             'v': 37}
            >>> w3.mc.sendRawTransaction(signed.rawTransaction)
        '''
       ...
       # allow from field, *only* if it matches the private key
        ...
        # add for raw transaction
        if 'gas' in transaction_dict:
            sanitized_transaction = dissoc(sanitized_transaction, 'gas')
