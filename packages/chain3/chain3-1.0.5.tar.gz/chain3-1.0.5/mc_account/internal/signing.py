...
from mc_hash.auto import keccak as keccak_256

from hexbytes import (
    HexBytes,
)

from mc_account.internal.transactions import (
...
def sign_transaction_dict(mc_key, transaction_dict):
    #print('**************************',mc_key,'****************************', transaction_dict)
    # generate RLP-serializable transaction, with defaults filled
    unsigned_transaction = serializable_unsigned_transaction_from_dict(transaction_dict)
    '''
    print('************unsigned_transaction**************', list(unsigned_transaction))
    transaction_list = []
    transaction_list.append(unsigned_transaction.nonce)
    transaction_list.append(unsigned_transaction.systemContract)
    transaction_list.append(unsigned_transaction.gasPrice)
    transaction_list.append(unsigned_transaction.gasLimit)
    transaction_list.append(unsigned_transaction.to)
    transaction_list.append(unsigned_transaction.value)
    transaction_list.append(unsigned_transaction.data)
    transaction_list.append('')
    # transaction_list.append(to_hex(unsigned_transaction.shardingFlag))
    transaction_list.append(unsigned_transaction.via)
    transaction_list.append(unsigned_transaction.v)
    transaction_list.append('')
    transaction_list.append('')
    ...
    '''
    encode = rlp.encode(unsigned_transaction)
    #encode = rlp.encode(transaction_list)
    print('************encode**************', encode_hex(encode))
    keccakcode = keccak(encode)
    print('************keccak**************', encode_hex(keccakcode))
    transaction_hash = HexBytes(keccakcode)
    ...
    (v, r, s) = sign_transaction_hash(mc_key, transaction_hash, chain_id)
...
def to_mc_v(v_raw, chain_id=None):
    if chain_id is None:
        v = v_raw + V_OFFSET
    else:
...
def sign_transaction_hash(account, transaction_hash, chain_id):
    ...
    v = to_mc_v(v_raw, chain_id)
    return (v, r, s)


def _pad_to_mc_word(bytes_val):
...
def sign_message_hash(key, msg_hash):
    ...
    (v_raw, r, s) = signature.vrs
    v = to_mc_v(v_raw)
    mc_signature_bytes = to_bytes32(r) + to_bytes32(s) + to_bytes(v)
    return (v, r, s, mc_signature_bytes)
