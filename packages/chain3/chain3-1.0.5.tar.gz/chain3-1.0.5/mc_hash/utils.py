...
def get_backend_in_environment():
    return os.environ.get('MC_HASH_BACKEND', None)


def load_backend(backend_name):
    return importlib.import_module('mc_hash.backends.%s' % backend_name)
...
def load_environment_backend(env_backend):
    if env_backend in SUPPORTED_BACKENDS:
        try:
            ...
            raise ImportError(
                "The backend specified in MC_HASH_BACKEND, '{0}', is not installed. "
                "Install with `pip install mc-hash[{0}]`.".format(env_backend)
            )
    else:
        raise ValueError(
            "The backend specified in MC_HASH_BACKEND, %r, is not supported. "
...
def choose_available_backend():
    ...
    raise ImportError(
        ...
        "Install with `pip install mc-hash[%s]`." % (
            SUPPORTED_BACKENDS,
