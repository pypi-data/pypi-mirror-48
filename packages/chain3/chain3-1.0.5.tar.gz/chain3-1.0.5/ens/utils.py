...
def ensure_hex(data):
    if not isinstance(data, str):
        return Chain3().toHex(data)
    return data


def init_chain3(providers=default):
    from chain3 import Chain3

    if providers is default:
        w3 = Chain3(ens=None)
    else:
        w3 = Chain3(providers, ens=None)

    return customize_chain3(w3)


def customize_chain3(w3):
    from chain3.contract import ConciseContract
    from chain3.middleware import make_stalecheck_middleware
    ...
    w3.mc.setContractFactory(ConciseContract)
    return w3
...
def dot_mc_name(label):
    return label_to_name(label, 'mc', RECOGNIZED_TLDS)
...
def dot_mc_label(name):
    '''
    Convert from a name, like 'ethfinex.mc', to a label, like 'ethfinex'
    If name is already a label, this should be a noop, except for converting to a string
    and validating the name syntax.
    '''
    label = name_to_label(name, registrar='mc')
    if len(label) < MIN_MC_LABEL_LENGTH:
...
def name_to_hash(name):
    ...
    if name:
        ...
        for label in reversed(labels):
            ...
            node = Chain3().sha3(node + labelhash)
    return node


def dot_mc_namehash(name):
    '''
    ...
    This will add '.mc' to name if no TLD given. Also, it normalizes the name with
    `nameprep
    <https://github.com/ethereum/EIPs/blob/master/EIPS/eip-137.md#name-syntax>`_
    before hashing.
    ...
    :raises InvalidName: if ``name`` has invalid syntax
    '''
    expanded_name = dot_mc_name(name)
