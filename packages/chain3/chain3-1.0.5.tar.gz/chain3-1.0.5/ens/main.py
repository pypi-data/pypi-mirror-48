...
class ENS:
    ...
    namehash = staticmethod(dot_mc_namehash)
    ...
    def __init__(self, providers=default, addr=None):
        ...
        :type providers: instance of `chain3.providers.base.BaseProvider`
        ...
        '''
        self.chain3 = init_chain3(providers)

        ens_addr = addr if addr else ENS_MAINNET_ADDR
        self.ens = self.chain3.mc.contract(abi=abis.ENS, address=ens_addr)
        self._resolverContract = self.chain3.mc.contract(abi=abis.RESOLVER)

    @classmethod
    def fromChain3(cls, chain3, addr=None):
        '''
        Generate an ENS instance with chain3

        :param `chain3.Chain3` chain3: to infer connection information
        :param hex-string addr: the address of the ENS registry on-chain. If not provided,
            ENS.py will default to the mainnet ENS registry address.
        '''
        return cls(chain3.manager.providers, addr=addr)
    ...
    def address(self, name, guess_tld=True):
        ...
        :param bool guess_tld: should `name` be appended with '.mc' if no common TLD found?
        :raises InvalidName: if `name` has invalid syntax
        '''
        if guess_tld:
            expanded = dot_mc_name(name)
        else:
            expanded = name
    ...
    def setup_address(self, name, address=default, transact={}):
        ...
        Example: If the caller owns ``parentname.mc`` with no subdomains
        and calls this method with ``sub.parentname.mc``,
        ...
        :param dict transact: the transaction configuration, like in
            :meth:`~chain3.mc.Eth.sendTransaction`
        :raises InvalidName: if ``name`` has invalid syntax
        ...
        return resolver.setAddr(dot_mc_namehash(name), address, transact=transact)
    ...
    def setup_name(self, name, address=None, transact={}):
        ...
        :param dict transact: the transaction configuration, like in
            :meth:`~chain3.mc.sendTransaction`
    ...
    def owner(self, name):
        ...
        deed holder in the '.mc' registrar. Learn more about the difference
    ...
    def setup_owner(self, name, new_owner=default, transact={}):
        ...
        If the caller owns ``parentname.mc`` with no subdomains
        and calls this method with ``sub.parentname.mc``,
        then ``sub`` will be created as part of this call.
    ...
    def _assert_control(self, account, name, parent_owned=None):
        if not address_in(account, self.chain3.mc.accounts):
    ...
    def _set_resolver(self, name, resolver_addr=None, transact={}):
        if not resolver_addr:
            resolver_addr = self.address('resolver.mc')
        namehash = dot_mc_namehash(name)
    ...
    def _reverse_registrar(self):
        ...
        return self.chain3.mc.contract(address=addr, abi=abis.REVERSE_REGISTRAR)
