"""Interaction with smart contracts over chain3 connector.
...
from chain3.utils.contracts import (
    encode_abi,
...
from chain3.utils.datatypes import (
...
from chain3.utils.events import (
    get_event_data,
)
from chain3.utils.filters import (
    construct_event_filter_params,
)
from chain3.utils.function_identifiers import (
    FallbackFn,
)
from chain3.utils.normalizers import (
...
class ContractFunctions:
    ...
    def __init__(self, abi, chain3, address=None):
        if abi:
            self.abi = abi
...
class Contract:
    ...
    :meth:`chain3.mc.mc.contract` that takes compiled Solidity contract
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def __init__(self, address=None):
        ...
        """
        if self.chain3 is None:
            raise AttributeError(
                'The `Contract` class has not been initialized.  Please use the '
                '`chain3.contract` interface to create your contract class.'
            )

        if address:
            self.address = normalize_address(self.chain3.ens, address)

        if not self.address:
            raise TypeError("The address argument is required to instantiate a contract.")

        self.functions = ContractFunctions(self.abi, self.chain3, self.address)
        self.events = ContractEvents(self.abi, self.chain3, self.address)
        self.fallback = Contract.get_fallback_function(self.abi, self.chain3, self.address)

    @classmethod
    def factory(cls, chain3, class_name=None, **kwargs):

        kwargs['chain3'] = chain3

        normalizers = {
            'abi': normalize_abi,
            'address': partial(normalize_address, kwargs['chain3'].ens),
        ...
        setattr(contract, 'functions', ContractFunctions(contract.abi, contract.chain3))
        setattr(contract, 'events', ContractEvents(contract.abi, contract.chain3))
        setattr(contract, 'fallback', Contract.get_fallback_function(contract.abi, contract.chain3))
    ...
    def deploy(cls, transaction=None, args=None, kwargs=None):
        ...
        txn_hash = cls.chain3.mc.sendTransaction(deploy_transaction)
        return txn_hash
    ...
    def constructor(cls, *args, **kwargs):
        ...
        return ContractConstructor(cls.chain3,
                                   cls.abi,
                                   cls.bytecode,
                                   *args,
                                   **kwargs)
    ...
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        ...
        return encode_abi(cls.chain3, fn_abi, fn_arguments, data)
    ...
    def eventFilter(self, event_name, filter_params={}):
        ...
        log_filter = self.chain3.mc.filter(event_filter_params)
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.chain3.mc.defaultAccount is not empty:
            estimate_transaction.setdefault('from', self.chain3.mc.defaultAccount)
    ...
    def call(self, transaction=None):
        """
        Execute a contract function call using the `mc_call` interface.
        ...
        .. code-block:: python
            ...
            ContractFactory = w3.mc.contract(
        ...
        :param transaction: Dictionary of transaction info for chain3 interface
        ...
        if self.chain3.mc.defaultAccount is not empty:
            call_transaction.setdefault('from', self.chain3.mc.defaultAccount)
    ...
    def transact(self, transaction=None):
        """
        Execute a contract function call using the `mc_sendTransaction`
        interface.
        ...
        .. code-block:: python
            ...
            # Deposit to the `chain3.mc.coinbase` account.
            >>> wallet.functions.deposit().transact({'value': 12345})
            '0x5c504ed432cb51138bcf09aa5e8a410dd4a1e204ef84bfed1be16dfba1b22060'
            # Deposit to some other account using funds from `chain3.mc.coinbase`.
            >>> wallet.functions.deposit(chain3.mc.accounts[1]).transact({'value': 54321})
        ...
        if self.chain3.mc.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.chain3.mc.defaultAccount)
    ...
    def all_functions(self):
        return find_functions_by_identifier(
            self.abi, self.chain3, self.address, lambda _: True
        )
    ...
    def get_function_by_signature(self, signature):
        ...
        fns = find_functions_by_identifier(self.abi, self.chain3, self.address, callable_check)
    ...
    def find_functions_by_name(self, fn_name):
        ...
        return find_functions_by_identifier(
            self.abi, self.chain3, self.address, callable_check
        )
    ...
    @staticmethod
    def get_fallback_function(abi, chain3, address=None):
    ...
    def _encode_constructor_data(cls, args=None, kwargs=None):
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(
                encode_abi(cls.chain3, constructor_abi, arguments, data=cls.bytecode)
            )
        else:
...
class ContractConstructor:
    ...
    """
    def __init__(self, chain3, abi, bytecode, *args, **kwargs):
        self.chain3 = chain3
        self.abi = abi
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(
                encode_abi(self.chain3, constructor_abi, arguments, data=self.bytecode)
            )
        else:
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.chain3.mc.defaultAccount is not empty:
            estimate_gas_transaction.setdefault('from', self.chain3.mc.defaultAccount)

        estimate_gas_transaction['data'] = self.data_in_transaction

        return self.chain3.mc.estimateGas(estimate_gas_transaction)
    ...
    def transact(self, transaction=None):
        ...
        print('*************** defaultAccount **********************', self.chain3.mc.defaultAccount)
        if self.chain3.mc.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.chain3.mc.defaultAccount)

        transact_transaction['data'] = self.data_in_transaction
        # add for subchain
        transact_transaction['shardingFlag'] = 0
        transact_transaction['via'] = '0x0000000000000000000000000000000000000000'



        # TODO: handle asynchronous contract creation
        return self.chain3.mc.sendTransaction(transact_transaction)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if self.chain3.mc.defaultAccount is not empty:
            built_transaction.setdefault('from', self.chain3.mc.defaultAccount)

        built_transaction['data'] = self.data_in_transaction
        built_transaction['to'] = b''
        return fill_transaction_defaults(self.chain3, built_transaction)
...
class ConciseContract:
    ...
    > contract.withdraw(amount, transact={'from': mc.accounts[1], 'gas': 100000, ...})

    is equivalent to this call in the classic contract:

    > contract.functions.withdraw(amount).transact({'from': mc.accounts[1], 'gas': 100000, ...})
    '''
...
class ContractFunction:
    ...
    def call(self, transaction=None, block_identifier='latest'):
        ...
        block_id = parse_block_identifier(self.chain3, block_identifier)
    ...
    def _encode_transaction_data(cls):
        return add_0x_prefix(encode_abi(cls.chain3, cls.abi, cls.arguments, cls.selector))
...
def call_contract_function(
    ...
    if block_id is None:
        return_data = chain3.mc.call(call_transaction)
    else:
        return_data = chain3.mc.call(call_transaction, block_identifier=block_id)
    ...
    try:
        ...
        is_missing_code_error = (
            ...
            chain3.mc.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
        )
...
def parse_block_identifier(chain3, block_identifier):
    last_block = chain3.mc.getBlock('latest').number
    if isinstance(block_identifier, int) and abs(block_identifier) <= last_block:
        ...
        return chain3.mc.getBlock(block_identifier).number
    else:
...
def transact_with_contract_function(
    ...
    txn_hash = chain3.mc.sendTransaction(transact_transaction)
    return txn_hash
...
def estimate_gas_for_function(
    ...
    gas_estimate = chain3.mc.estimateGas(estimate_transaction)
    return gas_estimate
...
def build_transaction_for_function(
    ...
    prepared_transaction = fill_transaction_defaults(chain3, prepared_transaction)

    return prepared_transaction


def find_functions_by_identifier(contract_abi, chain3, address, callable_check):
