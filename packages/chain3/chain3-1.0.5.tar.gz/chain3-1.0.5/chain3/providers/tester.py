...
def ethtestrpc_string_middleware(make_request, chain3):
...
ethtestrpc_middleware = construct_formatting_middleware(
    request_formatters={
        'mc_uninstallFilter': apply_formatter_at_index(to_integer_if_hex, 0),
        'mc_getFilterChanges': apply_formatter_at_index(to_integer_if_hex, 0),
        'mc_getFilterLogs': apply_formatter_at_index(to_integer_if_hex, 0),
    },
    result_formatters={
        # mc
        'mc_newFilter': apply_formatter_if(is_integer, hex),
        'mc_protocolVersion': apply_formatter_if(is_integer, str),
        'mc_getTransactionByHash': apply_formatters_to_dict(TRANSACTION_FORMATTERS),
        # Net
...
ethtestrpc_exception_middleware = construct_exception_handler_middleware(
    method_handlers={
        'mc_getBlockByHash': (ValueError, static_result(None)),
        'mc_getBlockByNumber': (ValueError, static_result(None)),
    },
)


def ethereum_tester_personal_remapper_middleware(make_request, chain3):
    def middleware(method, params):
...
class EthereumTesterProvider(BaseProvider):
    ...
    @deprecated_for("chain3.providers.mc_tester.EthereumTesterProvider")
    def __init__(self,
                 *args,
                 **kwargs):
        if not is_testrpc_available():
            raise Exception("`TestRPCProvider` requires the `mc-testrpc` package to be installed")
