...
class BaseProvider:
    ...
    def request_func(self, chain3, outer_middlewares):
        '''
        ...
        if cache_key is None or cache_key != all_middlewares:
            self._request_func_cache = (
                all_middlewares,
                self._generate_request_func(chain3, all_middlewares)
            )
        return self._request_func_cache[-1]

    def _generate_request_func(self, chain3, middlewares):
...
class JSONBaseProvider(BaseProvider):
    ...
    def isConnected(self):
        try:
            response = self.make_request('chain3_clientVersion', [])
        except IOError:
            return False
        else:
