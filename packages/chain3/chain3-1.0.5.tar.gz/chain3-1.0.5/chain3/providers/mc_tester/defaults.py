...
from mc_tester.exceptions import (
    BlockNotFound,
    FilterNotFound,
    TransactionNotFound,
    ValidationError,
...
@curry
def call_mc_tester(fn_name, mc_tester, fn_args, fn_kwargs=None):
    if fn_kwargs is None:
        fn_kwargs = {}
    return getattr(mc_tester, fn_name)(*fn_args, **fn_kwargs)


def without_mc_tester(fn):
    # workaround for: https://github.com/pytoolz/cytoolz/issues/103
    # @functools.wraps(fn)
    def inner(mc_tester, params):
        return fn(params)
    return inner
...
@curry
def preprocess_params(mc_tester, params, preprocessor_fn):
    return mc_tester, preprocessor_fn(params)
...
def client_version(mc_tester, params):
    # TODO: account for the backend that is in use.
    from mc_tester import __version__
...
def get_transaction_by_block_hash_and_index(mc_tester, params):
    block_hash, transaction_index = params
    block = mc_tester.get_block_by_hash(block_hash, full_transactions=True)
...
@null_if_indexerror
@null_if_block_not_found
def get_transaction_by_block_number_and_index(mc_tester, params):
    block_number, transaction_index = params
    block = mc_tester.get_block_by_number(block_number, full_transactions=True)
    transaction = block['transactions'][transaction_index]
    return transaction


def create_log_filter(mc_tester, params):
    filter_params = params[0]
    filter_id = mc_tester.create_log_filter(**filter_params)
    return filter_id


def get_logs(mc_tester, params):
    filter_params = params[0]
    logs = mc_tester.get_logs(**filter_params)
    return logs
...
@without_params
def create_new_account(mc_tester):
    return mc_tester.add_account(_generate_random_private_key())


def personal_send_transaction(mc_tester, params):
    transaction, password = params

    try:
        mc_tester.unlock_account(transaction['from'], password)
        transaction_hash = mc_tester.send_transaction(transaction)
    finally:
        mc_tester.lock_account(transaction['from'])
...
API_ENDPOINTS = {
    'chain3': {
        ...
        'sha3': compose(
            ...
            keccak,
            decode_hex,
            without_mc_tester(operator.itemgetter(0)),
        ),
    },
    'net': {
    ...
    'mc': {
        ...
        'getCode': call_mc_tester('get_code'),
        'sign': not_implemented,
        'sendTransaction': call_mc_tester('send_transaction'),
        'sendRawTransaction': call_mc_tester('send_raw_transaction'),
        'call': call_mc_tester('call'),  # TODO: untested
        'estimateGas': call_mc_tester('estimate_gas'),  # TODO: untested
        'getBlockByHash': null_if_block_not_found(call_mc_tester('get_block_by_hash')),
        'getBlockByNumber': null_if_block_not_found(call_mc_tester('get_block_by_number')),
        'getTransactionByHash': null_if_transaction_not_found(
            call_mc_tester('get_transaction_by_hash')
        ),
        ...
        'getTransactionReceipt': null_if_transaction_not_found(compose(
            ...
            call_mc_tester('get_transaction_receipt'),
        )),
        ...
        'newBlockFilter': call_mc_tester('create_block_filter'),
        'newPendingTransactionFilter': call_mc_tester('create_pending_transaction_filter'),
        'uninstallFilter': excepts(
            FilterNotFound,
            compose(
                is_null,
                call_mc_tester('delete_filter'),
            ),
            static_return(False),
        ),
        'getFilterChanges': null_if_filter_not_found(call_mc_tester('get_only_filter_changes')),
        'getFilterLogs': null_if_filter_not_found(call_mc_tester('get_all_filter_logs')),
        'getLogs': get_logs,
    ...
    'personal': {
        ...
        'importRawKey': call_mc_tester('add_account'),
        'listAccounts': call_mc_tester('get_accounts'),
        'lockAccount': excepts(
            ValidationError,
            compose(static_return(True), call_mc_tester('lock_account')),
            static_return(False),
        ),
        'newAccount': create_new_account,
        'unlockAccount': excepts(
            ValidationError,
            compose(static_return(True), call_mc_tester('unlock_account')),
            static_return(False),
        ),
    ...
    'testing': {
        'timeTravel': call_mc_tester('time_travel'),
    },
    'txpool': {
    ...
    'evm': {
        'mine': call_mc_tester('mine_blocks'),
        'revert': call_mc_tester('revert_to_snapshot'),
        'snapshot': call_mc_tester('take_snapshot'),
    },
}
