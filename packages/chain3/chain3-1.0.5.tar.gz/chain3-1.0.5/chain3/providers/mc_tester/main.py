...
class EthereumTesterProvider(BaseProvider):
    ...
    def __init__(self, ethereum_tester=None, api_endpoints=None):
        if ethereum_tester is None:
            # do not import mc_tester until runtime, it is not a default dependency
            from mc_tester import EthereumTester
            self.ethereum_tester = EthereumTester()
        else:
            self.ethereum_tester = ethereum_tester

        if api_endpoints is None:
            # do not import mc_tester derivatives until runtime, it is not a default dependency
