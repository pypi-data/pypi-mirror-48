...
transaction_params_formatter = compose(
    # remove nonce for now due to issue https://github.com/ethereum/mc-tester/issues/80
...
ethereum_tester_middleware = construct_formatting_middleware(
    request_formatters={
        # mc
        'mc_getBlockByNumber': apply_formatters_to_args(
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
        ),
        'mc_getFilterChanges': apply_formatters_to_args(hex_to_integer),
        'mc_getFilterLogs': apply_formatters_to_args(hex_to_integer),
        'mc_getBlockTransactionCountByNumber': apply_formatters_to_args(
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
        ),
        'mc_getUncleCountByBlockNumber': apply_formatters_to_args(
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
        ),
        'mc_getTransactionByBlockHashAndIndex': apply_formatters_to_args(
            identity,
            to_integer_if_hex,
        ),
        'mc_getTransactionByBlockNumberAndIndex': apply_formatters_to_args(
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
            to_integer_if_hex,
        ),
        'mc_getUncleByBlockNumberAndIndex': apply_formatters_to_args(
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
            to_integer_if_hex,
        ),
        'mc_newFilter': apply_formatters_to_args(
            filter_params_transformer,
        ),
        'mc_getLogs': apply_formatters_to_args(
            filter_params_transformer,
        ),
        'mc_sendTransaction': apply_formatters_to_args(
            transaction_params_transformer,
        ),
        'mc_estimateGas': apply_formatters_to_args(
            transaction_params_transformer,
        ),
        'mc_call': apply_formatters_to_args(
            transaction_params_transformer,
            apply_formatter_if(is_not_named_block, to_integer_if_hex),
        ),
        'mc_uninstallFilter': apply_formatters_to_args(hex_to_integer),
        # EVM
    ...
    result_formatters={
        'mc_getBlockByHash': apply_formatter_if(
            is_dict,
            block_key_remapper,
        ),
        'mc_getBlockByNumber': apply_formatter_if(
            is_dict,
            block_key_remapper,
        ),
        'mc_getBlockTransactionCountByHash': apply_formatter_if(
            is_dict,
            transaction_key_remapper,
        ),
        'mc_getBlockTransactionCountByNumber': apply_formatter_if(
            is_dict,
            transaction_key_remapper,
        ),
        'mc_getTransactionByHash': apply_formatter_if(
            is_dict,
        ...
        'mc_newFilter': integer_to_hex,
        'mc_newBlockFilter': integer_to_hex,
        'mc_newPendingTransactionFilter': integer_to_hex,
        'mc_getLogs': apply_formatter_if(
            is_array_of_dicts,
            apply_formatter_to_array(log_key_remapper),
        ),
        'mc_getFilterChanges': apply_formatter_if(
            is_array_of_dicts,
            apply_formatter_to_array(log_key_remapper),
        ),
        'mc_getFilterLogs': apply_formatter_if(
            is_array_of_dicts,
            apply_formatter_to_array(log_key_remapper),
        ),
        # EVM
...
def guess_from(chain3, transaction):
    coinbase = chain3.mc.coinbase
    if coinbase is not None:
        return coinbase

    try:
        return chain3.mc.accounts[0]
...
def guess_gas(chain3, transaction):
    return chain3.mc.estimateGas(transaction) * 2


@curry
def fill_default(field, guess_func, chain3, transaction):
    if field in transaction and transaction[field] is not None:
        return transaction
    else:
        guess_val = guess_func(chain3, transaction)
        return assoc(transaction, field, guess_val)


def default_transaction_fields_middleware(make_request, chain3):
    fill_default_from = fill_default('from', guess_from, chain3)
    fill_default_gas = fill_default('gas', guess_gas, chain3)

    def middleware(method, params):
        # TODO send call to mc-tester without gas, and remove guess_gas entirely
        if method == 'mc_call':
