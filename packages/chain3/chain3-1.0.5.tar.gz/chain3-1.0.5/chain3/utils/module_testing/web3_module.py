...
from chain3.utils.ens import (
    ens_addresses,
)


class chain3ModuleTest:
    def test_chain3_clientVersion(self, chain3):
        client_version = chain3.version.node
        self._check_chain3_clientVersion(client_version)

    def _check_chain3_clientVersion(self, client_version):
    ...
    def test_soliditySha3(self, chain3, types, values, expected):
        if isinstance(expected, type) and issubclass(expected, Exception):
            with pytest.raises(expected):
                chain3.soliditySha3(types, values)
            return

        actual = chain3.soliditySha3(types, values)
    ...
    def test_soliditySha3_ens(self, chain3, types, values, expected):
        with ens_addresses(chain3, {
            'one.mc': "0x49EdDD3769c0712032808D86597B84ac5c2F5614",
            'two.mc': "0xA6b759bBbf4B59D24acf7E06e79f3a5D104fdCE5",
        }):
    ...
    def test_soliditySha3_same_number_of_types_and_values(self, chain3, types, values):
        with pytest.raises(ValueError):
            chain3.soliditySha3(types, values)

    def test_is_connected(self, chain3):
        assert chain3.isConnected()
