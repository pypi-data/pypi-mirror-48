...
class StaticENS:
    ...
    def address(self, name, guess_tld=True):
        # no automated chain3 usages should be guessing the TLD
...
def contract_ens_addresses(contract, name_addr_pairs):
    ...
    with contract_ens_addresses(mycontract, [('resolve-as-1s.mc', '0x111...111')]):
        # any contract call or transaction in here would only resolve the above ENS pair
    '''
    with ens_addresses(contract.chain3, name_addr_pairs):
        yield
