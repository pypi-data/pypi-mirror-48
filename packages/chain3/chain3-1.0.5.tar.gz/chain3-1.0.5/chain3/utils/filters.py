...
from chain3.utils.validation import (
    validate_address,
)
...
class Filter:
    ...
    def __init__(self, chain3, filter_id):
        self.chain3 = chain3
    ...
    def get_new_entries(self):
        log_entries = self._filter_valid_entries(self.chain3.mc.getFilterChanges(self.filter_id))
        return self._format_log_entries(log_entries)

    def get_all_entries(self):
        log_entries = self._filter_valid_entries(self.chain3.mc.getFilterLogs(self.filter_id))
        return self._format_log_entries(log_entries)
...
class ShhFilter(Filter):
    ...
    def get_new_entries(self):
        log_entries = self._filter_valid_entries(self.chain3.shh.getMessages(self.filter_id))
        return self._format_log_entries(log_entries)
