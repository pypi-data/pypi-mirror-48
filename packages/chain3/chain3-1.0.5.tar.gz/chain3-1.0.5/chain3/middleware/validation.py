...
from chain3.middleware.formatting import (
    construct_chain3_formatting_middleware,
)
...
@curry
def validate_chain_id(chain3, chain_id):
    if chain_id == chain3.net.chainId:
        return chain_id
    else:
...
def check_extradata_length(val):
    ...
    if len(result) > MAX_EXTRADATA_LENGTH:
        raise ValidationError(
            ...
            "Refer "
            "http://chain3py.readthedocs.io/en/stable/middleware.html#geth-style-proof-of-authority "
...
@curry
def chain_id_validator(chain3):
    return compose(
        apply_formatter_at_index(transaction_normalizer, 0),
        transaction_param_validator(chain3)
    )


def build_validators_with_chain3(w3):
    return dict(
        request_formatters={
            'mc_sendTransaction': chain_id_validator(w3),
            'mc_estimateGas': chain_id_validator(w3),
            'mc_call': chain_id_validator(w3),
        },
        result_formatters={
            'mc_getBlockByHash': block_validator,
            'mc_getBlockByNumber': block_validator,
        },
    )


validation_middleware = construct_chain3_formatting_middleware(build_validators_with_chain3)
