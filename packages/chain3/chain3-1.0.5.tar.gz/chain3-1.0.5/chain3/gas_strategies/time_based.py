...
def _get_avg_block_time(w3, sample_size):
    ...
    oldest = w3.mc.getBlock(latest['number'] - constrained_sample_size)
    return (latest['timestamp'] - oldest['timestamp']) / constrained_sample_size


def _get_raw_miner_data(w3, sample_size):
    latest = w3.mc.getBlock('latest', full_transactions=True)
    ...
    for _ in range(sample_size - 1):
        ...
        block = w3.mc.getBlock(block['parentHash'], full_transactions=True)
...
def construct_time_based_gas_price_strategy(max_wait_seconds,
    ...
    :param probability: An integer representation of the desired probability
        ...
        and 100 means 100%.
    """
    def time_based_gas_price_strategy(chain3, transaction_params):
        avg_block_time = _get_avg_block_time(chain3, sample_size=sample_size)
        wait_blocks = int(math.ceil(max_wait_seconds / avg_block_time))

        raw_miner_data = _get_raw_miner_data(chain3, sample_size=sample_size)
