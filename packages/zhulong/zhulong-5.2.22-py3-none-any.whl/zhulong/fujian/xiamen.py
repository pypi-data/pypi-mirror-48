...
def f3(driver, url):
    ...
    if "/xmebid/governBid.do?" in url:
        ...
        locator = (By.XPATH, "//div[@id='main'][string-length()>100]")
...
def get_iframe_data(driver, ):
    ...
    try:
        ...
        driver.find_element_by_xpath("//div[contains(@class, 'Section')][string-length()>50]")
        before = len(driver.page_source)
        time.sleep(1)
        after = len(driver.page_source)
        i = 0
        ...
        page = driver.page_source
        soup = BeautifulSoup(page, 'html.parser')
        div = soup('div', class_=re.compile('Section'))
        return  div
    ...
    try:
        locator = (
        By.XPATH, "//embed[@id='plugin'] | (//div[@class='textLayer'])[{}][string-length()>100]".format(tnum))
        WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
    except:
        locator = (
        By.XPATH, "//embed[@id='plugin'] | (//div[@class='textLayer'])[{}][string-length()>100]".format(tnum - 1))
        WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
    before = len(driver.page_source)
    time.sleep(0.5)
    after = len(driver.page_source)
