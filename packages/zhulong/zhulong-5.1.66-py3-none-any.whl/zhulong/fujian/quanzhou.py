...
def f1(driver, num):
    ...
    if ("/project/projectList.do?" in url) or ("/project/otherBidInfo.do?" in url):
        ...
        for tr in trs[1:]:
            ...
            else:link = "http://www.qzzb.gov.cn" + href + "&leftIndex=1"
        ...
        for tr in trs:
            ...
            if 'http' in href:
                ...
                link = "http://www.qzzb.gov.cn" + href
...
def f3(driver, url):
    ...
    try:
        ...
        time.sleep(1)
    if ("Internal Server Error" in driver.page_source) or ('404' in driver.title):
        return 404
    ...
    if 'iframepage' in driver.page_source:
        ...
        locator = (By.XPATH, "//div[@class='warp'][string-length()>30]")
        WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(locator))
    time.sleep(1)
    before = len(driver.page_source)
    time.sleep(0.1)
...
data = [
    ...
    ["gcjs_gqita_bian_bu_gg",
     "http://www.qzzb.gov.cn/project/anQuestionList.do?centerId=-1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ...
    ["qsy_gqita_bian_bu_gg",
     "http://www.qzzb.gov.cn/project/otherBidAnQuestion.do?centerId=-1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
...
if __name__=='__main__':
    work(conp=["postgres","since2015","192.168.3.171","fujian","quanzhou"],pageloadtimeout=60)


    # driver=webdriver.Chrome()
    # url = "http://www.qzzb.gov.cn/project/otherBidAnQuestion.do?centerId=-1"
    # driver.get(url)
    # df = f2(driver)
    # print(df)
    # driver=webdriver.Chrome()
