...
def f3(driver, url):
    if "/zhongbiaogg" in url:
        ...
        time.sleep(2)
        try:
            locator = (By.XPATH, "//div[@class='wrap'][string-length()>100] | //div[@class='app'][string-length()>100]")
            WebDriverWait(driver, 5).until(EC.presence_of_all_elements_located(locator))
            flag = 1
        except:
            try:
                locator = (By.XPATH, "//div[@class='bidderPublic'][string-length()>100]")
    ...
    time.sleep(2)
    try:
        locator = (By.XPATH, "//div[@class='wrap'][string-length()>100]")
        WebDriverWait(driver, 5).until(EC.presence_of_all_elements_located(locator))
        flag = 1
    except:
        try:
            locator = (By.XPATH, "//div[@class='view'][string-length()>100]")
            WebDriverWait(driver, 1).until(EC.presence_of_all_elements_located(locator))
            flag = 2
        except:
            locator = (By.XPATH, "//embed[@id='plugin']")
...
if __name__=='__main__':
    ...
    #         print(d)
    # driver = webdriver.Chrome()
    # df = f3(driver, 'http://qtgcztb.4008705191.cn/views/ebid/combine/v1/entp-view.html?type=tp&tpId=5cba8c6eb18dcc0fb0e4c491&flag=true/zhongbiaogg')
    # print(df)
