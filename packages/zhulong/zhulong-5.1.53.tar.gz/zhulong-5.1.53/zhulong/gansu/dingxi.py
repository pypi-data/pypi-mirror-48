...
def f1(driver, num):
    ...
    for li in lis:
        ...
        url=re.findall('javascript:redirectpage\("(.+?)","(.+?)"\)',href_js)[0][0]
        data_cate=re.findall('javascript:redirectpage\("(.+?)","(.+?)"\)',href_js)[0][1]

        if len(data_cate) == 12:
            href = '/'.join(['http://ggzy.dingxi.gov.cn/jyxx', data_cate[:6],data_cate[:9],data_cate, ggstart_time.replace('-', ''),
                             url + '.html'])
        else:
            href = '/'.join(['http://ggzy.dingxi.gov.cn/jyxx', data_cate[:6], data_cate,
                             ggstart_time.replace('-', ''),
                             url + '.html'])
        if '004005' in driver.current_url:
            href=href_js
...
def f3(driver, url):
    ...
    if 'javascript' in url:
        driver.get('http://ggzy.dingxi.gov.cn/jyxx/project.html?categoryNum=004005')
        ...
        locator = (
        By.XPATH, '//div[@id="content"][string-length()>100]')
...
if __name__=="__main__":
    work(conp=["postgres","since2015",'192.168.3.171',"gansu","dingxi"],num=2,headless=True,total=2)
    # driver=webdriver.Chrome()
