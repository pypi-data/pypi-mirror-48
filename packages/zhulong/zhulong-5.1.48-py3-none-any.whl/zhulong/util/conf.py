...
zhulong_diqu_dict = {
    ...
    'shanxi': ['chenzhou', 'shanxi', 'weinan', 'xian', 'xianyang', 'yanan', 'public', 'ankang', 'xianyang2'],
...
def create_all_schemas():
    ...
    if not res:
        cursor.execute("create database if not exists %s" % conp[3])

    for w in zhulong_diqu_dict.keys():
        conp[3] = w
        conn = psycopg2.connect(database=conp[0], host=conp[2], user=conp[0], password=conp[1])
        conn.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
        cursor = conn.cursor()
        cursor.execute("SELECT 1 FROM pg_database WHERE datname='%s'" % conp[3])
        res = bool(cursor.fetchone()[0])
        if not res:
            print('database %s not exists' % w)
            cursor.execute("create database if not exists %s" % conp[3])
