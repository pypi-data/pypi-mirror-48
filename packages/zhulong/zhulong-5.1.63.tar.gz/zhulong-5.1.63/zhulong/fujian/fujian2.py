...
from zhulong.util.etl import add_info,est_meta,est_html,est_tbs,add_info
import math
    ...
    for tr in trs:
        ...
        if ss == 'dayi':
            ...
            if 'http' in href:
                ...
                link = "https://www.fjggfw.gov.cn/Website/FJBID_DATA/" + href.strip()
            try:
                td = tr.find_all("span", class_='span_o')[0].span.text.strip()
            except:
                td = '-'
        tmp = [title, td, link]
        data.append(tmp)
...
def f1(driver, num):
    ...
    if proxies:
        res = requests.post(url=start_url, headers=headers, data=data, proxies=proxies,timeout=60)
    else:
        res = requests.post(url=start_url, headers=headers, data=data,timeout=60)
    # 需要判断是否为登录后的页面
...
def f3(driver, url):
    driver.get(url)
    locator = (By.XPATH, "//div[@class='gc_body'][string-length()>30]")
...
if __name__=='__main__':
    ...
    # url="https://www.fjggfw.gov.cn/Website/FJBID_DATA/FJBID_DATA_LIST.aspx/dayi"
