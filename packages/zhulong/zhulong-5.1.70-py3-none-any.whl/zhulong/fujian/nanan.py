            ...
            if "getTenderInfoPage.do" in url:
                ...
                for data in datalist:
                    ...
                    try:
                        ...
                        signUpType = ""
                    link = "http://120.33.41.196/hyweb/naebid/bidDetails.do?flag=" + "{}".format(noticeType) +"&tenderProjCode=" + data['tenderProjCode'] + "&tenderProjId=" +\
                ...
                if int(noticeType) != 6:
                    ...
                    for data in datalist:
                        ...
                        try:
                            ...
                            link = "http://120.33.41.196/hyweb/commons/mongoGovBid.do?projId=" + data['projId'] + "&flag=2&flag=" + "{}".format(noticeType)
                        tmp = [projName, publishTime, link]
                        data_list.append(tmp)
                else:
                    html = json.loads(html)
...
def f3(driver, url):
    ...
    if "naebid/otherBid.do?srcNoticeId=" in url:
        ...
        if 'flag' in url:
            locator = (By.XPATH, "//div[@id='main'][string-length()>30]")
            WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
            flag = re.findall(r'flag=(\d+?)', url)[0]
            fl1 = OrderedDict([('1', '招标公告'), ('2', '变更公告'), ('3', '中标候选人公示'),
                               ('4', '中标公示'), ('8', '资格预审'), ('7', '异常公告')])
            if int(flag) == 8:
                flag='1'
            locator = (By.XPATH, "//ul[@id='LoutiNav']/li[contains(@code-value, '%s')]" % flag)
        ...
        fl2 = OrderedDict([('1', '采购/资审公告'), ('6', '预采公告'), ('2', '中标公告'),
                           ('4', '更正事项'), ('7', '招标终止公告')])
        flag = re.findall(r'flag=(\d+?)', url)[-1]
        # print(flag)
        # print(fl2[flag])

        locator = (By.XPATH, "//ul[@id='LoutiNav']/li[contains(@value, '%s')]" % flag)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator)).click()
        locator = (By.XPATH, "//div[@id='main'][string-length()>15]")
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
