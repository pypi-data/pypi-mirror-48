...
def f1(driver, num):
    ...
    val = WebDriverWait(driver, 5).until(EC.presence_of_element_located(locator)).get_attribute('href').rsplit('/',maxsplit=1)[1]
...
def f2(driver):
    ...
    for t in range(1, 1000):
        try:
            ...
            locator = (By.XPATH, "//div[@class='news-list list-ul']/ul/li[1]/a")
            va = WebDriverWait(driver, 5).until(EC.presence_of_element_located(locator)).text.strip()
            if '暂无信息' in va:
                num = t-1
                break
    total=num-1
