...
class AWSStatusMaintainer:
    ...
    def is_service(self, service_name: str) -> bool:
        return service_name in self.table_data["services"]

    def is_application(self, app_name: str) -> bool:
        app_details = self._get_first_application(app_name)
        return True if app_details else False

    def get_service_deployment_outputs(self, service_name: str) -> {}:
        if service_name in self.table_data["services"]:
            return self.table_data["services"][service_name].get("outputs", {})
