...
from infrascloudy.types import Rendering
from infrascloudy.utils.yaml import dumper

OBJECT_SEPARATOR = "\n---\n"


def _camel_to_dict(self):
    ...
    for attr, camel_attr in self.attribute_map.items():
        ...
        if isinstance(value, list):
            result[camel_attr] = list(
                map(
                    lambda x: _camel_to_dict(x) if hasattr(x, "to_dict") else x,
                    value,
                )
            )
        elif hasattr(value, "to_dict"):
            result[camel_attr] = _camel_to_dict(value)
...
def dump(rendering: Rendering) -> str:
    ...
    for template_path, k8s_objects in rendering.items():
        ...
        for k8s_object in k8s_objects:
            ...
            k8s_object.to_dict = MethodType(_camel_to_dict, k8s_object)
