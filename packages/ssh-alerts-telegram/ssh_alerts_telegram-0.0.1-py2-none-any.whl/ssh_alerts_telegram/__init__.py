...
def add_to_source():
    if not (os.geteuid()==0):
        ...
        #Conditions
        while True:
            s = "ssh_alerts_telegram"
            if s in text: #string in present in the text file
                break
            else: #string is absent in the text file
                os.system('echo "python -m ssh_alerts_telegram 2>> /dev/null" >> /etc/profile')
                break
        fobj.close()
        ...
        if (os.path.exists(os.environ['HOME'] + "/.bash_profile")):
            ...
            while True:
                ...
                if s in text: #string in present in the text file
                    ...
                    os.system('echo "python -m ssh_alerts_telegram 2>> /dev/null" >> $HOME/.bash_profile')
                    break
            fobj.close()
