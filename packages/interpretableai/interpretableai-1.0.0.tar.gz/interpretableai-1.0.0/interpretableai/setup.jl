...
if isdefined(Main, :IAISysImg)
  ...
  if iai_version == nothing
    error("IAI is not present in your Julia installation. Please follow the " *
          "instructions at " *
          "https://docs.interpretable.ai/IAI-Python/stable/installation")
  end
...
const REQUIRED_IAI_VERSION = v"0.1.0"
if Base.thispatch(iai_version) < Base.thispatch(REQUIRED_IAI_VERSION)
  error("This version of the `iai` Python package requires IAI version " *
        "$REQUIRED_IAI_VERSION. Version $iai_version of IAI modules is " *
        "installed. Please upgrade your IAI installation or downgrade to an " *
        "older version of the Python `iai` package.")
end
...
@eval IAI begin
  function to_html(roc::ROCCurve)
    io = IOBuffer()
    show(io, MIME("text/html"), roc)
    String(take!(io))
  end

  function to_html(lnr::OptimalTrees.OptimalTreeLearner)
    if IAIBase.hasmodel(lnr)
      io = IOBuffer()
      show(io, MIME("text/html"), lnr)
      String(take!(io))
    else
      return nothing
    end
