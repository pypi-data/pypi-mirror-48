...
from zhulong2.util.etl import est_meta, est_html, add_info,est_meta_large
...
def f3(driver, url):
    driver.get(url)
    if "请输入正确的用户名和密码！" in driver.page_source:
        raise ValueError('本网页需要注册，才可以爬取')
...
if __name__ == '__main__':
    ...
    #
    #     print(f2(driver))
    # work(conp=["postgres", "since2015", "192.168.3.171", "zlest", "sichuansheng"])
    driver = webdriver.Chrome()
