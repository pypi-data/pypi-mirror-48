...
class ShuffleNetV26Stages(torch.nn.Module):
    def __init__(self, stages_repeats, stages_out_channels, num_classes=1000):
        super(ShuffleNetV26Stages, self).__init__()

        if len(stages_repeats) != 4:
            raise ValueError('expected stages_repeats as list of 4 positive ints')
        if len(stages_out_channels) != 6:
            raise ValueError('expected stages_out_channels as list of 6 positive ints')
        ...
        for name, repeats, output_channels in zip(
            ...
            seq = [torchvision.models.shufflenetv2.InvertedResidual(
                input_channels, output_channels, 2)]
            for _ in range(repeats - 1):
                seq.append(torchvision.models.shufflenetv2.InvertedResidual(
        ...
        self.fc = torch.nn.Linear(output_channels, num_classes)
...
class ShuffleNetV26StagesFactory(object):
    def __init__(self, torchvision_shufflenetv2):
        self.torchvision_shufflenetv2 = torchvision_shufflenetv2
    ...
    def blocks(self):
        return [
            ...
            self.torchvision_shufflenetv2.stage3,
            self.torchvision_shufflenetv2.stage4,
            self.torchvision_shufflenetv2.stage5,
            self.torchvision_shufflenetv2.conv6,
        ]
