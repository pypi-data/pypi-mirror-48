...
def factory_from_scratch(basename, headnames, *, pretrained=True):
    ...
    if basename == 'shufflenetv2x2w':
        base_vision = basenetworks.ShuffleNetV26Stages([4, 8, 4, 4], [24, 122, 244, 488, 976, 2048])
        return shufflenet6stages_factory_from_scratch(basename, base_vision, 2048, headnames)
...
def shufflenet6stages_factory_from_scratch(basename, base_vision, out_features, headnames):
    blocks = basenetworks.ShuffleNetV26StagesFactory(base_vision).blocks()
    basenet = basenetworks.BaseNetwork(
        torch.nn.Sequential(*blocks),
        basename,
