...
class RequestSender:
    ...
    config.read('secrets')
    api_url = 'https://{backlog_host}/api/v2/'.format(backlog_host=config['backlog']['Host'])
    api_key = config['backlog']['ApiKey']
    ...
    def get_file(self, path: str, url_param) -> Tuple[str, Response]:
        ...
        if not response.ok:
            raise AssertionError('Unexpected response :{exception_body}'.format(exception_body=response.content))
