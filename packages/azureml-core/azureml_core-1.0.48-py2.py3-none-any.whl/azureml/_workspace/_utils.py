...
def arm_deploy_template_new_resources(auth, resource_group_name,
    ...
    if show_output:
        ...
        if lro_poller._exception is not None:
            ...
            print(error_msg)
            if 'There is an existing workspace provisioning operation ongoing for the same workspace' in error_msg:
                delete_ws = False
                return vault_name, storage_name, insights_name, delete_ws
            else:
                return vault_name, storage_name, insights_name, delete_ws
