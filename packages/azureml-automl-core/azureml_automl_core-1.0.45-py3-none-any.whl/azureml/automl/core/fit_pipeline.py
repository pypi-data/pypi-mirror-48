...
from . import data_transformation, package_utilities, pipeline_run_helper, training_utilities
...
def _check_iteration_time(automl_settings: AutoMLBaseSettings,
    ...
    if iteration_timeout_min is not None:
        iteration_timeout_min = int(iteration_timeout_min - running_min)
    ...
    if iteration_timeout_min and iteration_timeout_min <= 0 and raise_exception:
        if automl_settings.experiment_timeout_minutes is not None:
            ...
            raise TimeoutException('Timeout reached after running itertaion for {} minutes, skipping iteration. '
