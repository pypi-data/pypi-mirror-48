...
class AbstractTimeSeriesTransformer(AutoMLTransformer):
    ...
    def construct_tsdf(self,
        ...
        try:
            ...
            raise DataException("There is more than one value for a "
                                "time_index and grain combination.")
        except KeyError:
            # This exception will happen before getting to creation of TimeSeriesDataFrame and
            # it will be KeyError instead of DataFrameMissingColumnException.
            raise DataException("The time column was not found in the dataframe.")
        except DataFrameMissingColumnException:
            raise DataException("The grain column was not found in the dataframe.")
        except NotSupportedException:
            raise DataException("Time and grain column names must be a string.")
        except Exception:
            raise DataException("Internal error formatting time-series data. "
