...
class NimbusMlWrapperBase(object):
    """
    Nimbusml base class.
    ...
    def get_classes(self, y: np.ndarray) -> np.ndarray:
        """
        Get the classes in the right order.

        :param y: parameter dictionary.
        :type args: np.ndarray
        :param y: Input target values.
        :return: Classes array.
        """
        classes = np.unique(y)
        class_indexes = list(range(0, len(classes)))
        class_indexes.sort(key=lambda a: str(a))
        return np.array(list(classes[i] for i in class_indexes))


class AveragedPerceptronClassifier(ClassifierMixin, _AbstractModelWrapper, NimbusMlWrapperBase):
    """
    Nimbusml Averaged Perceptron Classifier class.
    ...
    def get_model(self):
        """
        Return Nimbusml Averaged Perceptron model.
    ...
    def fit(self,
            ...
            **kwargs: Any) -> Any:
        """
        Fit function for Nimbusml Averaged Perceptron Classifier model.
        ...
        if len(classes) > 2:
            ...
            self.classes_ = self.get_classes(y)
            nml_model = nml_multiclass.OneVsRestClassifier(nml_model)

        nml_model = nml.Pipeline([nml_model], random_state=random_state)
    ...
    def predict(self,
        ...
        if self.model is None:
            raise sklearn.exceptions.NotFittedError()
        return self.model.predict(X)["PredictedLabel"].values
...
class FastLinearClassifier(ClassifierMixin, _AbstractModelWrapper, NimbusMlWrapperBase):
    """
    ...
    def get_model(self):
        """
        Return Nimbusml Fast Linear Classifier model.
    ...
    def fit(self,
            ...
            **kwargs: Any) -> Any:
        """
        Fit function for Nimbusml Fast Linear Classifier model.
        ...
        nml_model = nml.Pipeline([nml_linear.FastLinearClassifier(**args)],
    ...
    def get_params(self, deep=True):
        """
        Return parameters for Nimbusml Fast Linear Classifier model.

        :param deep: If True, will return the parameters for this estimator and contained subobjects that are
            estimators.
        :type deep: boolean
        :return: Parameters for the Nimbusml Fast Linear Classifier model.
        """
        if self.model:
            return self.model.get_params(deep)
        else:
    ...
    def predict(self,
                X: np.ndarray) -> np.ndarray:
        """
        Prediction function for Nimbusml Fast Linear Classifier model.

        :param X: Input data.
        :type X: numpy.ndarray
        :return: Prediction values from Nimbusml Fast Linear Classifier model.
        """
        if self.model is None:
            raise sklearn.exceptions.NotFittedError()
        return self.model.predict(X)["PredictedLabel"].values
    ...
    def predict_proba(self,
                      X: np.ndarray) -> np.ndarray:
        """
        Prediction class probabilities for X for Nimbusml Fast Linear Classifier model.

        :param X: Input data.
        :type X: numpy.ndarray
        :return: Prediction probability values from Fast Linear Classifier model.
        """
        if self.model is None:
            raise sklearn.exceptions.NotFittedError()
        return self.model.predict_proba(X)
...
class FastLinearRegressor(RegressorMixin, _AbstractModelWrapper, NimbusMlWrapperBase):
    """
    ...
    def fit(self,
            ...
            **kwargs: Any) -> Any:
        """
        Fit function for Nimbusml Fast Linear Regressor model.
        ...
        nml_model = nml.Pipeline([nml_linear.FastLinearRegressor(**args)],
                                 random_state=random_state)
        nml_model.fit(X, y, **kwargs)
        self.model = nml_model
    ...
    def predict(self,
                X: np.ndarray) -> np.ndarray:
        """
        Prediction function for Nimbusml Fast Linear Regressor model.

        :param X: Input data.
        :type X: numpy.ndarray
        :return: Prediction values from Nimbusml Fast Linear Regressor model.
        """
        if self.model is None:
            raise sklearn.exceptions.NotFittedError()
        return self.model.predict(X)["Score"].values
...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    FATAL_MAX_HORIZON_EXCEEDED = ("The prediction horizon exceeded for one of the grains. "
                                  "Please reduce the forecast_destination or shorten the X_pred.")
    ...
    def _check_max_horizon_and_grain(self, grain_tuple: Tuple[Any],
        ...
        if last_train is None:
            raise DataException(ForecastingPipelineWrapper.FATAL_NO_GRAIN_IN_TRAIN)
        if self._lag_or_rw_enabled():
            ...
            if horizon > self._ts_transformer.max_horizon:
                raise DataException(ForecastingPipelineWrapper.FATAL_MAX_HORIZON_EXCEEDED)
