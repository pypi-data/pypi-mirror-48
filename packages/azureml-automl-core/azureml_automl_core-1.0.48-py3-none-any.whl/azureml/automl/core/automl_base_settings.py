...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        if is_timeseries:
            ...
            if target_lags is not None:
                if target_lags < 1 or target_lags > AutoMLBaseSettings.MAX_LAG_LENGTH:
    ...
    def _verify_settings(self):
        ...
        if self.max_concurrent_iterations is not None and \
                (self.max_concurrent_iterations < 1 or self.max_concurrent_iterations > self.iterations):
            raise ValueError('Input parameter \"max_concurrent_iterations\" '
                             'needs to be between 1 and the number of iterations inclusive if set.')
