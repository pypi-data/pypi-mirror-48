...
class AWSDeployer(object):
    ...
    def deploy_package(self, template_file_path, aws_region):
        ...
        cmd = ["sam", "deploy", "--template-file", template_file_path, "--stack-name", f"testing{self.deploy_config['APPNAME_SINGLE']}", "--capabilities", "CAPABILITY_IAM", "--region", aws_region, "--parameter-overrides", *env_vars]
    ...
    def build_and_deploy(self, deploy=False):
        ...
        if deploy:
            if deploy == "prod":
                ...
                SAM_S3_BUCKET = "appdevstore-us-east-1"
                AWS_REGION = "us-east-1"
            upload_code_in_S3(zip_file_path, SAM_S3_BUCKET, AWS_REGION)
            self.deploy_package(template_file_path, AWS_REGION)
...
if __name__ == "__main__":
    ...
    AWSDeployer(configpath).build_and_deploy()
