            ...
            if len(data) == 0:
                try:
                    ...
                    if 'CONTENT-LENGTH' in self.headers:
                        warnings.warn(ValueError('Invalid HTTP Content-Length value: {content_length}'.format(content_length=repr(self.headers["CONTENT-LENGTH"]))))
                        self.reset()
                        return
...
class DHTTPClient(object):
    ...
    async def request(self, path = '/', method = 'GET', headers = (), data = b'', tls = False, cert_mode = tcpserver.CertMode.CM_GENERATED, fn_component = '', cert_file = None, key_file = None, key_size = 2048, handler = None):
        if fn_component:
            if key_file is None:
