...
print('Asia/Shanghai date: %s' % _TODAY)
...
class AppStoreProduct:
    ...
    def is_price_plan_equal(self, next_plan):
        ...
        # 找到起始区间
        ii = 0
        while ii < len(now_plan) and now_plan[ii].get('end_date') and now_plan[ii]['end_date'] <= _TODAY:
            ii += 1
        if ii >= len(now_plan):
            raise ValueError('当前价格区间不合法')
        jj = 0
        while jj < len(next_plan) and next_plan[jj].get('endDate') and next_plan[jj]['endDate'] <= _TODAY:
            jj += 1
        if jj >= len(next_plan):
            raise ValueError('更新价格区间不合法')

        if now_plan[ii]['tierId'] != next_plan[jj]['priceTier']['tier']['id']:
            print('tier different: %s, %s' % (now_plan[ii]['tierId'], next_plan[jj]['priceTier']['tier']['id']))
            return False
        if now_plan[ii].get('end_date', -1) != next_plan[jj].get('endDate', -1):
            print('end_date different: %s, %s' % (now_plan[ii].get('end_date'), next_plan[jj].get('endDate')))
            return False
        ii += 1
        jj += 1
        if len(now_plan) - ii != len(next_plan) - jj:
            print('interval count different')
            return False
        # FIXME: 更智能的比较
        while ii < len(now_plan):
            ...
            if now_item['start_date'] != next_item['startDate']:
                print('start_date different: %s, %s' % (now_item['start_date'], next_item['startDate']))
                return False
            if now_item.get('end_date', -1) != next_item.get('endDate', -1):
                print('end_date different: %s, %s' % (now_item.get('end_date'), next_item.get('endDate')))
                return False
            if now_item['tierId'] != next_item['priceTier']['tier']['id']:
                print('tier different: %s, %s' % (now_item['tierId'], next_item['priceTier']['tier']['id']))
                return False
            ii += 1
            jj += 1
        return True
