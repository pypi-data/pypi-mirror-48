...
输出参数:
    ...
    FitnV = (np.max(CombinObjV) - CombinObjV + 0.5) / (np.max(CombinObjV) - np.min(CombinObjV) + 0.5) # 计算种群适应度
    
    # 更新NDSet
    if NDSet is None:
        return FitnV, population[np.where(levels == 1)[0]]
    else:
        ...
        FitnV[np.where(punishFlag == 0)[0]] *= 0.5
