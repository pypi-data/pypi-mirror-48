...
class soea_EGA_templet(ea.Algorithm):
    
    """
soea_EGA_templet.py - Elitist Selection GA templet(精英选择的遗传算法模板)

算法描述:
    本模板实现的是基于杰出选择的单目标遗传算法。算法流程如下：
    1) 根据编码规则初始化N个个体的种群。
    2) 若满足停止条件则停止，否则继续执行。
...
"""
    ...
    def run(self):
        ...
        while self.terminated(population) == False:
            bestIndi = population[[np.argmax(population.FitnV)]] # 得到当代的最优个体
            # 选择
