...
class soea_DE_best_1_bin_templet(ea.Algorithm):
    
    """
soea_DE_best_1_bin : class - 经典差分进化算法模板

算法描述:
    本模板实现的是DE/best/1/bin单目标差分进化算法，算法流程略加修改，以适应矩阵化计算。
...
"""
    ...
    def __init__(self, problem, population):
        ...
        self.F = 0.8 # 差分变异缩放因子（可以设置为一个数也可以设置为一个列数与种群规模数目相等的列向量）
        self.pc = 0.2 # 交叉概率
        self.drawing = 1 # 绘图
    ...
    def run(self):
        ...
        while self.terminated(population) == False:
            ...
            mutPop.Chrom = ea.mutate(self.mutFunc, mutPop.Encoding, mutPop.Chrom, mutPop.Field, self.F, 1, mutPop.FitnV) # 差分变异，传入mutPop.FitnV以采用最优向量作为基向量
