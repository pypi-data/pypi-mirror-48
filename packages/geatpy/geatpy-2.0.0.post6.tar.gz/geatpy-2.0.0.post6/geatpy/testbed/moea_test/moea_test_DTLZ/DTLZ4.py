...
class DTLZ4(ea.Problem): # 继承Problem父类
    def __init__(self, M):
        self.name = 'DTLZ4' # 初始化name（函数名称，可以随意设置）
        self.M = M # 初始化M（目标维数）
    ...
    def aimFuc(self, Vars, CV):
        ...
        ObjV = np.fliplr(np.cumprod(np.hstack([ones_metrix, np.cos(Vars[:,:self.M-1]**alpha * np.pi / 2)]), 1)) * np.hstack([ones_metrix, np.sin(Vars[:, range(self.M - 2, -1, -1)]**alpha * np.pi / 2)]) * np.tile(1 + g, (1, self.M))
