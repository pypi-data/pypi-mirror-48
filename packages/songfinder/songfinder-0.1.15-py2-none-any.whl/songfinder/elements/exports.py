...
class ExportLatex(ExportBase):
	def __init__(self, element, **kwargs):
		ExportBase.__init__(self, element)
		self._newlineMarker = '\\\\'
...
class ExportHtml(ExportBase):
	def __init__(self, element, markdowner=None, htmlStyle=None, htmlStylePath=None, **kwargs):
		ExportBase.__init__(self, element)
		...
		if htmlStylePath:
			with open(htmlStylePath, 'r') as styleFile:
				self._styleText = styleFile.read()

	def _addTitle(self, text):
		start = text.find('<h')+4
		end = text.find('</h', start)
	...
	@property
	def exportText(self):
		markdownText = ExportMarkdown(self._element).exportText

		text = markdowner.convert(markdownText)
		text = self._addTitle(text)
		if self._htmlStyle:
			text = self._htmlStyle.replace('@@markdown@@', text)
		markdowner.reset()
