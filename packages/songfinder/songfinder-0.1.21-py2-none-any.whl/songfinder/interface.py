...
class Interface(object, tk.Frame):
	def __init__(self, fenetre, screens=None, fileIn=None, **kwargs):
		...
		self._expandDiapoListGuiButton = tk.Button(expandPanel, \
	...
	def _expandDiapoListGui(self):
		if self._expanded:
			...
			self._expandDiapoListGuiButton["text"] = self._expandOutLook
			self._expanded = False
		else:
			self._presentedListPanel.pack(side=tk.LEFT, fill=tk.BOTH, expand=1)
			self._expandDiapoListGuiButton["text"] = self._expandInLook
			self._expanded = True
