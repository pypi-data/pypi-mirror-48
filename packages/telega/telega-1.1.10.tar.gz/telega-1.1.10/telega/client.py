...
class TelegramTDLibClient:
    ...
    def destroy_local_data(self, close_client=True) -> None:
        """ Closes the TDLib instance, destroying all local data without a proper logout. The current user session will
        remain in the list of all active sessions. All local data will be destroyed. After the destruction completes
        updateAuthorizationState with authorizationStateClosed will be sent. """
        self.call_method('destroy')
        if close_client:
            self.close()
    ...
    def log_out(self, close_client=True, destroy_local_data=False) -> None:
        try:
            self.call_method('logOut')
        except errors.TDLibError as e:
            logger.warning('Error on log_out: %s(%s)', type(e).__name__, e)
            if self.is_authorized():
                raise e
        logger.info('Logged out %s. Current state: "%s"', self.phone, self.get_auth_state())

        if destroy_local_data:
            self.destroy_local_data()
        if close_client:
            self.close()
    ...
    def call_method(self, method_name: str, timeout=None, **params) -> dict:
        ...
        if self.is_closed:
            raise errors.ClientIsClosed('After close() method create a new client kudasai')
