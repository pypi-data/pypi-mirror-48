...
class App:
    ...
    def run_task(self, address="0.0.0.0", port=80):
        ...
        loop.call_soon(
            asyncio.start_server(self._run_handle, address, port)
        )

    @asyncio.coroutine
    def _run_handle(self, reader, writer):
        print("started")
        ...
        if route.startswith('/static'):
            ...
            callback = yield from self.router.get(route=route, method=parsed_request.get('method'))
