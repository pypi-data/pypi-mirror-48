...
def generate_qam_signal(power: typing.Union[typing.List, float], baudrate: typing.Union[float, typing.List],
    ...
    if not isinstance(baudrate, float):
        baudrate = [baudrate]
    ...
    sps_in_fiber = calc_sps_in_fiber(nch=len(power), spacing=spacing, baudrate=np.min(baudrate))

    for p, rate in zip(power, baudrate):
        signal = base.QamSignal(rate, sps_in_fiber=sps_in_fiber, mf=mf, symbol_length=symbol_length)
...
def to_wdm_array(wdmsignal: base.WdmSignal, signal_under_test_index):
    ...
    signal_under_test = wdmsignal.signals[signal_under_test_index]
    ...
    wdm_array_signal = WdmSignalFromArray(field=samples, frequencys=wdmsignal.absolute_frequences,
                                          ...
                                          signal_under_study=signal_under_test)
...
def tomat(signal, name):
    ...
    msg = signal.message
    mf = signal.mf
    savemat(name, dict(sample=samples, fs=fs, symbol=symbol, symbol_rate=symbol_rate, msg=msg, mf=mf))
