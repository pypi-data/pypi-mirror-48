...
class Library(CnvrgBase):
    ...
    def run(self, templates=None, datasets=None, image=None, commit=None, title=None, local=True, args=None):
        if local: return self.run_local(args=args, title=title)
        if not self.project: raise CnvrgError("You should specify Project on the library constructor")
        exp = self.project.run_experiment(cmd=self.info().get("command"), templates=templates, datasets=datasets, image=image, commit=commit, title=title, library=self.__library)
        return Experiment(exp.get("slug"))


    def run_local(self, args=None, title=None):
        if not self.project:
