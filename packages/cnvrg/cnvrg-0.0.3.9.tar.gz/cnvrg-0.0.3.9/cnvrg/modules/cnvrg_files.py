...
class CnvrgFiles(CnvrgBase):
    ...
    def get_storage_url(self):
        return os.path.join(self.get_base_url(), "client")


    def fetch_commit(self, commit_sha1=None, limit=100, offset=0, files=None):
        commit_sha1 = commit_sha1 or "latest"
        commit = apis_get(os.path.join(self.get_base_url(), "commits", commit_sha1), data={"limit": limit, "offset": offset, "files": files}).get("commit")
        return commit

    def get_commit_files(self, **kwargs):
        return self.fetch_commit(**kwargs).get("blobs")
    ...
    def start_commit(self, is_branch=False, force=False) -> str:
        ...
        """
        commit = apis_post(os.path.join(self.get_base_url(), 'commits'), data={"force": force, "is_branch": is_branch, "commit_sha1": self.get_current_commit()}).get("commit")
    ...
    def end_commit(self, sha1) -> bool:
        ...
        """
        success = apis_post(os.path.join(self.get_base_url(), 'commits', sha1)).get("status") == 200
    ...
    def status(self, current_tree=None, commit=None, to_commit=None, force=False, new_branch=False):
        ...
        resp = apis_post(os.path.join(self.get_base_url(), "commits", "status"), data={"tree": current_tree, "commit": commit, "to_commit": to_commit, "ignore": list(cnvrgignore_helper.get_full_ignore(self.get_working_dir()))})
    ...
    def upload_files(self, commit_sha1, upload_payload):
        ...
        """
        return apis_put(os.path.join(self.get_base_url(), 'commits', commit_sha1), data=upload_payload)
