...
class Experiment(CnvrgJob):
    ...
    def new(title=None, project=None, command=None):
        ...
        experiment = Experiment(url_join(project.get_project_name(), e.get("slug")))
        exit_status = yield(experiment)
        experiment.finish(exit_status=exit_status)
        return experiment

    @staticmethod
    def run(command, title=None, project=None, compute=None, computes=None, datasets=None, dataset=None):
        project = project or Project()
        ## support
        ...
        e = project.run_experiment(command, title=title, templates=computes, datasets=datasets)
        return Experiment(e.get("slug"))
