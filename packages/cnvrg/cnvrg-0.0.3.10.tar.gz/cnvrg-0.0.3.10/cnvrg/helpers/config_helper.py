...
def is_in_dir(dir_type, dir_element, working_dir):
    ...
    return config.get(":project_name") or config.get("project_name") or config.get("project") or config.get(":dataset_name") or config.get("dataset_name") or config.get("dataset") == dir_element


def get_element_slug(working_dir=None):
    config = load_config(find_config_dir(working_dir))
    return config.get(":project_name") or config.get("project_name") or config.get("project") or config.get(":dataset_name") or config.get("dataset_name") or config.get("dataset")


def get_element_owner(working_dir=None):
    config = load_config(find_config_dir(working_dir))
    return config.get(":owner") or config.get("owner")
