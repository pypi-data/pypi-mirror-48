...
class FracturesInput(OWWidget, RpcClient, InputMixin):
    ...
    priority = 20
    predefinedColumns = [ "Stage", "MD start", "MD end", "# of clusters", "Frac Left", "Frac Right",
                          "Frac Up", "Frac Down", "Azimuth" ]
      ...
      if data:
        ...
        for stage in data:
          row = [stage["id"],
                ...
                stage.get("fracLeft", math.nan),
                stage.get("fracRight", math.nan),
                stage.get("fracUp", math.nan),
                stage.get("fracDown", math.nan),
                stage.get("azimuth", math.nan)
          ]
          row = self.fillUserDefinedRows(stage, userCols, row)
          tableData.append(row)
          metas.append([stage["well"], stage["fractures"]])
