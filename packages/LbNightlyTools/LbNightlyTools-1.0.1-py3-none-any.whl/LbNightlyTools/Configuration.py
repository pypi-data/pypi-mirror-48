...
def loadConfig(config_module=None):
    '''
    ...
    try:
        if config_module is None:
            module, attribute = 'LHCbNightlyConf', 'slots'
        elif ':' in config_module:
            module, attribute = config_module.split(':', 1)
        else:
            module, attribute = config_module, 'slots'
        import importlib
        sys.path.insert(0, os.curdir)
        module = importlib.import_module(module)
        slot_list = getattr(module, attribute)
        logging.debug('Using explicit configuration')
        return dict((slot.name, slot) for slot in slot_list)
    except (ImportError, AttributeError, TypeError):
        pass
    sys.path = orig_path

    logging.warning('Using implicit configuration')
    configdir = os.curdir if config_module is None else config_module
...
def main(config_module):
    ...
    slots = loadConfig(config_module)
