...
### Encode or decode from a different case than my field?

JSON letter case by convention is camelCase, in Python members are by convention snake_cas.

**This library assumes your field follows the Python convention of snake_case naming.**
If your field is not snake_case to begin with, the behavior of encoding/decoding is undefined (most likely it will result in subtle bugs).

```python
@dataclass_json
@dataclass
class Student:
    id: int = field('encode_')
    name: str
```
