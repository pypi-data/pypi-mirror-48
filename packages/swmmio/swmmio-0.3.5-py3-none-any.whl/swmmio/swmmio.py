...
class Model(object):
    ...
    def to_map(self, filename=None, inproj='epsg:2272'):
        ...
        try:
            ...
            raise ImportError(
                'pyproj module needed. get this package here: https://pypi.python.org/pypi/pyproj')
        ...
        wgs = pyproj.Proj(
            proj='longlat',
            datum='WGS84',
            ellps='WGS84')  # google maps, etc

        # get center point
        c = ((nodes.X.max() + nodes.X.min()) / 2,
             (nodes.Y.max() + nodes.Y.min()) / 2)
            ...
            with open(filename, 'wb') as newmap:
                for line in bm:
                    if '//INSERT GEOJSON HERE ~~~~~' in line:
                        newmap.write(
                            'conduits = {};\n'.format(
                                geojson.dumps(geo_conduits)))
                        newmap.write(
                            'nodes = {};\n'.format(
                                geojson.dumps(geo_nodes)))
    ...
    def conduits(self):
        ...
        conduits_df = create_dataframeINP(
            inp.path, "[CONDUITS]", comment_cols=False)
        xsections_df = create_dataframeINP(
            inp.path, "[XSECTIONS]", comment_cols=False)
        conduits_df = conduits_df.join(xsections_df)
        coords_df = create_dataframeINP(
            inp.path, "[COORDINATES]")  # .drop_duplicates()
        ...
        xys = conduits_df.apply(
            lambda r: get_link_coords(
                r, coords_df, verts), axis=1)
        ...
        df['SlopeFtPerFt'] = (
            df.UpstreamInvert - df.DownstreamInvert) / df.Length
    ...
    def orifices(self):
        ...
        # create dataframes of relevant sections from the INP
        orifices_df = create_dataframeINP(
            inp.path, "[ORIFICES]", comment_cols=False)
        coords_df = create_dataframeINP(
            inp.path, "[COORDINATES]")  # .drop_duplicates()
    ...
    def node(self, node, conduit=None):
        ...
        n.nodes_upstream = functions.trace_from_node(
            self, node, mode='up')['nodes']
        n.subcats_direct = [k for k, v in subcats_inp.items() if v[1] == node]
        n.subcats_upstream = [
            k for k, v in subcats_inp.items() if v[1] in n.nodes_upstream]

        n.drainage_area_direct = sum(
            [float(x) for x in [v[2] for k, v in subcats_inp.items() if k in n.subcats_direct]])
        n.drainage_area_upstream = sum([float(
            x) for x in [v[2] for k, v in subcats_inp.items() if k in n.subcats_upstream]])
...
class SWMMIOFile(object):
    ...
    def findByteRangeOfSection(self, startStr):
        ...
        # returns the start and end "byte" location of substrings in a text
        # file
        ...
        with open(self.path) as f:
            ...
            for line in f:
                ...
                if start and line.strip() == "" and (l - start) > 100:
                    ...
                    # then we are at the first "blank" line after our start
                    # section (aka the end of the section)
                    end = l
                    break
    ...
    def createDictionary(self, sectionTitle=defaultSection):
        ...
        # #pull relevant section and clean headers
        preppedTempFilePath = txt.extract_section_from_file(
            self.path, sectionTitle)
        ...
        with open(preppedTempFilePath) as file:
            ...
            for line in file:
                ...
                # check if line is commented out (having a semicolon before
                # anything else) and skip accordingly
                if ";" == line.replace(" ", "")[0]:
                    continue  # omit this entire line

                # don't look at anything to right of a semicolon (aka a
                # comment)
                line = line.split(";")[0]

                line = ' '.join(re.findall(r'\"[^\"]*\"|\S+', line))
                rowdata = line.replace("\n", "").split(" ")
                # create dictionary row with key and array of remaing stuff on
                # line as the value
...
class rpt(SWMMIOFile):
    ...
    def __init__(self, filePath):
        ...
        with open(filePath) as f:
            # jump to 500 bytes before the end of file
            f.seek(self.file_size - 500)
            for line in f:
                if "Analysis begun on" in line:
                    date = line.split("Analysis begun on:  ")[
                        1].replace("\n", "")
        ...
        # populated if necessary elsewhere (LEGACY, can prob remove)
        self.byteLocDict = None
        self.elementByteLocations = {
            "Link Results": {},
            "Node Results": {}}  # populated if necessary elsewhere
    ...
    def createByteLocDict(self, sectionTitle="Link Results"):
        ...
        # create set of other headers that are not the desired one, use to find
        # end of section
        possibleNextSections = set(
            ['Link Results', 'Node Results', 'Subcatchment Results']) - set([sectionTitle])
        ...
        with open(self.path) as f:
            ...
            for line in f:
                ...
                if "<<<" and ">>>" in line:  # cr
                    ...
                    lineCleaned = ' '.join(re.findall(r'\"[^\"]*\"|\S+', line))
    ...
    def returnDataAtDTime(
            self, id, dtime, sectionTitle="Link Results", startByte=0):

        # this is a slow ass function, when the file is big - can we improve
        # this?
