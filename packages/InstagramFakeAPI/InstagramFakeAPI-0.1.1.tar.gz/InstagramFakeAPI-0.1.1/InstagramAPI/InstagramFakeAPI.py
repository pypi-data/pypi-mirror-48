...
class InstagramFakeAPI:
    ...
    def favorite(self, userId):
        data = json.dumps({'_uuid': self.uuid,
                           '_uid': self.username_id,
                           'user_id': userId,
                           '_csrftoken': self.token})
        return self.SendRequest('friendships/favorite/' + str(userId) + '/', self.generateSignature(data))

    def unfavorite(self, userId):
        data = json.dumps({'_uuid': self.uuid,
                           '_uid': self.username_id,
                           'user_id': userId,
                           '_csrftoken': self.token})
        return self.SendRequest('friendships/unfavorite/' + str(userId) + '/', self.generateSignature(data))
