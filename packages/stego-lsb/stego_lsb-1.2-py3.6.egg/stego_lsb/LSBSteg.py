...
def recover_message_from_image(input_image, num_lsb):
    ...
    plen = roundup(8 * bytes_to_recover / num_lsb)
    print(f"bytes_to_recover={bytes_to_recover}")
    print(f"len(color_data)={len(color_data)}")
    print(f"len(color_data[tag_bit_height:])={len(color_data[tag_bit_height:])}")
    print(f"plen={plen}")

    maximum_bytes_in_image = num_lsb*len(color_data[tag_bit_height:]) // 8
    if bytes_to_recover > maximum_bytes_in_image:
        raise ValueError(f"This image appears corrupted.\n" +
