...
ICONS = ['\x1b[0;95m{}\x1b[0m'.format(x) + ' ' for x in '❶❷❸❹❺❻❼❽❾❿']
...
def clear_empty_file(pathin, extensions=None, do_clear=False):
    if not os.path.exists(pathin):
        zlog.debug('{} not exist'.format(pathin))
        return
    ...
    for root, dirs, files in os.walk(pathin):
        for f in files:
            ...
            if not is_file_ok(_fpth):
                zlog.debug('_delete_ file: {}/{}'.format(root, f.split('-')[1]))
                if do_clear:
                    os.remove(_fpth)
...
def b64_file(src_file):
    ...
    if not z:
        zlog.warning('cannot open {} '.format(src_file))
        return z
