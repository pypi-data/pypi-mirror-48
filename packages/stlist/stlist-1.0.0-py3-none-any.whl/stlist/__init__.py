"""
stlist -- a library for reading and writing binary property list files.
...
class PlistReader(object):
    ...
    def readRoot(self):
        ...
        try:
            self.trailer = PlistTrailer._make(struct.unpack("!xxxxxxBBQQQ", trailerContents))
    ...
    def readObject(self):
        ...
        marker_byte = struct.unpack("!B", tmp_byte)[0]
    ...
    def readAsciiString(self, length):
        ...
        result = struct.unpack("!%ds" % length, data)[0]
            ...
            if hasattr(int, 'from_bytes'):
                ...
                for byte in data:
                    if not isinstance(byte, int): # Python3.0-3.1.x return ints, 2.x return str
                        byte = struct.unpack_from('>B', byte)[0]
...
class PlistWriter(object):
    ...
    def writeRoot(self, root):
        ...
        output += struct.pack('!xxxxxxBBQQQ', *self.trailer)
    ...
    def writeObject(self, obj, output, setReferencePosition=False):
        ...
        def proc_variable_length(format, length):
            result = b''
            if length > 0b1110:
                result += struct.pack('!B', (format << 4) | 0b1111)
                result = self.writeObject(length, result)
            else:
                result += struct.pack('!B', (format << 4) | length)
            return result
        ...
        if obj is None:
            output += struct.pack('!B', 0b00000000)
        elif isinstance(obj, BoolWrapper):
            if obj.value is False:
                output += struct.pack('!B', 0b00001000)
            else:
                output += struct.pack('!B', 0b00001001)
        elif isinstance(obj, Uid):
            size = self.intSize(obj.integer)
            output += struct.pack('!B', (0b1000 << 4) | size - 1)
            output += self.binaryInt(obj.integer)
        elif isinstance(obj, (int, long)):
            byteSize = self.intSize(obj)
            root = math.log(byteSize, 2)
            output += struct.pack('!B', (0b0001 << 4) | int(root))
            output += self.binaryInt(obj, as_number=True)
        elif isinstance(obj, FloatWrapper):
            # just use doubles
            output += struct.pack('!B', (0b0010 << 4) | 3)
            ...
            output += struct.pack('!B', 0b00110011)
            output += struct.pack('!d', float(timestamp))
        elif isinstance(obj, Data):
    ...
    def binaryReal(self, obj):
        # just use doubles
        result = struct.pack('>d', obj.value)
        return result
    ...
    def binaryInt(self, obj, byteSize=None, as_number=False):
        ...
        if byteSize is None:
            byteSize = self.intSize(obj)
        if byteSize == 1:
            result += struct.pack('>B', obj)
        elif byteSize == 2:
            result += struct.pack('>H', obj)
        elif byteSize == 4:
            result += struct.pack('>L', obj)
        elif byteSize == 8:
            if as_number:
                result += struct.pack('>q', obj)
            else:
                result += struct.pack('>Q', obj)
        elif byteSize <= 16:
            try:
                result = struct.pack('>Q', 0) + struct.pack('>Q', obj)
