...
async def read(cid, news_id, *auth_args):
    c = ctrl_catalog.simple_read(cid)

    # 授权检查
    pp = Passport().verify(*auth_args).operable('{}/*'.format(c.get('node')), 'news.read')
    visible_level = pp.number(c.get('node'), 'news.visible_level')
...
async def query_news(cid, page_no, page_size, *auth_args):
    c = ctrl_catalog.simple_read(cid)
