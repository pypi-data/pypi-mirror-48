...
class Login:
    ...
    def download(self, track_id, info_download={}, ip= None, output="/var/www/html/music/download",quality='128,320'):
        ...
        while not "MD5_ORIGIN" in str(infos):
            infos = login()
        if int(infos['FILESIZE_FLAC']) > 0  and 'flac' in quality:
            quality = "9"
            extension = ".flac"
            try:
        ...
        if int(infos['FILESIZE_MP3_320']) > 0  and '320' in quality:
            quality = "3"
            extension = ".mp3"
            try:
                ...
                pass
        if int(infos['FILESIZE_MP3_128']) > 0  and '128' in quality:
            quality = "1"
            extension = ".mp3"
            try:
