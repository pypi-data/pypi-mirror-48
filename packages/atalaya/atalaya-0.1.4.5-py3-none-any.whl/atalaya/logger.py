...
class Logger:
    def __init__(self, path='', name='exp', add_time=True, verbose=True,
                 grapher='', server='http://localhost', port=8097,
        ...
        self.logs_dir = os.path.join(path, self.name)
    ...
    def store(self, loss, save_every=1, overwrite=True):
        ...
        if loss < self._loss:
            self._save(os.path.join(self.logs_dir, 'best.pth'))
            self._loss = loss
            return True
        return False
    ...
    def register_plots(self, values, epoch, prefix, apply_mean=True, save_csv=True, info=True):
    ...
    def restore(self, folder=None, best=False):
        ...
        if not self.path:
            raise ValueError('You have initialized the logger without specifying a path. You cannot restore any model...')
