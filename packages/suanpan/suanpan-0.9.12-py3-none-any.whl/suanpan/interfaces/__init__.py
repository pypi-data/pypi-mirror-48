...
class HasArguments(object):
    ...
    def getArgList(cls, *args, **kwargs):  # pylint: disable=unused-argument
        tmp = {}
        tmp.update(cls.dictFromList(cls.getArgListFromEnv()))
        tmp.update(cls.dictFromList(cls.getArgListFromCli()))
        tmp.update({"--{}".format(k): v for k, v in kwargs.items()})
        print(tmp)
        return cls.listFromDict(tmp)
