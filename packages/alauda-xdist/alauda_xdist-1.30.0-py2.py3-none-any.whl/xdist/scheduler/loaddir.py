...
class LoadDirScheduling(LoadScopeScheduling):
    ...
    def __init__(self, config, log=None):
        super(LoadDirScheduling, self).__init__(config, log)
        if log is None:
            self.log = Producer("loaddirsched")
        else:
    ...
    def _split_scope(self, nodeid):
        ...
        single work unit when they reside in the same file.
         In the above example, scopes will be::
            ...
            example/loadsuite/test/
            example/loadsuite/test/
            example/loadsuite/epsilon/
        """
        return nodeid.split("::", 1)[0].rsplit("/",1)[0]
