...
class WMS:
	...
	def getmap(self, url, version=False, request="GetMap", layers="", styles="", crs="", bbox="", width="", height="", format="", **kargs):
		...
		# Add all parameters to a dict called params
		params = {"version":version, "service":"wms", "request":request, "layers":layers, "styles":styles, "crs":crs, "bbox":bbox, "width":width, "height":height, "format":format}
		params.update(kargs)
...
class getCapabilitiesObject:
	def __init__(self, response, autoDecode, source, session):
		...
		# test type
		if type in ['application/xml', 'text/xml; charset=UTF-8']:
			# Parse the xml
