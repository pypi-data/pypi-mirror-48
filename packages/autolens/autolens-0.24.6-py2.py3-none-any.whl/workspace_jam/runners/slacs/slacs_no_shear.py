...
mask = mask.resized_scaled_array_from_array(
    new_shape=(301, 301))
...
# from workspace_jam.pipelines.with_lens_light.power_law.from_inversion import lens_sersic_pl_source_inversion

pl_pixelization = pix.VoronoiBrightnessImage
pl_regularization = reg.Constant

pipeline_initializer = lens_sersic_exp_sie_source_sersic.make_pipeline(
    phase_folders=[data_filter, data_name], positions_threshold=1.0, inner_mask_radii=0.3,
...
pipeline_inversion = lens_sersic_exp_sie_source_inversion.make_pipeline(
    ...
    positions_threshold=1.0, inner_mask_radii=0.3)

# pipeline_power_law = lens_sersic_pl_source_inversion.make_pipeline(phase_folders=[data_type, data_name])
