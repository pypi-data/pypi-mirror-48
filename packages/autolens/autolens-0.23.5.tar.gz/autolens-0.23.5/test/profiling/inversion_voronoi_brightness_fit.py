...
pixelization = pix.VoronoiBrightnessImage(pixels=pixelization_pixels, weight_floor=0.0, weight_power=5.0)

source_galaxy = g.Galaxy(redshift=1.0, pixelization=pixelization, regularization=reg.Constant(coefficients=(1.0,)))
...
for data_resolution in ['Euclid', 'HST', 'HST_Up', 'AO']:
    ...
    lens_data = ld.LensData(ccd_data=ccd_data, mask=mask, sub_grid_size=sub_grid_size, cluster_pixel_scale=cluster_pixel_scale)

    source_galaxy = g.Galaxy(redshift=1.0,
                             light=lp.EllipticalSersic(centre=(0.0, 0.0), axis_ratio=0.8, phi=60.0,
    ...
    hyper_image_2d = lens_data.grid_stack.regular.scaled_array_2d_from_array_1d(array_1d=hyper_image)
    ...
    hyper_image_1d_binned = lens_data.cluster.mask.map_2d_array_to_masked_1d_array(array_2d=hyper_image_2d_binned)

    source_galaxy = g.Galaxy(redshift=1.0, pixelization=pixelization, regularization=reg.Constant(coefficients=(1.0,)))

    print('VoronoiBrightnessImage Inversion fit run times for image type ' + data_resolution + '\n')
    ...
    for i in range(repeats):
        cluster_weight_map = pixelization.cluster_weight_map_from_hyper_image(hyper_image=hyper_image_1d_binned)
    ...
    for i in range(repeats):
        sparse_to_regular_grid = grids.SparseToRegularGrid.from_total_pixels_cluster_grid_and_cluster_weight_map(
            total_pixels=pixelization.pixels, regular_grid=lens_data.grid_stack.regular,
            cluster_grid=lens_data.cluster, cluster_weight_map=cluster_weight_map)
