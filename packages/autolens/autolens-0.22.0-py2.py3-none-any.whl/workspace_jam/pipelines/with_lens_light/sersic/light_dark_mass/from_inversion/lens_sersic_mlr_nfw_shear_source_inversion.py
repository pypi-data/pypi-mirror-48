...
def make_pipeline(
    ...
    phase1 = LensSourcePhase(
        ...
        source_galaxies=dict(source=gm.GalaxyModel(redshift=redshift_source, pixelization=pix.VoronoiMagnification,
    ...
    class InversionPhase(ph.LensSourcePlanePhase):
        ...
        def pass_priors(self, results):
            ...
            self.source_galaxies.source = results.from_phase('phase_1_lens_sersic_mlr_nfw_shear_source_inversion'). \
    ...
    phase2 = InversionPhase(
        ...
        lens_galaxies=dict(lens=gm.GalaxyModel(redshift=redshift_lens, light=lmp.EllipticalSersic,
                                               dark=mp.SphericalNFW, shear=mp.ExternalShear)),
        source_galaxies=dict(source=gm.GalaxyModel(redshift=redshift_source, pixelization=pix.VoronoiMagnification,
                                                   regularization=reg.Constant)),
        sub_grid_size=sub_grid_size, bin_up_factor=bin_up_factor, positions_threshold=positions_threshold,
        inner_mask_radii=inner_mask_radii, interp_pixel_scale=interp_pixel_scale,
