...
class EngineAPI:
    ...
    @update_aml_env_vars(get_engine_api)
    def execute_inspectors(self, message_args: List[typedefinitions.ExecuteInspectorsMessageArguments]) -> Dict[str, typedefinitions.ExecuteInspectorCommonResponse]:
        response = self._engine.send_message('Engine.ExecuteInspectors', message_args)
    ...
    def register_secret(self, message_args: typedefinitions.RegisterSecretMessageArguments) -> typedefinitions.Secret:
        response = self._engine.send_message('SecretManager.RegisterSecret', message_args)
