...
from ._pandas_helper import have_pandas, have_pyarrow
...
from ._datastore_helper import (datastore_to_dataflow, get_datastore_value, _is_datapath, _is_datapaths, Datastore,
                                Datastores)
from ._dataframereader import get_dataframe_reader, _InconsistentSchemaError
...
class Dataflow:
    ...
    def to_pandas_dataframe(self, extended_types: bool = False, nulls_as_nan: bool = True) -> 'pandas.DataFrame':
        ...
        if have_pyarrow() and not extended_types:
            random_id = str(uuid4())
            get_dataframe_reader().register_incoming_dataframe(random_id, extended_types, nulls_as_nan)
        ...
        if use_preppy:
            if not extended_types and not fallback:
                warnings.warn('Please install pyarrow for improved performance of to_pandas_dataframe. You can ensure '
                              'the correct version is installed by running: pip install azureml-dataprep[pandas]')
