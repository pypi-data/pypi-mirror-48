...
from azureml.automl.core.package_utilities import (_all_dependencies,
                                                   _has_version_discrepancies,
                                                   get_sdk_dependencies)
from automl.client.core.common.exceptions import ClientException, OnnxConvertException, ConfigException
...
class AutoMLRun(Run):
    ...
    def get_output(self, iteration=None, metric=None, return_onnx_model=None):
        ...
        if iteration is not None and metric is not None:
            raise ValueError('Cannot spcify both metric and iteration to register.')
        ...
        if _has_version_discrepancies(run_deps):
            ...
            logging.warn(
                ...
                "\n\nTry running `pip install --upgrade azureml-sdk[automl]=={}`\n\n".format(ver)
            )
