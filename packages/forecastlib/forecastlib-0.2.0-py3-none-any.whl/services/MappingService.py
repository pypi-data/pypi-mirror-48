...
class MappingService(object):
    ...
    def merge_history(self, df: pd.DataFrame):
        ...
        if 'CHAN_CD' in self.history.columns:
            df = df.merge(self.history[["PROD_ID", "CHAN_CD", "OFFER_PERC_M", "AVG_UPA", "AVG_UPA_FULL"]], how="left", on=["PROD_ID", "CHAN_CD", "OFFER_PERC_M"])
        else:
            df = df.merge(self.history[["PROD_ID", "OFFER_PERC_M", "AVG_UPA", "AVG_UPA_FULL"]], how="left", on=["PROD_ID", "OFFER_PERC_M"])
