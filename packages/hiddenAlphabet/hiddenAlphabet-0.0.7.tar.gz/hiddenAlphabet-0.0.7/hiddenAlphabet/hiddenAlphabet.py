...
class HiddenAlphabet(object):
    ...
    def __init__(self, ticker, pandas=False):
        ...
        if pandas:
            self.sentiment_time_series = pd.DataFrame(self.rawSentimentData)
        else:
    ...
    def getSentiment(self, text):
        ...
        if type(text) == list:
            text = text
            r = requests.post('https://intense-bastion-55085.herokuapp.com/calculate', data=json.dumps({"text": str(text)}))
            return r.json()
        elif type(text) == str:
            text = text
            r = requests.post('https://intense-bastion-55085.herokuapp.com/calculate', data=json.dumps({"text": str(list(text))}))
            return r.json()
        else:
    ...
    def sentiment_volatility(self, data, periods, min_periods=1):
        ...
        df = pd.DataFrame(self.sentiment_time_series)
    ...
    def stockTimeseries(self, ticker):
        """
        Stock timeSeries
        """
        url = 'https://intense-bastion-55085.herokuapp.com/historical_stock'
