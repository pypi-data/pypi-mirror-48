    ...
    def save(self, client):
        ...
        if self.id or self.exists(client):
            ...
            if self.instance is None:
                ...
                logger.debug("Retrieving previously-saved instance of {}".format(self.__class__.__name__))
        ...
        if self.instance:
            ...
            logger.debug("Added {} to cache: {}".format(self.__class__.__name__, self.id))
...
def cache(f):
    ...
    def wrapper(cls, instance, client, use_cache=True):
        if use_cache and instance.data["@id"] in KGObject.cache:
            ...
            logger.debug("Found {} in cache: {}".format(cls.__name__, obj.id))
            return obj
        else:
            obj = f(cls, instance, client)
            KGObject.cache[obj.id] = obj
            logger.debug("Added {} to cache: {}".format(cls.__name__, obj.id))
            return obj
    return wrapper
...
class KGProxy(object):
    ...
    def resolve(self, client):
        ...
        if self.id in KGObject.cache:
            ...
            logger.debug("Added {} to cache: {}".format(self.cls.__name__, self.id))
            return obj
