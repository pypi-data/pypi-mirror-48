...
let RemoteMachineTrainingService = class RemoteMachineTrainingService {
    ...
    async setClusterMetadata(key, value) {
        switch (key) {
            ...
            case trialConfigMetadataKey_1.TrialConfigMetadataKey.MACHINE_LIST:
                ...
                await util_1.execRemove(this.getLocalGpuMetricCollectorDir());
                break;
    ...
    async cleanupConnections() {
        try {
            for (const [rmMeta, sshClientManager] of this.machineSSHClientMap.entries()) {
                let jobpidPath = utils_1.unixPathJoin(this.getRemoteScriptsPath(rmMeta.username), 'pid');
    ...
    async generateGpuMetricsCollectorScript(userName) {
        ...
        await util_1.execMkdir(path.join(gpuMetricCollectorScriptFolder, userName));
        ...
        const gpuMetricsCollectorScriptContent = typescript_string_operations_1.String.Format(gpuData_1.GPU_INFO_COLLECTOR_FORMAT_LINUX, remoteGPUScriptsDir, utils_1.unixPathJoin(remoteGPUScriptsDir, 'pid'));
    ...
    async initRemoteMachineOnConnected(rmMeta, conn) {
        ...
        await sshClientUtility_1.SSHClientUtility.copyFileToRemote(path.join(localGpuScriptCollectorDir, rmMeta.username, 'gpu_metrics_collector.sh'), utils_1.unixPathJoin(remoteGpuScriptCollectorDir, 'gpu_metrics_collector.sh'), conn);
        sshClientUtility_1.SSHClientUtility.remoteExeCommand(`bash ${utils_1.unixPathJoin(remoteGpuScriptCollectorDir, 'gpu_metrics_collector.sh')}`, conn);
    ...
    async launchTrialOnScheduledMachine(trialJobId, trialWorkingFolder, form, rmScheduleInfo) {
        ...
        await util_1.execCopydir(path.join(this.trialConfig.codeDir, '*'), trialLocalTempFolder);
    ...
    async runHostJob(form) {
        ...
        await sshClientUtility_1.SSHClientUtility.copyFileToRemote(path.join(localDir, 'run.sh'), utils_1.unixPathJoin(remoteDir, 'run.sh'), sshClient);
        sshClientUtility_1.SSHClientUtility.remoteExeCommand(`bash ${utils_1.unixPathJoin(remoteDir, 'run.sh')}`, sshClient);
    ...
    getJobPidPath(jobId) {
        ...
        if (trialJobDetail.form.jobType === 'TRIAL') {
            ...
            jobpidPath = utils_1.unixPathJoin(this.getHostJobRemoteDir(jobId), 'jobpid');
        }
        else {
