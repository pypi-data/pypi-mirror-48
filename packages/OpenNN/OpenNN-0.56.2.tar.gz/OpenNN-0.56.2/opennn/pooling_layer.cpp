...
namespace OpenNN {
    ...
    PoolingLayer::PoolingLayer(const Vector<size_t>& input_size) : Layer()
    {
    ...
    PoolingLayer::PoolingLayer(const Vector<size_t>& input_size, const Vector<size_t>& pool_size) : Layer()
    {
    ...
    Matrix<double> PoolingLayer::calculate_outputs_average_pooling(const Matrix<double> & inputs) const
    {
        ...
        for(size_t neuron = 0; neuron < neurons_number; neuron++)
        {
            ...
            const Vector<double> mean_rows_values = submatrix.calculate_rows_sum() / static_cast<double>(submatrix.get_columns_number());
...
//        instance_of = "PoolingLayer";
        layer_type = Layer::Pooling_Layer;
