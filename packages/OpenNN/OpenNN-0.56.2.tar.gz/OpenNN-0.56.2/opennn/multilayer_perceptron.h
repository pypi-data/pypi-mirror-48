...
public:
   ...
   struct Pointers
   {
       ...
       void update_parameters_sgd(double*&, const double&, const bool&, const double&, const double&, const size_t&);

       void update_parameters_adam(double*&, const double&, const double&, const double&, const double&, const double&, const size_t&);
   ...
   struct ForwardPropagation
   {
       ...
       ForwardPropagation(const size_t layers_number)
       {
           layers_activations.set(layers_number);
           layers_activations_derivatives.set(layers_number);
       }
       ...
       void print() const
       {
           const size_t layers_number = layers_activations.size();

           for(size_t i = 0; i < layers_number; i++)
           {
               cout << "Activations layer " << i << ":" << endl;

               cout << layers_activations[i] << endl;

               cout << "Activations derivatives layer " << i << ":" << endl;

               cout << layers_activations_derivatives[i] << endl;
           }
       }

       Vector< Matrix<double> > layers_activations;
       Vector< Matrix<double> > layers_activations_derivatives;
   };
   ...
   inline Vector<size_t> get_layers_neurons_numbers() const
   {
       const size_t layers_number = get_layers_number();

       Vector<size_t> layers_size(layers_number);

       for(size_t i = 0; i < layers_number; i++)
           layers_size[i] = layers[i].get_neurons_number();
   ...
   Vector<float*> get_layers_biases_cuda() const;
   Vector<float*> get_layers_synaptic_weights_cuda() const;
   ...
   Vector<size_t> get_layers_biases_numbers() const;
   Vector<size_t> get_layers_synaptic_weights_numbers() const;
   ...
   ForwardPropagation calculate_forward_propagation(const Matrix<double>&) const;
...
//   Vector< Vector< Matrix<double> > > calculate_layers_Jacobian(const Matrix<double>&) const;
  ...
  struct CudaForwardPropagation
  {
      /// Default constructor.

      CudaForwardPropagation(MultilayerPerceptron* new_multilayer_perceptron_pointer, const size_t& new_batch_size)
      {
      ...
      virtual ~CudaForwardPropagation()
      {
            free();
      }
      ...
      Vector<float*> layers_combinations;
      Vector<float*> layers_activations;
      Vector<float*> layers_activations_derivatives;

      float* ones = nullptr;
  };


  void cuda_calculate_forward_propagation(const DataSet::CudaBatch&,
                                          MultilayerPerceptron::CudaForwardPropagation&) const;
...
protected:
   ...
   /// Multilayer perceptron layers. It is built as a vector of perceptron layers.
