...
void NeuralNetwork::set(const Vector<size_t>& new_multilayer_perceptron_architecture)
{
    ...
    {
        layers_pointers.push_back(multilayer_perceptron_pointer->get_layer_pointer(i));
    }
...
void NeuralNetwork::set(const size_t& new_inputs_number, const size_t& new_outputs_number)
{
    ...
    layers_pointers.push_back(multilayer_perceptron_pointer->get_layer_pointer(0));
...
void NeuralNetwork::set(const size_t& new_inputs_number, const size_t& new_hidden_neurons_number, const size_t& new_outputs_number)
{
    ...
    layers_pointers.push_back(multilayer_perceptron_pointer->get_layer_pointer(0));
    layers_pointers.push_back(multilayer_perceptron_pointer->get_layer_pointer(1));

    set_default();
}


void NeuralNetwork::set_new(const size_t& new_inputs_number, const size_t& new_hidden_neurons_number, const size_t& new_outputs_number)
{
    delete_pointers();

    inputs_pointer = new Inputs(new_inputs_number);

    PerceptronLayer hidden_layer(new_inputs_number, new_hidden_neurons_number);

    PerceptronLayer output_layer(new_hidden_neurons_number, new_outputs_number, PerceptronLayer::Linear);
...
void NeuralNetwork::set_layers_pointer(Vector<Layer*>& layers_vector)
{
    layers_pointers = layers_vector;
}
...
size_t NeuralNetwork::get_inputs_number_new() const
{
    if(layers_pointers.size() > 0)
    {
        Layer* first_layer = layers_pointers[0];
        return first_layer->get_biases().size();
    }
...
size_t NeuralNetwork::get_outputs_number_new() const
{
    if(layers_pointers.size() > 0)
    {
        Layer* first_layer = layers_pointers[layers_pointers.size() -1];
        return first_layer->get_biases().size();
    }
...
Vector<size_t> NeuralNetwork::get_architecture_new() const
{
    Vector<size_t> architecture;

    const size_t inputs_number = get_inputs_number_new();
    ...
    const size_t layers_number = get_layers_number_new();
...
size_t NeuralNetwork::get_parameters_number() const
{
    ...
    else
    {
        ...
        {
            parameters_number += layers_pointers[i]->get_parameters_number();
        }
    }
...
Vector<double> NeuralNetwork::get_parameters_new() const
{
    // Only network parameters
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        const Vector<double> new_parameters = layers_pointers[i] -> get_parameters();
...
Vector<size_t> NeuralNetwork::get_layers_parameters_number() const
{
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        layers_parameters_number[i] = layers_pointers[i] -> get_parameters_number();
    }
...
Vector<size_t> NeuralNetwork::get_layers_parameters_number_new() const
{
    const size_t layers_number = get_layers_number_new();
...
void NeuralNetwork::set_parameters(const Vector<double>& new_parameters)
{
    ...
    {
        Vector<double> parameters_tmp = new_parameters;

        for(size_t layer = 0; layer < layers_pointers.size(); layer++)
        {

            const Vector<double> layer_parameters = parameters_tmp.get_first(layers_pointers[layer] -> get_parameters_number());

            layers_pointers[layer] -> set_parameters(layer_parameters);

            parameters_tmp = parameters_tmp.delete_first(layers_pointers[layer] -> get_parameters_number());
...
void NeuralNetwork::construct_scaling_layer(const Eigen::MatrixXd& input_statistics)
{
    ...
    Vector< Statistics<double> > statistics(rows_number);

    for(Eigen::Index i=0; i < rows_number; i++)
    {
        statistics[i] = Statistics<double>(input_statistics(i, 0),
                                      input_statistics(i, 1),
                                      input_statistics(i, 2),
                                      input_statistics(i, 3));
    }

    construct_scaling_layer(statistics);
}
...
void NeuralNetwork::construct_unscaling_layer(const Eigen::MatrixXd& target_statistics)
{
    ...
    Vector< Statistics<double> > statsistics(rows_number);

    for(Eigen::Index i=0; i < rows_number; i++)
    {
        statsistics[i] = Statistics<double>(target_statistics(i, 0),
                                      target_statistics(i, 1),
                                      target_statistics(i, 2),
                                      target_statistics(i, 3));
    }

    construct_unscaling_layer(statsistics);
}
...
size_t NeuralNetwork::get_layers_number_new() const
{
...
void NeuralNetwork::initialize_parameters(const double& value)
{
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        layers_pointers[i]->initialize_parameters(value);
    }
}


void NeuralNetwork::initialize_parameters_new(const double& value)
{
    if(layers_pointers.size() > 0)
    {
        for(size_t i = 0; i < layers_pointers.size(); i++)
...
void NeuralNetwork::randomize_parameters_uniform()
{
    if(layers_pointers.size() > 0)
    {
        ...
        CudaLayersParameters layers_parameters(layers_number);
        layers_parameters.biases.randomize_uniform();
        layers_parameters.synaptic_weights.randomize_uniform();


        for(int i = 0; i < layers_pointers.size(); i++)
        {
            Layer* layer = layers_pointers[i];
            layer->randomize_parameters_uniform();
        }
    }
...
void NeuralNetwork::randomize_parameters_uniform_new()
{
    const size_t layers_number = layers_pointers.size();
    if(layers_number > 0)
    {
        ...
        for(size_t i = 0; i < layers_number; i++)
        {
            layers_pointers[i]->randomize_parameters_uniform();
        }
    }
}
...
Matrix<double> NeuralNetwork::calculate_outputs(const Matrix<double>& inputs) const
{
    ...
    if(multilayer_perceptron_pointer)
    {
        ...
        if(inputs_size != inputs_number)
        {
            ...
            buffer << "OpenNN Exception: NeuralNetwork class.\n"
                   << "Matrix<double> calculate_outputs(const Vector<double>&) const method.\n"
...
Matrix<double> NeuralNetwork::calculate_outputs_new(const Matrix<double>& inputs) const
{
    ...
    Matrix<double> outputs = layers_pointers[0] -> calculate_outputs(inputs);
...
Matrix<double> NeuralNetwork::calculate_outputs_new(const Matrix<double>& inputs, const Vector<double>& parameters) const
{
    const size_t layers_number = layers_pointers.size();
    ...
    if(layers_number <= 0)
    {
        ...
        buffer << "OpenNN Exception: NeuralNetwork class.\n"
               << "Matrix<double> calculate_outputs_new(const Matrix<double>&, cons Vector<double>&) const method.\n"
    ...
    const Vector<size_t> architecture = get_architecture_new();

    const CudaLayersParameters layers_parameters(architecture, parameters);

    if(layers_pointers[0] -> is_trainable())
    {
        const size_t first_layer_parameters_number = layers_pointers[0] -> get_parameters_number();
        const Vector<double> first_layer_parameters = parameters.get_first(first_layer_parameters_number);

//        const Matrix<double> first_layer_synaptic_weights = layers_pointers[0] -> parameters_to_synaptic_weights(first_layer_parameters);
//        const Vector<double> first_layer_biases = layers_pointers[0] -> parameters_to_biases(first_layer_parameters);

        const Matrix<double> first_layer_synaptic_weights = layers_pointers[0] -> get_synaptic_weights(first_layer_parameters);
        const Vector<double> first_layer_biases = layers_pointers[0]->get_biases(first_layer_parameters);
        output = layers_pointers[0] -> calculate_outputs(inputs, first_layer_biases, first_layer_synaptic_weights);

        layer_parameters = parameters.delete_first(first_layer_parameters_number);
    }
    else
    {
        output = layers_pointers[0] -> calculate_outputs(inputs);
    }

    for(size_t i = 1; i < layers_number; i++)
    {
        const size_t layer_parameters_number = layers_pointers[i] -> get_parameters_number();
        const Vector<double> this_layer_parameters = layer_parameters.get_first(layer_parameters_number);

//        const Matrix<double> synaptic_weights = layers_pointers[i] -> parameters_to_synaptic_weights(this_layer_parameters);
//        const Vector<double> biases = layers_pointers[i] -> parameters_to_biases(this_layer_parameters);

        const Matrix<double> synaptic_weights = layers_pointers[i]->get_synaptic_weights(this_layer_parameters);
        const Vector<double> biases = layers_pointers[i]->get_biases(this_layer_parameters);

        if(layers_pointers[i] -> is_trainable())
        {
            output = layers_pointers[i] -> calculate_outputs(output, biases, synaptic_weights);
            layer_parameters = layer_parameters.delete_first(layer_parameters_number);
        }
        else
        {
            output = layers_pointers[i] -> calculate_outputs(output);
        }
    }
    return output;
}
...
Matrix<double> NeuralNetwork::calculate_Jacobian(const Vector<double>& inputs, const double&) const
{
#ifdef __OPENNN_DEBUG__
...
NeuralNetwork::ForwardPropagation NeuralNetwork::calculate_first_order_forward_propagation_new(const Matrix<double>& inputs)
{
    const size_t layers_number = get_layers_number_new();

    ForwardPropagation forward_propagation(layers_number);

    Matrix<double> layer_combinations = layers_pointers[0] -> calculate_combinations(inputs);

    forward_propagation.layers_activations[0] = layers_pointers[0] -> calculate_activations(layer_combinations);

    forward_propagation.layers_activations_derivatives[0] = layers_pointers[0] -> calculate_activations_derivatives(layer_combinations);

    for(size_t i = 1; i < layers_number; i++)
    {
        layer_combinations = layers_pointers[i] -> calculate_combinations(forward_propagation.layers_activations[i-1]);

        forward_propagation.layers_activations[i] =  layers_pointers[i] -> calculate_activations(layer_combinations);

        forward_propagation.layers_activations_derivatives[i] =  layers_pointers[i] -> calculate_activations_derivatives(layer_combinations);
    }
