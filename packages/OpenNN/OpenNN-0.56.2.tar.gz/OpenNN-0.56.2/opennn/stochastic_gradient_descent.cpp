...
OptimizationAlgorithm::Results StochasticGradientDescent::perform_training_cuda()
{
    // Check CUDA
...
//   Vector<double> nesterov_increment(parameters_number);
   ...
   for(size_t epoch = 0; epoch <= epochs_number; epoch++)
   {
       ...
       for(size_t iteration = 0; iteration < batches_number; iteration++)
       {
           ...
           multilayer_perceptron_pointers_device.update_parameters_sgd(first_order_loss.gradient_device,
                                                                       momentum,
                                                                       nesterov,
       ...
       {
           selection_error = loss_index_pointer->calculate_batch_error_cuda(instances.get_selection_indices(),
                                                                            multilayer_perceptron_pointers_device);
       }
...
void StochasticGradientDescent::CudaParameters::allocate()
{
    const MultilayerPerceptron* multilayer_perceptron_pointer
            = stochastic_gradient_descent_pointer->get_loss_index_pointer()->get_neural_network_pointer()->get_multilayer_perceptron_pointer();
...
void StochasticGradientDescent::CudaParameters::print() const
{
    const MultilayerPerceptron* multilayer_perceptron_pointer
            = stochastic_gradient_descent_pointer->get_loss_index_pointer()->get_neural_network_pointer()->get_multilayer_perceptron_pointer();
    ...
    Vector<float> parameters_host;
    parameters_host.from_device(parameters, parameters_number);
...
void StochasticGradientDescent::CudaParameters::free()
{
    cudaFree(parameters);
}


void StochasticGradientDescent::CudaParameters::get_parameters() const
{
    const MultilayerPerceptron* multilayer_perceptron_pointer
            = stochastic_gradient_descent_pointer->get_loss_index_pointer()->get_neural_network_pointer()->get_multilayer_perceptron_pointer();
    ...
    const Vector<size_t> layers_biases_numbers = multilayer_perceptron_pointer->get_layers_biases_numbers();
    const Vector<size_t> layers_synaptic_weights_numbers = multilayer_perceptron_pointer->get_layers_synaptic_weights_numbers();

    const Vector<float*> cuda_layers_biases = multilayer_perceptron_pointer->get_layers_biases_cuda();
    const Vector<float*> cuda_layers_synaptic_weights = multilayer_perceptron_pointer->get_layers_synaptic_weights_cuda();

    size_t index = 0;

    for(size_t i = 0; i < layers_number; i++)
    {
        if(cudaMemcpy(parameters + index, cuda_layers_synaptic_weights[i], layers_synaptic_weights_numbers[i]*sizeof(float), cudaMemcpyDeviceToDevice) != cudaSuccess)
            cout << "Synaptic weights copy error" << endl;

        index += layers_synaptic_weights_numbers[i];

        if(cudaMemcpy(parameters + index, cuda_layers_biases[i], layers_biases_numbers[i]*sizeof(float), cudaMemcpyDeviceToDevice) != cudaSuccess)
            cout << "Biases copy error" << endl;

        index += layers_biases_numbers[i];
    }
}


void StochasticGradientDescent::CudaParameters::set_parameters() const
{
    const MultilayerPerceptron* multilayer_perceptron_pointer
            = stochastic_gradient_descent_pointer->get_loss_index_pointer()->get_neural_network_pointer()->get_multilayer_perceptron_pointer();
    ...
    for(size_t i = 0; i < layers_number; i++)
    {
        if(cudaMemcpy(cuda_layers_synaptic_weights[i], parameters + index, layers_synaptic_weights_numbers[i]*sizeof(float), cudaMemcpyDeviceToDevice) != cudaSuccess)
            cout << "Synaptic weights copy error" << endl;

        index += layers_synaptic_weights_numbers[i];

        if(cudaMemcpy(cuda_layers_biases[i], parameters + index, layers_biases_numbers[i]*sizeof(float), cudaMemcpyDeviceToDevice) != cudaSuccess)
            cout << "Biases copy error" << endl;
...
void StochasticGradientDescent::cuda_update_parameters(const LossIndex::CudaFirstOrderLoss& cuda_first_order_loss, StochasticGradientDescent::CudaParameters& cuda_parameters) const
{
    // Neural network

    MultilayerPerceptron* multilayer_perceptron_pointer = loss_index_pointer->get_neural_network_pointer()->get_multilayer_perceptron_pointer();

    const size_t parameters_number = multilayer_perceptron_pointer->get_parameters_number();

    const Vector<size_t> layers_neurons_number = multilayer_perceptron_pointer->get_layers_neurons_numbers();
    const Vector<size_t> layers_inputs_number = multilayer_perceptron_pointer->get_layers_inputs_number();

    const Vector<float*> layers_biases_cuda = multilayer_perceptron_pointer->get_layers_biases_cuda();
    const Vector<float*> layers_synaptic_weights_cuda = multilayer_perceptron_pointer->get_layers_synaptic_weights_cuda();

//    const size_t learning_rate_iteration = 1;

//    const float learning_rate =  initial_learning_rate / (1.0 + learning_rate_iteration*initial_decay);

    // Training strategy

    cuda_parameters.get_parameters();

//    float* parameters_increment;

//    cudaMalloc(&parameters_increment, parameters_number*sizeof(float));

//    cudaMemcpy(parameters_increment, cuda_first_order_loss.gradient, parameters_number*sizeof(float), cudaMemcpyDeviceToDevice);

//    float alpha = static_cast<float>(-0.001);

//    cublasSscal(handle, static_cast<int>(parameters_number), &alpha, parameters_increment, 1);

//    Vector<float> parameters_increment_host;
//    parameters_increment_host.from_device(parameters_increment, parameters_number);

//    cout << parameters_increment_host << endl;

//    const float momentum_cuda = static_cast<float>(momentum);

//    float* last_increment;

    if(momentum == 0.0)
    {
        float alpha = static_cast<float>(-initial_learning_rate);

        cublasSaxpy(handle, static_cast<int>(parameters_number),
                    &alpha,
                    cuda_first_order_loss.gradient, 1,
                    cuda_parameters.parameters, 1);
    }
    else if(!nesterov)
    {
    }
    ...
    cuda_parameters.set_parameters();

    if(momentum > 0.0 && !nesterov)
    {
//        cublasSaxpy(handle, static_cast<int>(parameters_number),
//                    &momentum_cuda, last_increment, 1,
//                    parameters_increment, 1);

//        cudaMemcpy(last_increment, parameters_increment, parameters_number*sizeof(float), cudaMemcpyDeviceToDevice);
    }
//    else if(momentum > 0.0 && nesterov)
//    {
//        cublasSaxpy(handle, static_cast<int>(parameters_number), &momentum_cuda, last_increment, 1, parameters_increment, 1);

//        cudaMemcpy(last_increment, parameters_increment, parameters_number*sizeof(float), cudaMemcpyDeviceToDevice);

//        cublasSscal(handle, static_cast<int>(parameters_number), &momentum_cuda, parameters_increment, 1);

//        alpha = -learning_rate;

//        cublasSaxpy(handle, static_cast<int>(parameters_number), &alpha, cuda_first_order_loss.gradient, 1, parameters_increment, 1);
//    }

}


OptimizationAlgorithm::Results StochasticGradientDescent::cuda_perform_training()
{
    // Check CUDA
   ...
   DataSet::CudaBatch cuda_batch(data_set_pointer);
   ...
   multilayer_perceptron_pointer->allocate();
   multilayer_perceptron_pointer->copy();

   const Vector<size_t> architecture = multilayer_perceptron_pointer->get_architecture();

   const size_t parameters_number = neural_network_pointer->get_parameters_number();

   MultilayerPerceptron::CudaForwardPropagation cuda_forward_propagation(multilayer_perceptron_pointer, batch_size);

   // Loss index stuff

   MeanSquaredError* mean_squared_error_pointer = new MeanSquaredError(neural_network_pointer, data_set_pointer);

   LossIndex::CudaFirstOrderLoss cuda_first_order_loss(mean_squared_error_pointer);

   // Optimization algorithm stuff

   CudaParameters cuda_parameters(this);
   ...
   for(size_t epoch = 0; epoch <= epochs_number; epoch++)
   {
       ...
       for(size_t iteration = 0; iteration < batches_number; iteration++)
       {          
           cuda_batch.copy_inputs_host(data, training_batches[iteration], inputs_indices);

           if(cudaMemcpy(cuda_batch.inputs, cuda_batch.inputs_host, batch_size*inputs_number*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess)
               cout << "Inputs copy error" << endl;

           cuda_batch.copy_targets_host(data, training_batches[iteration], targets_indices);

           if(cudaMemcpy(cuda_batch.targets, cuda_batch.targets_host, batch_size*targets_number*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess)
               cout << "Targets copy error" << endl;

           multilayer_perceptron_pointer->cuda_calculate_forward_propagation(cuda_batch, cuda_forward_propagation);

           mean_squared_error_pointer->cuda_calculate_first_order_loss(cuda_batch, cuda_forward_propagation, cuda_first_order_loss);

           cuda_update_parameters(cuda_first_order_loss, cuda_parameters);
       }
       ...
       elapsed_time = difftime(current_time, beginning_time);
        ...
        cout << "Error: " << cuda_first_order_loss.loss << endl;
