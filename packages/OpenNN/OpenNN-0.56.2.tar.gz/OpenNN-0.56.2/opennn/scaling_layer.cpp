...
ScalingLayer::ScalingLayer(const size_t& new_scaling_neurons_number) : Layer()
{
...
ScalingLayer::ScalingLayer(const Vector< Statistics<double> >& new_statistics) : Layer()
{
    set(new_statistics);
}
...
{
    return get_scaling_neurons_number();
}
...
void ScalingLayer::set(const ScalingLayer& new_scaling_layer)
{
    ...
    trainable = new_scaling_layer.trainable;

    layer_type = Scaling_Layer;
}
...
void ScalingLayer::prune_scaling_neuron(const size_t& index)
{
    ...
    statistics.erase(statistics.begin() + static_cast<long long>(index));
}
