...
void PerceptronLayer::set_input_shape(const Vector<size_t>& size)
{
    if(size.empty() || size.size() > 1)
    {
//        throw exception("EXCEPTION: The new size is incompatible.");
    }
...
Matrix<double> PerceptronLayer::calculate_activations(const Matrix<double>& combinations) const
{
    ...
    switch(activation_function)
    {
        ...
        {
             return Functions::linear(combinations);
        }

        case PerceptronLayer::Logistic:
        {
             return Functions::logistic(combinations);
        }

        case PerceptronLayer::HyperbolicTangent:
        {
             return Functions::hyperbolic_tangent(combinations);
        }

        case PerceptronLayer::Threshold:
        {
             return Functions::threshold(combinations);
        }

        case PerceptronLayer::SymmetricThreshold:
        {
             return Functions::symmetric_threshold(combinations);
        }

        case PerceptronLayer::RectifiedLinear:
        {
             return Functions::rectified_linear(combinations);
        }

        case PerceptronLayer::ScaledExponentialLinear:
        {
             return Functions::scaled_exponential_linear(combinations);
        }

        case PerceptronLayer::SoftPlus:
        {
             return Functions::soft_plus(combinations);
        }

        case PerceptronLayer::SoftSign:
        {
             return Functions::soft_sign(combinations);
        }

        case PerceptronLayer::HardSigmoid:
        {
             return Functions::hard_sigmoid(combinations);
        }

        case PerceptronLayer::ExponentialLinear:
        {
             return Functions::exponential_linear(combinations);
        }
    }
...
Tensor<double> PerceptronLayer::calculate_activations_derivatives(const Matrix<double>& combinations) const
{
    // Control sentence(if debug)
...
*/
        case PerceptronLayer::HyperbolicTangent:
        {
             ...
             return Functions::tensor_hyperbolic_tangent_derivatives(combinations);
        }
/*
        case PerceptronLayer::Threshold:
        {
...
/*
    ...
    default:
        ...
        buffer << "OpenNN Exception: Preceptron Layer class.\n"
...
Matrix<double> PerceptronLayer::calculate_activations_derivatives_matrix(const Matrix<double>& combinations) const
{
    // Control sentence(if debug)
    ...
    switch(activation_function)
    {
        case PerceptronLayer::Linear:
        {
             return Functions::linear_derivatives(combinations);
        }

        case PerceptronLayer::Logistic:
        {
             return Functions::logistic_derivatives(combinations);
        }

        case PerceptronLayer::HyperbolicTangent:
        {

             return Functions::hyperbolic_tangent_derivatives(combinations);
        }

        case PerceptronLayer::Threshold:
        {
             return Functions::threshold_derivatives(combinations);
        }

        case PerceptronLayer::SymmetricThreshold:
        {
             return Functions::symmetric_threshold_derivatives(combinations);
        }

        case PerceptronLayer::RectifiedLinear:
        {
             return Functions::rectified_linear_derivatives(combinations);
        }

        case PerceptronLayer::ScaledExponentialLinear:
        {
             return Functions::scaled_exponential_linear_derivatives(combinations);
        }

        case PerceptronLayer::SoftPlus:
        {
             return Functions::soft_plus_derivatives(combinations);
        }

        case PerceptronLayer::SoftSign:
        {
             return Functions::soft_sign_derivatives(combinations);
        }

        case PerceptronLayer::HardSigmoid:
        {
             return Functions::hard_sigmoid_derivatives(combinations);
        }

        case PerceptronLayer::ExponentialLinear:
        {
             return Functions::exponential_linear_derivatives(combinations);
        }
...
float* PerceptronLayer::get_biases_device() const
{
    return biases_device;
}


float* PerceptronLayer::get_synaptic_weights_device() const
{
    return synaptic_weights_device;
}


void PerceptronLayer::allocate()
{
    const size_t perceptrons_number = get_perceptrons_number();
    const size_t inputs_number = get_inputs_number();

    if(cudaMalloc(&biases_device, perceptrons_number*sizeof(float)) != cudaSuccess)
        cout << "Biases allocation error" << endl;

    if(cudaMalloc(&synaptic_weights_device, inputs_number*perceptrons_number*sizeof(float)) != cudaSuccess)
        cout << "Synaptic weights allocation error" << endl;
}


void PerceptronLayer::copy()
{
    if(biases_device == nullptr) cout << "Biases is null" << endl;
    if(synaptic_weights_device == nullptr) cout << "Synaptic weights is null" << endl;

    const Vector<float> biases_float = biases.to_float_vector();
    const Matrix<float> synaptic_weights_float = synaptic_weights.to_float_matrix();

    if(cudaMemcpy(biases_device, biases_float.data(), biases_float.size()*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess)
        cout << "Biases copy error" << endl;

    if(cudaMemcpy(synaptic_weights_device, synaptic_weights_float.data(), synaptic_weights_float.size()*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess)
        cout << "Synaptic weights copy error" << endl;
}


void PerceptronLayer::free()
{
    cudaFree(biases_device);

    cudaFree(synaptic_weights_device);

    biases_device = nullptr;

    synaptic_weights_device = nullptr;
}
...
void PerceptronLayer::print() const
{
    ...
    cout << "Biases device:" << endl;

    Vector<float> biases_host;
    biases_host.from_device(biases_device, neurons_number);

    cout << biases_host << endl;

    Matrix<float> synaptic_weights_host;
    synaptic_weights_host.from_device(biases_device, inputs_number, neurons_number);

    cout << "Synaptic weights device:" << endl;

    cout << synaptic_weights_host << endl;
}


void PerceptronLayer::cuda_calculate_combinations(const size_t& batch_size,
                                                  const float* inputs_device,
                                                  const float* ones,
                                                  float* combinations_device) const
{

//    cublasCreate(&handle);

    const int inputs_number = static_cast<int>(get_inputs_number());
    const int neurons_number = static_cast<int>(get_neurons_number());

    float alpha = 1.0;
    float beta = 1.0;

    cudaMemset(combinations_device, 0, static_cast<size_t>(batch_size*neurons_number)*sizeof(float));

    // Create bias matrix

    cublasSger(handle,
               batch_size, neurons_number,
               &alpha, ones, 1,
               biases_device, 1,
               combinations_device, batch_size);

    // Dot product

    cublasSgemm(handle,
                CUBLAS_OP_N, CUBLAS_OP_N,
                batch_size, neurons_number, inputs_number,
                &alpha, inputs_device, batch_size,
                synaptic_weights_device, inputs_number,
                &beta, combinations_device, batch_size);

//    cublasDestroy(handle);
}


void PerceptronLayer::cuda_calculate_activations(const size_t& batch_size, const float* combinations, float* activations) const
{
    const size_t neurons_number = get_neurons_number();

    const size_t size = batch_size*neurons_number;

    switch(activation_function)
    {
        case PerceptronLayer::Linear: linear(size, combinations, activations); return;

        case PerceptronLayer::Logistic: logistic(size, combinations, activations); return;

        case PerceptronLayer::HyperbolicTangent: hyperbolic_tangent(size, combinations, activations); return;

        case PerceptronLayer::Threshold: threshold(size, combinations, activations); return;

        case PerceptronLayer::SymmetricThreshold: symmetric_threshold(size, combinations, activations); return;

        case PerceptronLayer::RectifiedLinear: rectified_linear(size, combinations, activations); return;

        case PerceptronLayer::ScaledExponentialLinear: scaled_exponential_linear(size, combinations, activations); return;

        case PerceptronLayer::SoftPlus: soft_plus(size, combinations, activations); return;

        case PerceptronLayer::SoftSign: soft_sign(size, combinations, activations); return;

        //case PerceptronLayer::HardSigmoid: hard_sigmoid(combinations, activations); return;

        case PerceptronLayer::ExponentialLinear: exponential_linear(size, combinations, activations); return;
    }
}


void PerceptronLayer::cuda_calculate_activations_derivatives(const size_t& batch_size, const float* combinations, float* activations_derivatives) const
{
    const size_t neurons_number = get_neurons_number();
    ...
    switch(activation_function)
    {
        case PerceptronLayer::Linear: linear_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::Logistic: logistic_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::HyperbolicTangent: hyperbolic_tangent_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::Threshold: threshold_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::SymmetricThreshold: symmetric_threshold_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::RectifiedLinear: rectified_linear_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::ScaledExponentialLinear: scaled_exponential_linear_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::SoftPlus: soft_plus_derivative(size, combinations, activations_derivatives); return;

        case PerceptronLayer::SoftSign: soft_sign_derivative(size, combinations, activations_derivatives); return;

        //case PerceptronLayer::HardSigmoid: hard_sigmoid(size, combinations, activations_derivatives); return;

        case PerceptronLayer::ExponentialLinear: exponential_linear_derivative(size, combinations, activations_derivatives); return;
    }
}
