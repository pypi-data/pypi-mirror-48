...
double CorrelationAnalysis::calculate_logistic_correlation(const Matrix<double>& x, const Vector<double>& y)
{
    ...
    //training_strategy.get_quasi_Newton_method_pointer()->set_batch_size(data_set.get_instances().get_training_instances_number());
    ...
    const double regression_parameter = neural_network.get_multilayer_perceptron_pointer()->get_layers_synaptic_weights()[0](0,0);

    if (regression_parameter < 0.0)
    {
...
Vector<double> CorrelationAnalysis::calculate_logistic_regression(const Matrix<double>& x, const Vector<double>& y, const double& tolerance)
{
   ...
   if(y.size() != x.size()) {
    ...
    while(change_rate.calculate_maximum() > tolerance)
    {
        ...
        {
            const double factor = Products::dot(old_parameters, x_matrix.get_row(i));
        ...
        new_parameters = Products::dot(x_matrix, old_parameters);
        new_parameters = Products::dot(s_matrix, new_parameters);
        new_parameters = new_parameters + y - mu_vector;
        new_parameters = Products::dot(x_matrix.calculate_transpose(), new_parameters);
...
double CorrelationAnalysis::calculate_multiple_linear_correlation(const Matrix<double>& x, const Vector<double>& y)
{
    ...
    const Vector<double> other_approximation = Products::dot(x, multiple_linear_regression_parameters);
...
//    probabilistic_layer->set_probabilistic_method(ProbabilisticLayer::Threshold);
