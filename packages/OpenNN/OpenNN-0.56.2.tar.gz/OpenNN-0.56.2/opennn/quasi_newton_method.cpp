...
{
   return((Products::dot(inverse_Hessian_approximation, gradient)*(-1.0)).calculate_normalized());
}
...
const Vector<double>& old_parameters, const Vector<double>& parameters, const Vector<double>& old_gradient, const Vector<double>& gradient, const Matrix<double>& old_inverse_Hessian) const
{
   ...
   const double parameters_dot_gradient = Products::dot(parameters_difference, gradient_difference);
   ...
   else if(fabs(Products::dot(Products::dot(gradient_difference, old_inverse_Hessian), gradient_difference)) < 1.0e-50)
   {
      buffer << "OpenNN Exception: QuasiNewtonMethod class.\n"
             << "Matrix<double> calculate_DFP_inverse_Hessian(const Vector<double>&, const Vector<double>&, const Vector<double>&, const Vector<double>&, const Matrix<double>&) method.\n"
   ...
   const Vector<double> Hessian_dot_gradient_difference = Products::dot(old_inverse_Hessian, gradient_difference);
   ...
   inverse_Hessian_approximation -= (Hessian_dot_gradient_difference).direct(Hessian_dot_gradient_difference)
            /Products::dot(gradient_difference, Hessian_dot_gradient_difference);
...
//   }
   ...
   const double parameters_dot_gradient = Products::dot(parameters_difference, gradient_difference);
   const Vector<double> Hessian_dot_gradient = Products::dot(old_inverse_Hessian, gradient_difference);
   const double gradient_dot_Hessian_dot_gradient = Products::dot(gradient_difference, Hessian_dot_gradient);
...
double* old_inverse_Hessian_d, double* auxiliar_matrix_d) const
{
    ...
    dfpInverseHessian(old_parameters_d, parameters_d, old_gradient_d, gradient_d, old_inverse_Hessian_d, auxiliar_matrix_d,
                      inverse_Hessian_approximation_data, static_cast<int>(parameters_number));
...
double* old_inverse_Hessian_d, double* auxiliar_matrix_d) const
{
    ...
    bfgsInverseHessian(old_parameters_d, parameters_d, old_gradient_d, gradient_d, old_inverse_Hessian_d, auxiliar_matrix_d,
                       inverse_Hessian_approximation_data, static_cast<int>(parameters_number));
...
OptimizationAlgorithm::Results QuasiNewtonMethod::perform_training()
{
   ...
   for(size_t epoch = 0; epoch <= maximum_epochs_number; epoch++)
   {
       ...
       training_slope = Products::dot(gradient/gradient_norm, training_direction);
