...
double MinkowskiError::calculate_training_error_cpu() const
{
...
double MinkowskiError::calculate_selection_error_cpu() const
{
#ifdef __OPENNN_DEBUG__
...
double MinkowskiError::calculate_training_error_cpu(const Vector<double>& parameters) const
{
#ifdef __OPENNN_DEBUG__
...
Vector<double> MinkowskiError::calculate_training_error_gradient_cpu() const
{
#ifdef __OPENNN_DEBUG__
...
double MinkowskiError::calculate_error(const Vector<double>& parameters) const
{
   ...
   for(int i = 0; i < static_cast<int>(training_instances_number); i++)
   {
      ...
      const Matrix<double> inputs = data_set_pointer->get_instance(training_index, inputs_indices).to_row_matrix();
