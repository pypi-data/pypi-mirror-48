...
double WeightedSquaredError::calculate_training_error_cpu() const
{
    // Control sentence
    ...
    const Vector< Vector<size_t> > batch_indices = data_set_pointer->get_instances_pointer()->get_training_batches();
...
double WeightedSquaredError::calculate_training_error_cuda() const
{
    double training_error = 0.0;
...
double WeightedSquaredError::calculate_selection_error_cpu() const
{
    // Control sentence
    ...
    const Vector< Vector<size_t> > batch_indices = data_set_pointer->get_instances_pointer()->get_selection_batches();
...
double WeightedSquaredError::calculate_selection_error_cuda() const
{
    double selection_error = 0.0;
    ...
    loss_parameters[0] = positives_weight;
    loss_parameters[1] = negatives_weight;
    loss_parameters[2] = selection_normalization_coefficient;
    ...
    for(size_t i = 0; i < batches_number; i++)
    {
        ...
        Matrix<double> targets = data_set_pointer->get_targets(selection_batches[static_cast<unsigned>(i)]);
        const double* target_data = targets.data();
        const size_t target_rows = targets.get_rows_number();
        const size_t target_columns = targets.get_columns_number();
...
double WeightedSquaredError::calculate_training_error_cpu(const Vector<double>& parameters) const
{
    // Control sentence
...
double WeightedSquaredError::calculate_training_error_cuda(const Vector<double>& parameters) const
{
    double training_error = 0.0;
#ifdef __OPENNN_CUDA__
    const Vector<size_t> architecture = neural_network_pointer->get_multilayer_perceptron_pointer()->get_architecture();
...
Vector<double> WeightedSquaredError::calculate_training_error_gradient_cpu() const
{
#ifdef __OPENNN_DEBUG__
...
Vector<double> WeightedSquaredError::calculate_training_error_gradient_cuda() const
{
...
Matrix<double> WeightedSquaredError::calculate_error_terms_Jacobian() const
{
    ...
    Vector< Vector< Vector<double> > > forward_propagation(2);
    ...
    #pragma omp parallel for private(forward_propagation,  \
    ...
    for(int i = 0; i < static_cast<int>(training_instances_number); i++)
    {
        ...
        forward_propagation = multilayer_perceptron_pointer->calculate_forward_propagation(inputs);

        const Vector< Vector<double> >& layers_activation = forward_propagation[0];
        const Vector< Vector<double> >& layers_activation_derivative = forward_propagation[1];

        Vector<double> term;
        double term_norm;

            const Vector<double>& outputs = forward_propagation[0][layers_number-1];
