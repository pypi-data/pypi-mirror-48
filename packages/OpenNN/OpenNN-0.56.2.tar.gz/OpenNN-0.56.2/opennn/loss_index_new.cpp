...
Vector< Matrix<double> > LossIndex::calculate_layers_delta(const Vector< Tensor<double> >& layers_activation_derivative,
...
Vector< Matrix<double> > LossIndex::calculate_layers_delta(const Vector<Matrix<double>>& layers_activation, const Vector< Tensor<double> >& layers_activation_derivative, const Matrix<double>& output_gradient) const
{
    //const MultilayerPerceptron* multilayer_perceptron_pointer = neural_network_pointer->get_multilayer_perceptron_pointer();
...
*/
   ...
   if(layers_number > 0)
   {
       if( layers_activation_derivative[layers_number - 1].get_order() == 2 )
       {
       ...
       {
           layers_delta[layers_number - 1] = output_gradient.dot(layers_activation_derivative[layers_number - 1]);
       }
      ...
      for(int i = static_cast<int>(layers_number) - 2; i >= 0; i--)
      {
          ...
          else
          {
              ...
              layer_synaptic_weights_transpose = synaptic_weights.calculate_transpose();             
              layers_delta[static_cast<size_t>(i)] = layers_activation_derivative[static_cast<size_t>(i)].get_matrix(0) * (layers_delta[static_cast<size_t>(i+1)].dot(layer_synaptic_weights_transpose));
          }
      }
   }
   return layers_delta;
 ...
 const Vector< Matrix<double> >& layers_delta) const
{
   ...
   const Vector<size_t> layers_parameters_number = neural_network_pointer -> get_layers_parameters_number();

   Vector<double> error_gradient(parameters_number);

   Layer* layer_pointer = neural_network_pointer -> get_layer_pointer(0);

   size_t index = 0;


   error_gradient.tuck_in(index, calculate_layer_error_gradient(layers_delta[0], inputs, layer_pointer));
   ...
   for(size_t i = 1; i < layers_number; i++)
   {
      ...
      layer_pointer = neural_network_pointer -> get_layer_pointer(i);

      error_gradient.tuck_in(index, calculate_layer_error_gradient(layers_delta[i], layers_activations[i-1], layer_pointer));
...
Vector<double> LossIndex::calculate_perceptron_layer_error_gradient(const Matrix<double>& layer_deltas, const Matrix<double>& layer_inputs) const
{
...
Vector<double> LossIndex::calculate_convolutional_layer_error_gradient(const ConvolutionalLayer* convolutional_layer, const Matrix<double>& layer_deltas, const Matrix<double>& layer_inputs) const
{
    const size_t neurons_number = layer_deltas.get_columns_number();
    const size_t samples_number = layer_inputs.get_rows_number();

    const size_t parameters_number = convolutional_layer -> get_parameters_number();

    const size_t biases_number  = convolutional_layer -> get_biases().size();
    const size_t weights_number = (parameters_number - biases_number);
    ...
    for(size_t sample = 0; sample < samples_number; sample ++)
    {
        ...
        {
            const double layer_delta = layer_deltas(sample, neuron);

            const Vector<size_t> inputs_for_neuron = convolutional_layer -> get_inputs_indices(neuron);

            for(size_t weight = 0; weight < weights_number; weight++)
            {
                    layer_error_gradient[weight] += layer_delta * layer_inputs(sample, inputs_for_neuron[weight]);
            }

            // Biases.
            // Checked: OK
            for( size_t bias = 0; bias < biases_number; bias++ )
            {
                layer_error_gradient[ parameters_number - biases_number + bias ] += layer_delta;
            }
        }
    }
...
Vector<double> LossIndex::calculate_pooling_layer_error_gradient(const PoolingLayer* pooling_layer, const Matrix<double>& layers_delta, const Matrix<double>& layer_inputs) const
{

    const size_t inputs_number = layer_inputs.get_columns_number();

    Vector<double> layer_error_gradient(inputs_number, 0.0);
...
Vector<double> LossIndex::calculate_layer_error_gradient(const Matrix<double>& layer_deltas, const Matrix<double>& layer_inputs, const Layer* layer) const
{
    const string layer_type = layer -> get_layer_tag();

    if(!layer_type.compare("PerceptronLayer") || layer -> is_trainable() )
    {
        return calculate_perceptron_layer_error_gradient(layer_deltas, layer_inputs);
    }
    else if(!layer_type.compare("ConvolutionalLayer"))
    {
        return calculate_convolutional_layer_error_gradient((ConvolutionalLayer*) layer, layer_deltas, layer_inputs);

    } else if(!layer_type.compare("PoolingLayer"))
    {
        return calculate_pooling_layer_error_gradient((PoolingLayer*) layer, layer_deltas, layer_inputs);
    }

    return Vector<double>(layer_inputs.get_columns_number(), 0.0);
}
