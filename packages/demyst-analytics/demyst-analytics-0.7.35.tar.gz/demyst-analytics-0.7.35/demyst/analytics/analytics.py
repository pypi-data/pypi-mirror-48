...
class Analytics(object):
    ...
    def product_stats(self, provider_names=[], all_products=False):
        return track_function(lambda: self.do_product_stats(provider_names, all_products),
                              self.C, "API : Python : Hogfish Stats",
                              { "provider_names": provider_names,
                                "all_products": all_products })

    def do_product_stats(self, provider_names, all_products):
        res = []
        if all_products:
        ...
        df_res.rename(columns={'consistency_rate':'attribute_consistency_rate','entity_name':'input_entity','error_rate':'product_error_rate','field_is_populated_rate':'attribute_fill_rate','flattened_name':'attribute_flattened_name','generic_flattened_name':'attribute_generic_name','hit_rate':'product_match_rate','num_distinct_values':'attribute_unique_values','full_field_name':'attribute_full_name'}, inplace =True)
        return(df_res)
    ...
    def product_catalog(self, provider_names=[], all_products=False):
    ...
    def product_inputs(self, provider_names=[], all_products=False):
        if all_products:
            provider_names = self.C.all_provider_names()
        result = []
        for provider_name in provider_names:
            catalog = self.connectors.product_catalog(provider_name)
            if catalog:
                for i, c in enumerate(catalog):
                    self.product_inputs_for_single_catalog(provider_name, c, i, result)
        return self.shape_dataframe(pd.DataFrame(result))

    def product_inputs_for_single_catalog(self, provider_name, c, i, result):
        for option_number, req_inputs in enumerate(c["input"]["required"]):
            option = "option" + str(i + 1) + "/required_input" + str(option_number + 1)
            self.collect_inputs(provider_name, req_inputs, option, result, "")
        if "optional" in c["input"]:
            option = "optional_input"
            self.collect_inputs(provider_name, c["input"]["optional"], option, result, "")

    def collect_inputs(self, provider_name, req_inputs, option, result, name_prefix):
        for input in req_inputs:
            ...
            result.append(self.make_row(provider_name, name, option, type, description))
            if (type == "Dictionary"):
                self.collect_inputs(input["children"], option, result, name + "/")
    ...
    def product_outputs(self, provider_names=[], all_products=False):
        ...
        for provider_name in provider_names:
            ...
            if catalog:
                for c in catalog:
                    self.collect_outputs(provider_name, c["output"], result, "")
        return self.shape_dataframe(pd.DataFrame(result))

    def collect_outputs(self, provider_name, outputs, result, name_prefix):
        for o in outputs:
            name = name_prefix + (o.get("name") or "unknown")
            type = o.get("type") or "unknown"
            description = o.get("description") or ""
            result.append(self.make_row(provider_name, name, "output", type, description))
            if (type == "Dictionary"):
                self.collect_outputs(provider_name, o["children"], result, name + "/")

    def make_row(self, provider_name, attribute, option, type, description):
    ...
    def sample_enrich(self, csv_path_or_df):
        ...
        display(self.search(typed_df))
        providers = self.search(typed_df, notebook=False)
        provider_names = [p["name"] for p in providers]
        headline("Running sample enrichment...")
        return self.enrich_and_download(provider_names, typed_df.head(100), validate=False)
