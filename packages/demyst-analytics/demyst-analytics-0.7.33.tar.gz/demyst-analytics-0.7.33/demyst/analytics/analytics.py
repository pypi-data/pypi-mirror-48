...
class Analytics(object):
    ...
    def product_catalog(self, provider_name, dataframe=False):
        if dataframe:
            inputs = self.product_inputs(provider_name)
            outputs = self.product_outputs(provider_name)
            return pd.concat([inputs, outputs])
        else:
    ...
    def product_inputs(self, provider_name):
    ...
    def collect_inputs(self, provider_name, req_inputs, option, result, name_prefix):
        for input in req_inputs:
            ...
            if (type == "Dictionary"):
                result.append({ "option": option, "provider_name": provider_name,
                                "attribute": name, "type": type, "description": description })
        ...
        for o in outputs:
            ...
            if (type == "Dictionary"):
                result.append({ "attribute": name, "type": type, "description": description,
                                "provider_name": provider_name, "option": "output" })
