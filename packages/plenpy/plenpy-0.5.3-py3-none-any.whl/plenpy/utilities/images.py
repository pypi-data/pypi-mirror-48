...
from skimage.measure import compare_mse as sk_mse
from skimage.measure import compare_nrmse as sk_nrmse
from skimage.measure import compare_psnr as sk_psnr
...
def rgb_to_grey(img: ndarray, weighted: bool = True) -> ndarray:
    """Convert a RGB image to grayscale.
    Result will be of type float in range (0, 1).
    Output shape will be (x, y).

    See Also:
         :func:`plenpy.utilities.core.SpectralArray.get_grey()`
    """
    logger.warn(DeprecationWarning("The function rgb_to_grey is deprecated. Use skimage.color.rgb2gray instead."))
    return np.squeeze(np.asarray(core.SpectralArray(img).get_grey(weighted=weighted)))


# Synonym
rgb_to_gray = rgb_to_grey
...
def overlay_images(img1: ndarray,
    ...
    grayscale using :func:`.rgb_to_gray()`.
    ...
    image1 = rgb_to_gray(img1)
    image2 = rgb_to_gray(img2)
