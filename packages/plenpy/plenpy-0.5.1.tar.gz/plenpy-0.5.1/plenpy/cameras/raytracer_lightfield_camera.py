...
class RayTracerLF(AbstractLightFieldCamera):
    ...
    def calibrate(self,
        ...
        if filename is not None:
            # TODO: What about absolute paths?
            filename = os.path.join(self.calibrationFolderPath, filename)
            self._calDataFilename = filename + ".npz"
    ...
    def _get_wi_db_entry(self, path: str) -> ndarray:
        ...
        db_entry = np.array(
            ...
            dtype=[('path', 'U999'), ('focal_length', 'f8'), ('focus_distance', 'f8')])
    ...
    def _get_wi(self, path: str, process: bool = True):
        ...
        """
        wi = imageio.imread(path, format=self._format)
