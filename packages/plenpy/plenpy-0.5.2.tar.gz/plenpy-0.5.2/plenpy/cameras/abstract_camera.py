...
class AbstractCamera(ABC):
    ...
    Attributes:
        cameraFolderPath (str): System path the main camera folder.

        imageFolderPath (str): System path to image folder.

        calibrationFolderPath (str): System path to calibration folder.

        referenceFolderPath (str): System path to reference folder.
    ...
    def __init__(self, camera_folder_path: str):
        """:class:`AbstractCamera` base class initialization.

        Args:
            camera_folder_path: System path to the main camera folder.

        """

        # Set the camera folder path as absolute path and add trailing "/"
        # if necessary.
        self.cameraFolderPath: str = os.path.join(
            os.path.abspath(camera_folder_path), '')
        logger.info(f"Initializing camera at {self.cameraFolderPath}.")

        # Check that necessary camera folder exists
        if not os.path.isdir(self.cameraFolderPath):
            raise ValueError(f"Camera folder {self.cameraFolderPath} does "
                             "not exist. "
                             "Please create it before initializing a camera.")

        # Set image, calibration and reference folder (absolute) paths
        self.imageFolderPath: str = os.path.join(self.cameraFolderPath,
                                                 "Images/")
        self.calibrationFolderPath: str = os.path.join(self.cameraFolderPath,
                                                       "Calibration/")
        self.referenceFolderPath: str = os.path.join(self.cameraFolderPath,
                                                     "Reference/")
        # Check that necessary folders Images/ and Calibration/ exist.
        if not os.path.isdir(self.imageFolderPath):
            raise ValueError("'Images' subdirectory does not exist. "
                             "Please create it before initializing a camera.")

        if not os.path.isdir(self.calibrationFolderPath):
        ...
        logger.info(f"Reading sensor image paths from {self.imageFolderPath}.")
        ...
        for extension in get_avail_extensions():
            for file in Path(self.imageFolderPath).glob('**/*' + extension):
                files_found.append(file)

        # Sort outcome and copy
        files_found = sorted(files_found)
        for i in range(len(files_found)):
            self._sensorImages[i] = SensorImage(os.path.join(self.imageFolderPath, files_found[i]))
    ...
    def load_sensor_image_from_path(self,
        ...
        self._sensorImages[num] = SensorImage(os.path.abspath(path))
