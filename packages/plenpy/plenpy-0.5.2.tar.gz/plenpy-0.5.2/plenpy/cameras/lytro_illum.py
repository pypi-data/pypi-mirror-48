...
class LytroIllum(AbstractLightFieldCamera):
    ...
    def __init__(self, camera_folder_path: str, format='LYTRO-ILLUM-RAW'):
        """
        Args:
            camera_folder_path: Folder path of camera.
            format: The ``imageio`` format of the white images.
                    Default: LYTRO-ILLUM-RAW.
        """
        # Call init from AbstractLightfieldCamera base class
        super().__init__(camera_folder_path,
                         microlens_size=14,
                         grid_type='hex',
    ...
    def calibrate(self,
        ...
        The calibration estimates the microlens centers from the ideal
        white image. Based on the chosen method, this will yield ml centers
        with (``det_method='own'``) or without (``det_method='dans'``)
        subpixel precision.

        Using the estimated ml centers, an ideal grid is estimated that best
        approximates the ML centers. This can either be done using the
        Fourier transform of the ideal white image (``est_method='fourier'``),
        in the spatial domain (``est_method='spatial'``) or using a combined
        method.
        ...
        if filename is not None:
            filename = os.path.join(self.calibrationFolderPath, filename)
            if not filename.endswith(".npz"):
                filename = filename + ".npz"
            self._calDataFilename = filename

        # If a calibration file is found, load it
        if os.path.isfile(self._calDataFilename) and not force:
            logger.info(
    ...
    def _get_wi_db_entry(self, path: str) -> ndarray:
        ...
        Args:
            path: Path to the white image, used as database identfication
    ...
    def _get_wi(self, path: str, process: bool = True):
        ...
        """
        wi = imageio.imread(path, format=self._format).copy()
