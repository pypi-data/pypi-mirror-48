...
from .input_utility import create_Karka_iterator, get_dimensions_from_Karka_dataset
from .loss_graph import AbstractLossGraph, RMSELossGraph
from .prediction_graph import AbstractPredictionGraph, DotProductPredictionGraph
from .rec_graph import (
...
from .rep_graph import AbstractRepresentationGraph, LinearRepresentationGraph
from .sessions import get_session
from .utility import sample_items, calculate_batched_alpha, datasets_from_raw_input
...
class Karka(object):
    ...
    def __init__(self,
        ...
        See Karka.representation_graphs for examples.
        ...
        user/item reprs.
        See Karka.prediction_graphs for examples.
        ...
        See Karka.loss_graphs for examples.
        :param biased: bool
        ...
        if (n_components is None) or (n_tastes is None) or (user_repr_graph is None) or (item_repr_graph is None) \
            ...
            raise ValueError("All arguments to Karka() must be non-None")
        if n_components < 1:
        ...
        # Calling the break routine during __init__ creates all the attrs on the Karka object with an initial value
        # of None
    ...
    def _build_input_iterators(self):
        self.tf_user_feature_iterator = create_Karka_iterator(name='tf_user_feature_iterator')
        self.tf_item_feature_iterator = create_Karka_iterator(name='tf_item_feature_iterator')
        self.tf_interaction_iterator = create_Karka_iterator(name='tf_interaction_iterator')
    ...
    def fit(self, interactions, user_features, item_features, epochs=100, learning_rate=0.1, alpha=0.00001,
        ...
        """
        Constructs the Karka graph and fits the model.
        ...
        If a Dataset, the Dataset must follow the format used in Karka.input_utils.
    ...
    def fit_partial(self, interactions, user_features, item_features, epochs=1, learning_rate=0.1,
        ...
        if self.tf_prediction is None:
            ...
            _, n_user_features = get_dimensions_from_Karka_dataset(first_batch[1])
            _, n_item_features = get_dimensions_from_Karka_dataset(first_batch[2])

            # Numbers of features are either learned at fit time from the shape of these two matrices or specified at
            # Karka construction and cannot be changed.
    ...
    def save_model(self, directory_path):
        ...
        session_path = os.path.join(directory_path, 'Karka_session.cpkt')
        saver.save(sess=get_session(), save_path=session_path)

        # Break connections to the graph before saving the python object
        self._break_graph_hooks()
        Karka_path = os.path.join(directory_path, 'Karka.pkl')
        with open(Karka_path, 'wb') as file:
    ...
    def load_model(cls, directory_path):
        """
        Loads the Karka model and TensorFlow session saved in the given directory.
        :param directory_path: str
        The path to the directory containing the saved model.
        :return:
        """

        graph_path = os.path.join(directory_path, 'Karka_session.cpkt.meta')
        ...
        Karka_path = os.path.join(directory_path, 'Karka.pkl')
        with open(Karka_path, 'rb') as file:
