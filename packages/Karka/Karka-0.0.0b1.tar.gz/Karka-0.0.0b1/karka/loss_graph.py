...
class SeparationLossGraph(AbstractLossGraph):
    ...
    def connect_loss_graph(self, tf_prediction_serial, tf_interactions_serial, **kwargs):
        ...
        tf_positive_predictions = tf.boolean_mask(
            tf_prediction_serial, tf_positive_mask)
        tf_negative_predictions = tf.boolean_mask(
            tf_prediction_serial, tf_negative_mask)

        tf_pos_mean, tf_pos_var = tf.nn.moments(
            tf_positive_predictions, axes=[0])
        tf_neg_mean, tf_neg_var = tf.nn.moments(
            tf_negative_predictions, axes=[0])
...
class SeparationDenseLossGraph(AbstractLossGraph):
    ...
    def connect_loss_graph(self, tf_prediction, tf_interactions, **kwargs):
        ...
        int_serial_shape = tf.cast(
            [interactions_shape[0] * interactions_shape[1]], tf.int32)
        ...
        pred_serial_shape = tf.cast(
            [prediction_shape[0] * prediction_shape[1]], tf.int32)
        tf_prediction_serial = tf.reshape(
            tf_prediction, shape=pred_serial_shape)
...
class BalancedWMRBLossGraph(WMRBLossGraph):
    ...
    def weighted_margin_rank_batch(self, tf_prediction_serial, tf_interactions, tf_sample_predictions, tf_n_items,
        ...
        listening_sum_per_item = tf.sparse_reduce_sum(
            positive_interactions, axis=0)
