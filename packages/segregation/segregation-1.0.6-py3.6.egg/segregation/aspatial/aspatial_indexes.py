...
__all__ = [
    'Dissim', 'Gini_Seg', 'Entropy', 'Isolation', 'Exposure', 'Atkinson',
    'Correlation_R', 'Con_Prof', 'Modified_Dissim', 'Modified_Gini_Seg',
    'Bias_Corrected_Dissim', 'Density_Corrected_Dissim'
]
...
def _gini_seg(data, group_pop_var, total_pop_var):
    ...
    data = data.assign(ti=data.total_pop_var,
                       pi=np.where(data.total_pop_var == 0, 0,
                                   data.group_pop_var / data.total_pop_var))

    num = (
        np.matmul(
            np.array(data.ti)[np.newaxis].T,
            np.array(data.ti)[np.newaxis]) *
        abs(np.array(data.pi)[np.newaxis].T -
            np.array(data.pi)[np.newaxis])).sum()
    den = (2 * T**2 * P * (1 - P))
    G = num / den
...
def _entropy(data, group_pop_var, total_pop_var):
    ...
    H = np.nansum(t * (E - Ei) /
                  (E * T))  # If some pi is zero, numpy will treat as zero
...
def _conprof(data, group_pop_var, total_pop_var, m=1000):
    ...
    R = ((threshold - ((curve[grid < threshold]).sum() / m -
...
class Modified_Gini_Seg:
    ...
    def __init__(self, data, group_pop_var, total_pop_var, iterations=500):
        ...
        aux = _modified_gini_seg(data, group_pop_var, total_pop_var,
                                 iterations)
...
def _density_corrected_dissim(data, group_pop_var, total_pop_var, xtol=1e-5):
    ...
    sigma_hat_j = np.sqrt(((p1_i * (1 - p1_i)) / n1) + ((p0_i *
                                                         (1 - p0_i)) / n0))
