...
from ptcaffe.utils.utils import tensor2blob, check_file_exists, python_version, SafeNetwork, make_list, get_layer_bnames, get_layer_tnames
...
class CaffeNet(nn.Module):
    ...
    def set_automatic_outputs(self, phase=""):
        ...
        for layer in layers:
            ...
            if 'include' in layer and 'selector' in layer['include']:
                ...
                if self.selector != '' and selector != self.selector:
                    continue
            bnames = get_layer_bnames(layer, phase)
            bnames = bnames if isinstance(bnames, list) else [bnames]
            tnames = get_layer_tnames(layer, phase)
    ...
    def create_blob_eval_relys(self, net_info):
    ...
    def forward(self, *inputs):
        ...
        if (not self.training) and self.save_memory_eval:
            blob_eval_relys = self.create_blob_eval_relys(self.net_info)
        ...
        for layer in layers:
            ...
            try:
                ...
                print("tname:{}, bname{}".format(tname, bname))
    ...
    def create_network(self, net_info):
        ...
        for layer in layers:
            ...
            if ltype == 'Python':
                ...
                models[lname] = self.create_python_layer(layer, *input_shapes)
    ...
    def create_python_layer(self, layer, *input_shapes):
        ...
        if 'bottom' in layer:
            layer2['bottom'] = get_layer_bnames(layer,phase)
