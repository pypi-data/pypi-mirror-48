...
class keyword_tweets:
    ...
    def __init__(self, keyword):
        ...
        def cleaning_text(text):
            ...
            text = ' '.join([word for word in tokenizer.tokenize(' '.join(words)) if word not in stop_words])
            return text

        self.original_tweets = [' '.join(item.text.split()) for item in tweets_count]
        self.cleaned_tweets = [cleaning_text(tweet) for tweet in self.original_tweets]
...
class username_tweets:

    """
    Collects tweets of the social-media content in Twitter when hashtag is given.
    :param tweets: Unique identification keyword for every social-media tweets in Twitter.
    :returns: Returns all the user tweets.
    """
    ...
    def __init__(self, username):
        ...
        options=options
        browser = webdriver.Chrome(options=options)
        browser.get("https://twitter.com/" + str(self.username))
...
class hashtag_tweets:
    ...
    def __init__(self, hashtag):
        ...
        options=options
        browser = webdriver.Chrome(options=options)
        browser.get("https://twitter.com/" + str(%23) + str(self.hashtag))
