    ...
    def __init__(self, file, world):
        ...
        with io.open(file) as data_file:
            if self.file.split(".")[-1] == "json":
                ...
                self.model = yaml.load(data_file)
            ...
            try:
                ...
                self.background = []
            try:
                self.mechanisms = {str(k): CausalModel.myEval(v) for k, v in self.model["mechanisms"].items()}
            except:
                self.mechanisms = {}
