...
class ArgModel:   
    ...
    def parseFormula(string):
        ...
        for p in pol:
            if p in ["Top", "Bottom"]:
                myStack.append(eval(p + "()"))
            elif p in ["Good", "Bad", "Neutral", "Not", "Atom"]:
                myStack.append(eval(p + "(" + repr(myStack.pop()) + ")"))
            elif p in ["And", "Because", "Same", "Or", "Impl", "BiImpl", "Causes", "Intervene"]:
                myStack.append(eval(p + "(" + repr(myStack.pop()) + ", " + repr(myStack.pop()) + ")"))
            elif p in ["Forall", "Exists"]:
                first = myStack.pop()
                second = myStack.pop()
                myStack.append(eval(p + "(" + repr(first) + ", " + repr(second) + ")"))
