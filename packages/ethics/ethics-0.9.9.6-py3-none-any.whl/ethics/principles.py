...
class DoNoHarmPrinciple(Principle):
    ...
    def _check(self):
        ...
        for c in self.cons:
            if f == None:
                f = Impl(Causes(self.model.action, c), Not(Bad(c)))
            else:
                f = And(Impl(Causes(self.model.action, c), Not(Bad(c))), f)
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
...
class DoNoInstrumentalHarmPrinciple(Principle):
    ...
    def _check(self):
        ...
        for c1 in self.cons:
            for c2 in self.cons:
                if f == None:
                    f = Impl(And(Causes(self.model.action, c1), And(Causes(c1, c2), I(c2))), Not(Bad(c1)))
                else:
                    f = And(Impl(And(Causes(self.model.action, c1), And(Causes(c1, c2), I(c2))), Not(Bad(c1))), f)
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
...
class IntentionFocusedDeontologicalPrinciple(DeontologicalPrinciple):
    ...
    def _check(self):
        f = Not(Bad(self.model.action))
        for c in self.model.getAllConsequences():
            f = And(Impl(I(c), Not(Bad(c))), f)
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
