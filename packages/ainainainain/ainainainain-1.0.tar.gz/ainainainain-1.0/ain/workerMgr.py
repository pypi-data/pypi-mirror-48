...
class Worker():
	...
	def run(self, optionRun):
		...
		instanceUtil.InstanceUtil.createTtyd(self.hostTtydSocket)

		# open docker container for ain worker server
		instanceUtil.constantsInstanceUtil.createContainer(constants.IMAGE, self.ENVS)
	...
	def terminate(self):
		...
		try:
			instanceUtil.InstanceUtil.removeContainer("ain_worker")
		...
		try:
			instanceUtil.InstanceUtil.removeTtyd(self.hostTtydSocket)
  ...
  def init(self):
    for key in ["NAME", "LIMIT_COUNT", "PRICE", "DESCRIPTION", "MNEMONIC", "SERVER_IP"]:
