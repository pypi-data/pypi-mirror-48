#!/usr/bin/env python3
from olctools.accessoryFunctions.accessoryFunctions import run_subprocess, SetupLogging
...
class VCF(object):
    ...
    def __init__(self, path, threads, debug, reference_mapper, variant_caller, matching_hashes):
        ...
        if not os.path.isdir(self.dependency_path):
            ...
            logging.debug('wget -P {dep_root} https://ndownloader.figshare.com/files/14619260 && tar -xf {download}'
                          .format(dep_root=self.dependency_root,
                                  download=os.path.join(self.dependency_root, '14619260')))
            run_subprocess('wget -P {dep_root} https://ndownloader.figshare.com/files/14619260 && tar -xf {download}'
                           .format(dep_root=self.dependency_root,
                                   download=os.path.join(self.dependency_root, '14619260')))
