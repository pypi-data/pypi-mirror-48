...
class Client(Fist):
    ...
    def insert_basket_order(self, tg_name, basket_order, child_order_type, basket_name='', **algo_args):
    ...
    def insert_basket_order_w_twap(self, tg_name, basket_order, start_time, end_time, sec_interval, basket_name=''):
        start_nano = Timer.get_nano(start_time)
        end_nano = Timer.get_nano(end_time)
        return self.insert_basket_order(tg_name, basket_order, OrderType.TWAP, basket_name=basket_name, start_nano=start_nano, end_nano=end_nano, sec_interval=sec_interval)
    ...
    def on_pub_frame(self, f):
        ...
        if MsgType.is_market_data_type(msg_type):
            if msg_type == MsgType.MKT_SNAP:
                ...
                snap_obj = f.get_obj(message_market.MktIndex)
                self.on_mkt_index(snap_obj, msg_type, frame_nano)

            elif msg_type == MsgType.MKT_ORDER:
                snap_obj = f.get_obj(message_market.MktOrder)
                self.on_mkt_order(snap_obj, msg_type, frame_nano)

            elif msg_type == MsgType.MKT_TRADE:
                snap_obj = f.get_obj(message_market.MktTrade)
                self.on_mkt_trade(snap_obj, msg_type, frame_nano)
