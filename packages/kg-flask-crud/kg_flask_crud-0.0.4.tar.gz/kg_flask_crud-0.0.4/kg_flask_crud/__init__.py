...
@ns_token.route('/hello')
class ResourceRoute(Resource):
    @ns_token.doc('helloworld',security='token')
    @requires_auth([])
    def get(self):
...
def request_to_class(dbclass,json_request): # Make classmethod
    ...
    for k,v in json_request.items():
        if k == 'tags' and v != []:
            ...
            if hasattr(dbclass, 'many_to_many'):
                if k in dbclass.many_to_many :
...
class CRUD():
    def __init__(self, namespace, cls, model, name, db, view_auth=[], edit_auth=['admin'], delete_constraints={}, security='token',validate_json=False, custom_post=False, custom_put=False ):
        self.ns = namespace
        self.cls = cls
        self.model = model
        self.name = name
...
def create_crud(name,description,cls,db,view_auth=[],edit_auth=['admin'],delete_constraints={},security='token',validate_json=False,custom_post=False,custom_put=False):
    ns_obj = Namespace(name,description=description)
    obj_model = ns_obj.schema_model(name,cls.validator)
    CRUD(ns_obj,cls,obj_model,name,db,view_auth=view_auth,edit_auth=edit_auth,delete_constraints=delete_constraints,security=security,validate_json=validate_json,custom_post=custom_post,custom_put=custom_put)
    return ns_obj
