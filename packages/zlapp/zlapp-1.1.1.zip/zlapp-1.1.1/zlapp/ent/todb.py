...
def add_t_base_src():
    ...
    df=db_query(sql="""select * from(select distinct(entname)  as name from public.ent as b where not exists(select 1 from cdc.t_base_src as a where a.jgmc=b.entname)) as t 
        order by random() """
