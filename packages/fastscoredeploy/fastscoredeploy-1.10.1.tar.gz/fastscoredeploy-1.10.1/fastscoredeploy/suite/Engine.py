...
class Engine(BaseEngine):
    ...
    def score(self, data, encode=True):
        ...
        if (type(transport_0) == dict and 'Type' in transport_0.keys() and transport_0['Type'] != 'REST') and transport_0 != 'REST':
            raise FastScoreError("Input stream must use REST transport.")
        transport_1 = active_streams[1].descriptor['Transport']
        if (type(transport_1) == dict and 'Type' in transport_1.keys() and transport_1['Type'] != 'REST') and transport_1 != 'REST':
        ...
        while not pig_received:
            ...
            if {"$fastscore": "pig"} in [json.loads(x) for x in outputs]:
                pig_received = True
