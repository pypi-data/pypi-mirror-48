...
class Station:
    ...
    def __init__(self, workspace, stationName, config, resultConnector, stationID):
        if not Station.instance:
            Station.instance = Station.__Station(workspace, stationName, config, resultConnector, stationID)
    ...
    class __Station:
        ...
        def loadProduct(self, name, version, revision=None, ioHandler=None, ui=None, discardChanges=False):
            self.loadedProduct = Product.Product.load(self, name, version, revision, ioHandler=ioHandler, ui=ui)
        ...
        def listProductRevisions(self, productName, versionName):
            productID = self.getProductID(productName)
            if productID != None:
                filt = "`product_id`=%d and `version` like '%s'"%(productID, versionName)
    ...
    def loadStationAsync(workspace, stationName, ioHandler=None, ui=None):
        ...
        ctor = ProductExecutorCtor.ProductExecutorCtor(ioHandler, params, ioHandler, ui)
