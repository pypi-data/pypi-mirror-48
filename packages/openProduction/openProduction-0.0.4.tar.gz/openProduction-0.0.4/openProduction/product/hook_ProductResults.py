...
class ResultConnector(ProductExecutor.ProductExecutor):
    ...
    def install(self, testSuite):
        ...
        testSuite.addMethodHint(self.load_result_00100, hint)
        testSuite.addMethodHint(self.teardown_result_00100, hint)
        self.testSuite = testSuite
        
    def load_result_00100(self, params, values):
        """Synchronisation Datenablage"""
        if self.station == None:
            self.logger.info("station is NONE")
            return TestSuite.ExecutionResult.SKIP
        if self.station.loadedProduct == None:
            self.logger.info("product is NONE")
            return TestSuite.ExecutionResult.SKIP
    ...
    def teardown_result_00100(self, params, values):
        ...
        if self.station != None and self.station.loadedProduct != None:
            ...
            for f in values.getFiles():
                if "sftp_config_id" in params:
                    ...
                    sftp_config_id = 1
                data = {"result_id":result_id, "filename_public": f, "sftp_config_id": sftp_config_id}
                rv, data = self.station.serverIF.createProductResultFile(data)
                self.logger.info("Übertrage Datei %s"%f)
                self.assertEqual(rv, ConnectorErrors.NO_ERROR, "Fehler beim Speichern der Datei %s"%f)
        else:
            self.logger.info("Skippe Erbenisübertragung, station=%s, loadedProduct=%s"%(str(self.station), str(self.station.loadedProduct)))
            return TestSuite.ExecutionResult.SKIP
        ...
        # self.assertNotEqual(values.DEVICE_UUID.getValue(), None)
        # rv = self.connector.saveValues(self.dbName, self.tableName, values)
        # self.assertEqual(rv, True, "Fehler beim Ablegen der Durchlaufergebnisse")