...
#        self.assertEqual(results.wasSuccessful(), True)
    ...
    def load__00100(self, params, values):
        ...
        vals = TestParams.TestParams({}, None)
        ctor = ProductExecutorCtor.ProductExecutorCtor(self.ioHandler, params, vals, self.ui)
    ...
    def load__00200(self, params, values):
        ...
        grps = self.testSuite.getTestCaseGroups()
        for grp in grps:
            ok, desc = grp.obj.probe()
            self.assertEqual(ok, True, "Probing HW of group %s failed, with msg %s"%(grp.name, desc))
