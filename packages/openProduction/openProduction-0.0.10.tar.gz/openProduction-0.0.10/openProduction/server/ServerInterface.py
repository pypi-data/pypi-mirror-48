...
class ServerInterface:
    ...
    def getPreviousStationID(self, station_id, product_id):
        ...
        if rv == ConnectorErrors.NO_ERROR:
            ...
            # "`product_step_id` = %d and revision_id=(select max(revision_id) from `%s`.`%s` WHERE `product_step_id` = %d)"
            
            columns=["MAX(`order`)", "`station_id`"]
            rv, data = self.listProductStationLinks(filt)
    ...
    def listProductResults(self, filt=None):
