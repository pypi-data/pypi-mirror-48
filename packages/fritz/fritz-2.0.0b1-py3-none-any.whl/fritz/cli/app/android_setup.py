...
from fritz.cli.print_utils import print_code, print_ack, print_title
from fritz.cli.file_utils import (
    find_all,
    find_text_in_file,
    insert_code_before,
)
from fritz.cli.app.setup_utils import pause_confirmation, setup_app_info
...
def show_android_setup_steps(cwd):
    ...
    """
    file_paths = find_all("AndroidManifest.xml", cwd)
    gradle_file_paths = find_all("build.gradle", cwd)
    ...
    print_title("Step 1: Register your app with Fritz")
    app_info = setup_app_info(app_name, package_name, "android")

    print_title("Step 2: Update your AndroidManifest")
    update_android_manifest(android_manifest_file)

    print_title("Step 3: Update your app dependencies")
    ...
    print_title("Step 4: Initialize the SDK")
    click.echo(
        "Initialize the SDK in the Application / MainActivity "
        "onCreate lifecycle method.\n"
    )

    print_code(create_fritz_configure_content(app_info["api_key"]))
...
def search_and_find_app_name(cwd, path_to_manifest):
    ...
    if "@string/" in app_label:
        ...
        string_files = find_all("strings.xml", cwd)
...
def update_dependencies_with_fritz(
    ...
    if not find_text_in_file(repo, app_project_gradle_file):
        click.echo(
        ...
        print_code(ANDROID_PROJECT_BUILD_REPO)
        requires_mod = True
    else:
        print_ack("Fritz Maven repository already exists")

    if not find_text_in_file("ai.fritz:core", app_build_gradle_file_path):
        click.echo(
            "\n\nInclude the SDK in your app build.gradle file\n[{app_build_gradle_file_path}].\n".format(
                app_project_gradle_file=app_project_gradle_file
            )
        )
        print_code(ANDROID_APP_BUILD_DEPENDENCIES)
        requires_mod = True
    else:
        print_ack("Fritz Core dependency already added")
...
def search_for_apk_id_in_gradle(gradle_file_paths):
    ...
    for file_path in gradle_file_paths:
        line = find_text_in_file("applicationId", file_path)
        if line:
...
def update_android_manifest(manifest_path):
    ...
    if find_text_in_file(
        ...
        print_ack("Already added the service to the AndroidManifest")
        return

    click.echo("Update your AndroidManifest.xml\n\n")
    print_code(ANDROID_MANIFEST_SERVICE)
    ...
    code_to_insert = """
        <service
            ...
            android:permission="android.permission.BIND_JOB_SERVICE" />
"""
    insert_code_before("</application>", code_to_insert, manifest_path)
