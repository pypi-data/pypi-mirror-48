...
def setup_new_experiment(device):
    ...
    exp_beaker = answer_input('Specify the test system (A1, B1, F1, H1, R1, AN, HU): ', '([ABFPHR][0-9]|AN|HU){1}', 'ZZ', True)
    exp_run = int(answer_input('Specify the run number [1]: ', '[0-9]{1,2}', 1, False))
    exp_lot_we = answer_input('WE Lot # (required): ', '[0-9]{1,3}[A-Z0-9]{0,1}', 1, True)
    exp_lot_re = int(answer_input('CE/RE Lot # [000]: ', '[0-9]{0,3}', 0, False))
    exp_lot_sn = answer_input('Sensor ID [00]: ', '[A-Z0-9]{0,2}', '00', False)
    exp_type = answer_input('Sensor Type ([G]lucose, (O)xygen, (L)actate, (K)etone, (T)emp, (X) Other): ', '[GOLKTX]', 'G', False)
    ...
    expid = '{}{}{}{}{:02d}{:0>3.3}-{:4.04}-{:03d}-{:.2}-{}{:02d}{}{}{}{}'.format(
