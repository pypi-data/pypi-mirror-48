...
# (s�lo para linea de comandos):

ENCABEZADO = [
    ('tipo_reg', 1, A, u"0: encabezado"),
    ('cbte_modo', 4, A, u"Modalidad de autorizaci�n (CAI, CAE, CAEA)"),
    ...
    ('cbte_nro', 8, N, u"N�mero de comprobante"),
    ('cbte_fch', 8, A, u"Fecha en formato AAAAMMDD"),
    ('imp_total', 15, I, u"Importe total Double (13 + 2)"),
    ('cod_autorizacion', 14, A, u"N�mero de CAI, CAE, CAEA"),
...
OBSERVACION = [
    ...
    ('code', 5, N, u"C�digo de Observaci�n / Error / Evento"),
    ('msg', 255, A, u"Mensaje"),
]
...
class WSCDC(BaseWS):
    ...
    _public_attrs_ = [
        ...
        'EmisionTipo', 'CAE', 'CAEA', 'CAI', 'DocTipo', 'DocNro',
    ...
    def ConstatarComprobante(
        self, cbte_modo, cuit_emisor, pto_vta, cbte_tipo, cbte_nro, cbte_fch,
        imp_total, cod_autorizacion, doc_tipo_receptor=None,
        doc_nro_receptor=None, **kwargs
    ):
    ...
    def ConsultarTipoOpcionales(self, sep="|"):
        ...
        self.__analizar_errores(result)
        return [(u"\t%(Id)s\t%(Desc)s\t" % p['OpcionalTipo'])
                .replace("\t", sep)
                for p in res]
...
def main():
    ...
    url_wsaa = config.get('WSAA', 'URL') if config.has_option('WSAA',
                                                              'URL') else ""
    url_wscdc = config.get('WSCDC', 'URL') if config.has_option('WSCDC',
                                                                'URL') else ""
    ...
    if "--constatar" in sys.argv:
        if len(sys.argv) < 8:
            ...
            wscdc.ConstatarComprobante(
                *sys.argv[sys.argv.index("--constatar") + 1:])
