...
import zipfile,io, re, nltk, numpy
name = "docuscan"
class DocuScan():
    '''
    ...
    def findName(self):
        '''
            ...
            Usage:
                findName()
            Returns:
                List Of Names from Document 
        '''
        parsedText=self.returnFileText()
        tokens=nltk.word_tokenize(parsedText, language='french')
        tagged = nltk.pos_tag(tokens)
        tokens = nltk.chunk.ne_chunk(tagged)
        return (tokens)
        
hello=DocuScan('5438251.pdf')
hi = hello.returnFileText()
hell=hi.replace('\n','')
print(hell)
print(re.findall(r"[A-z0-9\.\-+_]+@[A-z0-9\.\-+_]+\.[A-z]+",hell))
