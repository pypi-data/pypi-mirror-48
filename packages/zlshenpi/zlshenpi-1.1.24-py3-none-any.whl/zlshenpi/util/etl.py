...
def ext_tb(tbname):
    ...
    ggtype_dict = {"zhaobiao": "招标公告", "zhongbiao": "中标公告", "zhongbiaohx": "中标候选公告", "liubiao": "流标公告", "kaibiao": "开标公告",
        ...
        , "beian": "备案", "hezhun": "核准", "jieguo": "结果", "hzqgs": "核准前公示", "shenpi": "审批", "pihou": "批后", "kaigong": "开工", "jungong": "竣工"  # 项目审批
...
def html_work(conp, f, size=None, headless=True):
    m = page()
    if size is not None:
        sql = "select distinct href,ggstart_time from %s.gg order by ggstart_time desc where not coalesce(info,'{}')::jsonb?'hreftype' or coalesce(info,'{}')::jsonb->>'hreftype'='可抓网页' limit %d" % (
            conp[4], size)
    else:
