...
def cmt(command, args=[], environment=None, cwd=None):
    ...
    return Popen(
        ...
        stderr=STDOUT).communicate()[0].decode()
...
class SetupProject(object):
    ...
    def check_environment(self):
        ...
        if 'BINARY_TAG' in self.environment:
            if ('CMTCONFIG' in self.environment and
                    self.environment['CMTCONFIG'] != os.environ['BINARY_TAG']):
                self._warning(
                    'both CMTCONFIG and BINARY_TAG set, using BINARY_TAG')
            self.environment['CMTCONFIG'] = os.environ['BINARY_TAG']
    ...
    def set_build_env(self):
        ...
        if self.project_info.policy == 'old':  # old style project
            ...
            ShellParser[self.shell](self.cmt(
                "setup", "-" + self.shell,
    ...
    def _prepare_parser(self):
        ...
        def nightly_option(_option, opt_str, _value, _parser):
            ...
            if os.path.exists(
                ...
                extraCMTPROJECTPATH = list(filter(
                    str, data.get("cmtProjectPathList", [])))
                ...
                if hat is None:
                    ...
                    if hat:
                        self._warning(
                            ...
                            ' correct name is %s/%s', pack, hat, pack)
                req.write("use %s %s %s\n" % (pack, ver, hat))
    ...
    def _prepend_prompt(self):
        new_prompt = ""
        if os.environ[
                "SP_PROMPT"] == "without_version" or not self.project_info.version:
            if self.shell == "sh":
    ...
    def prepare(self, args=None):
        ...
        if self.project_version:
            for v in versions:
                ...
                self._error("Cannot find version '%s' of %s. Try with --list."
        ...
        if required_root_version:
            ...
            lcg_version = FindLCGForROOT(
                required_root_version, SortVersionTuples(
    ...
    def main(self, args=None):
        ...
        if ('SITEROOT' not in self.environment
                and 'MYSITEROOT' in self.environment):
