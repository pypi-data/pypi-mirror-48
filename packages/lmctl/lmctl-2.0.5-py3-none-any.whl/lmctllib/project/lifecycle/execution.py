...
class ProjectLifecycleExecutor:
    ...
    def __execute_pipeline(self, pipeline_name, pipeline_task_name, executor_options):
        ...
        self.__process_execution_result(pipeline_name, execution_result)

    def __process_execution_result(self, pipeline_name, result):
        self.journal.open_chapter(JRNL_CHAPTER_EXEC_END)
        if result.code is pipeline.ResultCode.PASSED:
            self.journal.add_text("{0} Complete - PASSED".format(pipeline_name))
        else:
            self.journal.add_text(result.report())
        self.journal.close_chapter()

    def build(self, executor_options=None):
        if executor_options is None:
            executor_options = {}
        self.__execute_pipeline("Build", build_pipeline.task_name, executor_options)

    def push(self, executor_options=None):
        if executor_options is None:
            executor_options = {}
        self.__execute_pipeline("Push", push_pipeline.task_name, executor_options)

    def test(self, executor_options=None):
        if executor_options is None:
            executor_options = {}
        self.__execute_pipeline("Test", test_pipeline.task_name, executor_options)

    def pull(self, executor_options=None):
        if executor_options is None:
            executor_options = {}
        self.__execute_pipeline("Pull", pull_pipeline.task_name, executor_options)
