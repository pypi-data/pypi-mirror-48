...
from zlhawq.zlsys.core import add_quyu_tmp,restart_quyu_tmp
import traceback
from zlhawq.zlsys.data import zl_sys_dict
...
def add_quyu_db1_remote(quyu,tag='all'):
    ...
    try:
        c.run("""/opt/python35/bin/python3 -c "from zlhawq.sys.api import add_quyu_db1_local;add_quyu_db1_local('%s','%s') " """%(quyu,tag),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
...
def restart_quyu_db1_remote(quyu):
    ...
    c=Connection(conp_remote[0],connect_kwargs={"password":conp_remote[1]})
    try:
        c.run("""/opt/python35/bin/python3 -c "from zlhawq.sys.api import restart_quyu_db1_local;restart_quyu_db1_local('%s') " """%(quyu),pty=True,encoding='utf8')
    except:
        traceback.print_exc()
    finally:
        c.close()
...
def add_quyu1_sheng(sheng,tag='all'):
    quyus=zl_sys_dict[sheng]

    for quyu in quyus:
        add_quyu1(quyu,tag)

def add_quyu1_all():
    for sheng in zl_sys_dict.keys():
