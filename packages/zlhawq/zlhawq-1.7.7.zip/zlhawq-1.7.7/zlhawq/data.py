zhulong_diqu_dict={
  "beijing":["beijing_beijing"],

  "hubei": [
    "hubei_dangyang",
    "hubei_enshi",
    "hubei_huanggang",
    "hubei_huangshi",
    "hubei_jingmen",
    "hubei_lichuan",
    "hubei_shiyan",
    "hubei_suizhou",
    "hubei_wuhan",
    "hubei_xiangyang",
    "hubei_xiaogan",
    "hubei_yichang",
    "hubei_yidu"
  ],
  "ningxia": [
    "ningxia_yinchuan",
    "ningxia_guyuan",
    "ningxia_ningxia"
  ],
  "shanxi1": [
    "shanxi1_shanxi",
    "shanxi1_shanxi2"
  ],
  "shanxi": [
    "shanxi_weinan",
    "shanxi_xianyang",
    "shanxi_shanxi",
    "shanxi_chenzhou",
    "shanxi_xian",
    "shanxi_ankang",
    "shanxi_xianyang2",
    "shanxi_yanan"
  ],
  "fujian": [
    "fujian_fujian",
    "fujian_fuqing",
    "fujian_fuzhou",
    "fujian_jianou",
    "fujian_longyan",
    "fujian_nanan",
    "fujian_nanping",
    "fujian_ningde",
    "fujian_putian",
    "fujian_quanzhou",
    "fujian_sanming",
    "fujian_shaowu",
    "fujian_wuyishan",
    "fujian_xiamen",
    "fujian_yongan",
    "fujian_zhangzhou",
    "fujian_fujian2"
  ],
  "guangxi": [
    "guangxi_guilin",
    "guangxi_fangchenggang",
    "guangxi_guigang",
    "guangxi_chongzuo",
    "guangxi_qinzhou",
    "guangxi_hechi",
    "guangxi_hezhou",
    "guangxi_nanning",
    "guangxi_baise",
    "guangxi_laibin",
    "guangxi_beihai",
    "guangxi_wuzhou",
    "guangxi_liuzhou",
    "guangxi_guangxi"
  ],
  "hebei": [
    "hebei_hebei"
  ],
  "shandong": [
    "shandong_anqiu",
    "shandong_binzhou",
    "shandong_dezhou",
    "shandong_feicheng",
    "shandong_heze",
    "shandong_jiaozhou",
    "shandong_jinan",
    "shandong_jining",
    "shandong_laiwu",
    "shandong_liaocheng",
    "shandong_linqing",
    "shandong_linyi",
    "shandong_pingdu",
    "shandong_qingdao",
    "shandong_qufu",
    "shandong_rizhao",
    "shandong_rongcheng",
    "shandong_rushan",
    "shandong_shandong",
    "shandong_taian",
    "shandong_tengzhou",
    "shandong_weifang",
    "shandong_weihai",
    "shandong_xintai",
    "shandong_yantai",
    "shandong_yucheng",
    "shandong_zaozhuang",
    "shandong_zibo",
    "shandong_zoucheng",
    "shandong_dongying",
    "shandong_leling",
    "shandong_shandong2"
  ],
  "gansu": [
    "gansu_baiyin",
    "gansu_dingxi",
    "gansu_gansu",
    "gansu_jiayuguan",
    "gansu_jiuquan",
    "gansu_lanzhou",
    "gansu_longnan",
    "gansu_pingliang",
    "gansu_qingyang",
    "gansu_tianshui",
    "gansu_wuwei",
    "gansu_zhangye"
  ],
  "henan": [
    "henan_anyang",
    "henan_dengfeng",
    "henan_gongyi",
    "henan_hebi",
    "henan_jiaozhuo",
    "henan_jiyuan",
    "henan_jiyuan1",
    "henan_kaifeng",
    "henan_linzhou",
    "henan_luohe",
    "henan_luoyang",
    "henan_mengzhou",
    "henan_nanyang",
    "henan_pingdingshan",
    "henan_puyang",
    "henan_qinyang",
    "henan_ruzhou",
    "henan_sanmenxia",
    "henan_shangqiu",
    "henan_weihui",
    "henan_wugang",
    "henan_xinmi",
    "henan_xinxiang",
    "henan_xinyang",
    "henan_xinzheng",
    "henan_xuchang",
    "henan_yanshi",
    "henan_yongcheng",
    "henan_zhengzhou",
    "henan_zhoukou",
    "henan_zhumadian"
  ],
  "xinjiang": [
    "xinjiang_xinjiang",
    "xinjiang_wulumuqi",
    "xinjiang_kezhou",
    "xinjiang_beitun",
    "xinjiang_akesu",
    "xinjiang_aletai"
  ],
  "jiangxi": [
    "jiangxi_dexing",
    "jiangxi_fengcheng",
    "jiangxi_fuzhou",
    "jiangxi_ganzhou",
    "jiangxi_gaoan",
    "jiangxi_jian",
    "jiangxi_jiangxi",
    "jiangxi_jingdezhen",
    "jiangxi_jingdezhen2",
    "jiangxi_jinggangshan",
    "jiangxi_lushan",
    "jiangxi_nanchang",
    "jiangxi_ruichang",
    "jiangxi_ruijin",
    "jiangxi_shangrao",
    "jiangxi_xinyu",
    "jiangxi_yichun",
    "jiangxi_yingtan",
    "jiangxi_zhangshu"
  ],
  "jilin": [
    "jilin_jilin",
    "jilin_liaoyuan",
    "jilin_baicheng",
    "jilin_jilinshi",
    "jilin_tonghua",
    "jilin_siping",
    "jilin_changchun",
    "jilin_baishan",
    "jilin_songyuan"
  ],
  "xizang": [
    "xizang_xizang",
    "xizang_lasa",
    "xizang_rikaze"
  ],
  "hunan": [
    "hunan_changde",
    "hunan_changsha",
    "hunan_chenzhou",
    "hunan_hengyang",
    "hunan_huaihua",
    "hunan_hunan",
    "hunan_liling",
    "hunan_liuyang",
    "hunan_loudi",
    "hunan_shaoyang",
    "hunan_xiangtan",
    "hunan_yiyang",
    "hunan_yongzhou",
    "hunan_yuanjiang",
    "hunan_yueyang",
    "hunan_zhangjiajie",
    "hunan_zhuzhou"
  ],
  "heilongjiang": [
    "heilongjiang_qiqihaer",
    "heilongjiang_daqing",
    "heilongjiang_hegang",
    "heilongjiang_yichun",
    "heilongjiang_heilongjiang"
  ],
  "anhui": [
    "anhui_anqing",
    "anhui_bengbu",
    "anhui_bozhou",
    "anhui_chaohu",
    "anhui_chizhou",
    "anhui_chuzhou",
    "anhui_fuyang",
    "anhui_hefei",
    "anhui_huaibei",
    "anhui_huainan",
    "anhui_huangshan",
    "anhui_luan",
    "anhui_maanshan",
    "anhui_suzhou",
    "anhui_tongling",
    "anhui_wuhu",
    "anhui_xuancheng"
  ],
  "qinghai": [
    "qinghai_xining",
    "qinghai_qinghai"
  ],
  "chongqing": [
    "chongqing_chongqing",
    "chongqing_yongchuan"
  ],
  "zhejiang": [
    "zhejiang_longquan",
    "zhejiang_ruian",
    "zhejiang_pinghu",
    "zhejiang_yueqing",
    "zhejiang_zhejiang",
    "zhejiang_hangzhou",
    "zhejiang_shaoxing",
    "zhejiang_tongxiang",
    "zhejiang_ningbo",
    "zhejiang_wenling",
    "zhejiang_quzhou",
    "zhejiang_wenzhou",
    "zhejiang_jiaxing",
    "zhejiang_cixi",
    "zhejiang_dongyang",
    "zhejiang_yiwu",
    "zhejiang_jinhua",
    "zhejiang_shengzhou",
    "zhejiang_zhoushan",
    "zhejiang_lishui",
    "zhejiang_zhuji",
    "zhejiang_linhai",
    "zhejiang_huzhou",
    "zhejiang_yuhuan",
    "zhejiang_taizhou"
  ],
  "liaoning": [
    "liaoning_anshan",
    "liaoning_benxi",
    "liaoning_chaoyang",
    "liaoning_dalian",
    "liaoning_dandong",
    "liaoning_donggang",
    "liaoning_fuxin",
    "liaoning_haicheng",
    "liaoning_huludao",
    "liaoning_jinzhou",
    "liaoning_liaoning",
    "liaoning_liaoyang",
    "liaoning_panjin",
    "liaoning_shenyang",
    "liaoning_tieling",
    "liaoning_yingkou"
  ],
  "sichuan": [
    "sichuan_wanyuan",
    "sichuan_meishan",
    "sichuan_dujiangyan",
    "sichuan_deyang",
    "sichuan_yibin",
    "sichuan_sichuan",
    "sichuan_panzhihua",
    "sichuan_jianyang",
    "sichuan_guangan",
    "sichuan_leshan",
    "sichuan_jiangyou",
    "sichuan_bazhong",
    "sichuan_luzhou",
    "sichuan_guangyuan",
    "sichuan_chengdu",
    "sichuan_chongzhou",
    "sichuan_guanghan",
    "sichuan_pengzhou",
    "sichuan_shifang",
    "sichuan_mianyang1",
    "sichuan_mianyang2",
    "sichuan_longchang",
    "sichuan_dazhou",
    "sichuan_yaan",
    "sichuan_nanchong",
    "sichuan_qionglai",
    "sichuan_suining",
    "sichuan_neijiang",
    "sichuan_zigong",
    "sichuan_ziyang",
    "sichuan_sichuan2"
  ],
  "hainan": [
    "hainan_sanya",
    "hainan_danzhou",
    "hainan_sansha",
    "hainan_dongfang",
    "hainan_haikou",
    "hainan_qionghai",
    "hainan_hainan"
  ],
  "yunnan": [
    "yunnan_honghe",
    "yunnan_chuxiong",
    "yunnan_lijiang",
    "yunnan_puer",
    "yunnan_yunnan",
    "yunnan_kunming",
    "yunnan_wenshan",
    "yunnan_yunnan2",
    "yunnan_dali",
    "yunnan_zhaotong",
    "yunnan_xishuangbanna",
    "yunnan_baoshan",
    "yunnan_tengchong",
    "yunnan_yuxi",
    "yunnan_lincang",
    "yunnan_dehong"
  ],
  "jiangsu": [
    "jiangsu_danyang",
    "jiangsu_nanjing",
    "jiangsu_wuxi",
    "jiangsu_zhenjiang",
    "jiangsu_jiangyin",
    "jiangsu_nantong",
    "jiangsu_taizhou",
    "jiangsu_changshu",
    "jiangsu_yizheng",
    "jiangsu_yangzhou",
    "jiangsu_suzhou",
    "jiangsu_kunshan",
    "jiangsu_lianyungang",
    "jiangsu_jiangsu",
    "jiangsu_zhangjiagang",
    "jiangsu_xuzhou",
    "jiangsu_suqian",
    "jiangsu_changzhou",
    "jiangsu_huaian",
    "jiangsu_xinyi"
  ],
  "guizhou": [
    "guizhou_qiannan",
    "guizhou_liupanshui",
    "guizhou_qiandong",
    "guizhou_zunyi",
    "guizhou_shenghui",
    "guizhou_anshun",
    "guizhou_qianxi",
    "guizhou_bijie",
    "guizhou_guiyang",
    "guizhou_tongren",
    "guizhou_shenghui2",
  ],
  "neimenggu": [
    "neimenggu_baotou",
    "neimenggu_bayannaoer",
    "neimenggu_chifeng",
    "neimenggu_eeduosi",
    "neimenggu_huhehaote",
    "neimenggu_hulunbeier",
    "neimenggu_manzhouli",
    "neimenggu_tongliao",
    "neimenggu_wuhai",
    "neimenggu_wulanchabu",
    "neimenggu_xilinguolemeng",
    "neimenggu_xinganmeng"
  ],
  "guangdong": [
    "guangdong_chaozhou",
    "guangdong_dongguan",
    "guangdong_foshan",
    "guangdong_guangdong",
    "guangdong_guangzhou",
    "guangdong_heyuan",
    "guangdong_huizhou",
    "guangdong_jiangmen",
    "guangdong_jieyang",
    "guangdong_lianzhou",
    "guangdong_maoming",
    "guangdong_meizhou",
    "guangdong_nanxiong",
    "guangdong_shantou",
    "guangdong_shanwei",
    "guangdong_shaoguan",
    "guangdong_shenzhen",
    "guangdong_sihui",
    "guangdong_yangjiang",
    "guangdong_yingde",
    "guangdong_yunfu",
    "guangdong_zhanjiang",
    "guangdong_zhaoqing",
    "guangdong_zhongshan",
    "guangdong_zhuhai",
    "guangdong_qingyuan"
  ],
  "new":
    [
    "jiangsu_dongtai",
    "jiangsu_yancheng",
    "liaoning_beizhen",
    "liaoning_dalian",
    "neimenggu_alashan",
    "neimenggu_neimenggu"
    ]
}






zl_diqu_dict={
  "qycg": [
    "qycg_b2b_10086_cn",
    "qycg_b2bcoal_crp_net_cn",
    "qycg_baowu_ouyeelbuy_com",
    "qycg_bid_ansteel_cn",
    "qycg_bid_powerchina_cn",
    "qycg_bidding_ceiec_com_cn",
    "qycg_bidding_crmsc_com_cn",
    "qycg_buy_cnooc_com_cn",
    "qycg_caigou_ceair_com",
    "qycg_csbidding_csair_com",
    "qycg_dfqcgs_dlzb_com",
    "qycg_dzzb_ciesco_com_cn",
    "qycg_ebid_aecc_mall_com",
    "qycg_ec1_mcc_com_cn",
    "qycg_ec_ccccltd_cn",
    "qycg_ec_ceec_net_cn",
    "qycg_ec_chalieco_com",
    "qycg_ecp_cgnpc_com_cn",
    "qycg_ecp_sgcc_com_cn",
    "qycg_epp_ctg_com_cn",
    "qycg_eps_sdic_com_cn",
    "qycg_etp_fawiec_com",
    "qycg_fwgs_sinograin_com_cn",
    "qycg_gs_coscoshipping_com",
    "qycg_jzcg_cfhi_com",
    "qycg_srm_crland_com_cn",
    "qycg_syhggs_dlzb_com",
    "qycg_sytrq_dlzb_com",
    "qycg_thzb_crsc_cn",
    "qycg_uat_ec_chng_com_cn",
    "qycg_www_bidding_csg_cn",
    "qycg_www_cdt_eb_com",
    "qycg_www_cgdcbidding_com",
    "qycg_www_chdtp_com",
    "qycg_www_china_tender_com_cn",
    "qycg_www_chinabidding_com",
    "qycg_www_chinabidding_com_total",
    "qycg_www_cnbmtendering_com",
    "qycg_www_cnpcbidding_com",
    "qycg_www_crpsz_com",
    "qycg_www_dlswzb_com",
    "qycg_www_dlzb_com",
    "qycg_www_dlzb_com_c1608",
    "qycg_www_dlztb_com",
    "qycg_www_gmgitc_com",
    "qycg_www_mgzbzx_com",
    "qycg_www_namkwong_com_mo",
    "qycg_www_ngecc_com",
    "qycg_www_qhbidding_com",
    "qycg_www_sinochemitc_com",
    "qycg_www_sztc_com",
    "qycg_www_wiscobidding_com_cn",
    "qycg_www_ykjtzb_com",
    "qycg_www_zeec_cn",
    "qycg_www_zmzb_com",
    "qycg_wzcgzs_95306_cn",
    "qycg_ysky_dlzb_com",
    "qycg_zb_crlintex_com",
    "qycg_zgdxjt_dlzb_com",
    "qycg_zgdzxx_dlzb_com",
    "qycg_zghkgy_dlzb_com",
    "qycg_zghkyl_dlzb_com",
    "qycg_zgyy_dlzb_com",
    "qycg_zljt_dlzb_com"
  ],
  "zfcg": [
    "zfcg_anhui_anqing",
    "zfcg_anhui_huainan",
    "zfcg_anhui_luan",
    "zfcg_anhui_shenghui",
    "zfcg_beijing_beijing",
    "zfcg_fujian_fuzhou",
    "zfcg_fujian_longyan",
    "zfcg_fujian_nanping",
    "zfcg_fujian_nanping1",
    "zfcg_fujian_ningde",
    "zfcg_fujian_putian",
    "zfcg_fujian_quanzhou",
    "zfcg_fujian_sanming",
    "zfcg_fujian_sanming1",
    "zfcg_fujian_xiamen",
    "zfcg_fujian_zhangzhou",
    "zfcg_gansu_jinchang",
    "zfcg_gansu_shenghui",
    "zfcg_guangdong_dongguan",
    "zfcg_guangdong_guangzhou",
    "zfcg_guangdong_shenghui",
    "zfcg_guangdong_shenzhen",
    "zfcg_guangxi_baise",
    "zfcg_guangxi_fangchenggang",
    "zfcg_guangxi_guigang",
    "zfcg_guangxi_guilin",
    "zfcg_guangxi_liuzhou",
    "zfcg_guangxi_nanning",
    "zfcg_guangxi_qinzhou",
    "zfcg_guangxi_shenghui",
    "zfcg_guangxi_wuzhou",
    "zfcg_guizhou_shenghui",
    "zfcg_hainan_haikou",
    "zfcg_hainan_sanya",
    "zfcg_hainan_shenghui",
    "zfcg_hainan_wenchang",
    "zfcg_hebei_shenghui",
    "zfcg_heilongjiang_shenghui",
    "zfcg_heilongjiang_yichun",
    "zfcg_henan_anyang",
    "zfcg_henan_hebi",
    "zfcg_henan_henan",
    "zfcg_henan_jiaozuo",
    "zfcg_henan_kaifeng",
    "zfcg_henan_luohe",
    "zfcg_henan_luoyang",
    "zfcg_henan_nanyang",
    "zfcg_henan_pingdingshan",
    "zfcg_henan_puyang",
    "zfcg_henan_sanmenxia",
    "zfcg_henan_shangqiu",
    "zfcg_henan_xinxiang",
    "zfcg_henan_xinyang",
    "zfcg_henan_xuchang",
    "zfcg_henan_zhengzhou",
    "zfcg_henan_zhoukou",
    "zfcg_henan_zhumadian",
    "zfcg_hubei_ezhou",
    "zfcg_hubei_huanggang",
    "zfcg_hubei_hubei",
    "zfcg_hubei_jingmen",
    "zfcg_hubei_shiyan",
    "zfcg_hubei_wuhan",
    "zfcg_hunan_changde",
    "zfcg_hunan_changsha",
    "zfcg_hunan_changsha2",
    "zfcg_hunan_chenzhou",
    "zfcg_hunan_hengyang",
    "zfcg_hunan_hunan",
    "zfcg_hunan_loudi",
    "zfcg_hunan_xiangtan",
    "zfcg_hunan_yiyang",
    "zfcg_hunan_yueyang",
    "zfcg_hunan_zhangjiajie",
    "zfcg_hunan_zhuzhou",
    "zfcg_jiangsu_changzhou",
    "zfcg_jiangsu_huaian",
    "zfcg_jiangsu_lianyungang",
    "zfcg_jiangsu_nanjing",
    "zfcg_jiangsu_nantong",
    "zfcg_jiangsu_shenghui",
    "zfcg_jiangsu_suqian",
    "zfcg_jiangsu_suzhou",
    "zfcg_jiangsu_taizhou",
    "zfcg_jiangsu_wuxi",
    "zfcg_jiangsu_xuzhou",
    "zfcg_jiangsu_yangzhou",
    "zfcg_jiangxi_jian",
    "zfcg_jiangxi_jiangxi",
    "zfcg_jiangxi_nanchang",
    "zfcg_jiangxi_pingxiang",
    "zfcg_jilin_changchun",
    "zfcg_jilin_jilin",
    "zfcg_jilin_shenghui",
    "zfcg_liaoning_chaoyang",
    "zfcg_liaoning_dalian",
    "zfcg_liaoning_shenyang",
    "zfcg_liaoning_wafangdian",
    "zfcg_neimenggu_baotou",
    "zfcg_neimenggu_eerduosi",
    "zfcg_neimenggu_huhehaote",
    "zfcg_neimenggu_shenghui",
    "zfcg_neimenggu_tongliao",
    "zfcg_ningxia_shenghui",
    "zfcg_qinghai_shenghui",
    "zfcg_shandong_dezhou",
    "zfcg_shandong_dongying",
    "zfcg_shandong_laiwu",
    "zfcg_shandong_liaocheng",
    "zfcg_shandong_linyi",
    "zfcg_shandong_qingdao",
    "zfcg_shandong_rizhao",
    "zfcg_shandong_shandong",
    "zfcg_shandong_weihai",
    "zfcg_shandong_yantai",
    "zfcg_shanxi1_changzhi",
    "zfcg_shanxi1_shenghui",
    "zfcg_shanxi1_taiyuan",
    "zfcg_shanxi1_yuncheng",
    "zfcg_shanxi_shenghui",
    "zfcg_sichuan_mianyang",
    "zfcg_sichuan_shenghui",
    "zfcg_tianjin_tianjin",
    "zfcg_xinjiang_akesu",
    "zfcg_xinjiang_alashankou",
    "zfcg_xinjiang_changji",
    "zfcg_xinjiang_hetian",
    "zfcg_xinjiang_kashi",
    "zfcg_xinjiang_kelamayi",
    "zfcg_xinjiang_shenghui",
    "zfcg_xinjiang_shenghui2",
    "zfcg_xinjiang_tacheng",
    "zfcg_xinjiang_tulufan",
    "zfcg_xinjiang_wulumuqi",
    "zfcg_xinjiang_yining",
    "zfcg_xizang_shenghui",
    "zfcg_yunnan_shenghui",
    "zfcg_yunnan_yuxi",
    "zfcg_zhejiang_hangzhou",
    "zfcg_zhejiang_ningbo",
    "zfcg_zhejiang_shenghui",
    "zfcg_zhejiang_wenzhou",
    "zfcg_hebei_chengde",
    "zfcg_hebei_tangshan",
    "zfcg_jiangsu_nanjing",
    "zfcg_chongqing_chongqing",
    "zfcg_hubei_wuhan2",

    "zfcg_guangdong_shantou",
    "zfcg_guangdong_zhongshan",
    "zfcg_guizhou_tongren",
    "zfcg_ningxia_yinchuan",
    "zfcg_shanxi_hanzhong",
    "zfcg_shanxi_shangluo",
    "zfcg_shanxi_xian",
    "zfcg_xinjiang_changji2",
    "zfcg_xizang_shannan",
  ],
  "gcjs": [
    "gcjs_anhui_huainan",
    "gcjs_anhui_xuancheng",
    "gcjs_fujian_fuqing",
    "gcjs_fujian_fuzhou",
    "gcjs_fujian_quanzhou",
    "gcjs_fujian_sanming",
    "gcjs_fujian_zhangzhou",
    "gcjs_gansu_shenghui",
    "gcjs_guangdong_shantou",
    "gcjs_guangdong_shaoguan",
    "gcjs_guangdong_shenghui",
    "gcjs_guangdong_shenzhen",
    "gcjs_guangdong_yangjiang",
    "gcjs_guangdong_yunfu",
    "gcjs_guangdong_zhongshan",
    "gcjs_guangxi_baise",
    "gcjs_guangxi_beihai",
    "gcjs_guangxi_fangchenggang",
    "gcjs_guangxi_qinzhou",
    "gcjs_guangxi_shenghui",
    "gcjs_guizhou_shenghui",
    "gcjs_hebei_langfang",
    "gcjs_hebei_shenghui",
    "gcjs_hebei_shijiazhuang",
    "gcjs_hebei_tangshan",
    "gcjs_hebei_xingtai",
    "gcjs_heilongjiang_haerbin",
    "gcjs_heilongjiang_qqhaer",
    "gcjs_heilongjiang_shenghui",
    "gcjs_henan_kaifeng",
    "gcjs_henan_pingdingshan",
    "gcjs_henan_puyang",
    "gcjs_henan_sanmenxia",
    "gcjs_henan_zhengzhou",
    "gcjs_hunan_changsha1",
    "gcjs_hunan_changsha2",
    "gcjs_hunan_huaihua",
    "gcjs_hunan_loudi",
    "gcjs_hunan_shaoyang",
    "gcjs_hunan_shenghui",
    "gcjs_hunan_wugang",
    "gcjs_hunan_yueyang",
    "gcjs_jiangsu_changzhou",
    "gcjs_jiangsu_nanjing",
    "gcjs_jiangsu_nantong",
    "gcjs_jiangsu_shenghui",
    "gcjs_jiangsu_yangzhou",
    "gcjs_jiangxi_jiujiang",
    "gcjs_jiangxi_nanchang",
    "gcjs_jiangxi_pingxiang",
    "gcjs_jiangxi_shangrao",
    "gcjs_jilin_changchun",
    "gcjs_jilin_jilin",
    "gcjs_jilin_shenghui",
    "gcjs_jilin_siping",
    "gcjs_jilin_tonghua",
    "gcjs_liaoning_dalian",
    "gcjs_liaoning_shenghui",
    "gcjs_liaoning_shenyang",
    "gcjs_neimenggu_shenghui",
    "gcjs_ningxia_shenghui",
    "gcjs_shandong_dongying",
    "gcjs_shandong_heze",
    "gcjs_shandong_jinan",
    "gcjs_shandong_linyi",
    "gcjs_shandong_rizhao",
    "gcjs_shandong_shenghui",
    "gcjs_shandong_zaozhuang",
    "gcjs_shanxi1_datong",
    "gcjs_shanxi1_linfen",
    "gcjs_shanxi1_taiyuan",
    "gcjs_shanxi1_xinzhou",
    "gcjs_shanxi_ankang",
    "gcjs_shanxi_baoji",
    "gcjs_shanxi_hanzhong",
    "gcjs_shanxi_xian",
    "gcjs_shanxi_xianyang",
    "gcjs_shanxi_yanan",
    "gcjs_shanxi_yulin",
    "gcjs_sichuan_bazhong",
    "gcjs_sichuan_chengdu",
    "gcjs_sichuan_mianyang",
    "gcjs_sichuan_shenghui",
    "gcjs_sichuan_zigong",
    "gcjs_xinjiang_akesu",
    "gcjs_xinjiang_atushi",
    "gcjs_xinjiang_bole",
    "gcjs_xinjiang_changji",
    "gcjs_xinjiang_hami",
    "gcjs_xinjiang_kashi",
    "gcjs_xinjiang_shenghui",
    "gcjs_xinjiang_tacheng",
    "gcjs_xinjiang_wulumuqi",
    "gcjs_xinjiang_yining",
    "gcjs_zhejiang_hangzhou",
    "gcjs_zhejiang_huzhou",
    "gcjs_zhejiang_shenghui",
    "gcjs_zhejiang_zhoushan",
    "gcjs_shanxi1_changzhi",
    "gcjs_shanxi1_xinzhou2",
    "gcjs_zhejiang_jinhua",
    "gcjs_zhixiashi_beijing",
    "gcjs_zhixiashi_chongqing",
    "gcjs_zhixiashi_tianjin",
    "gcjs_zhixiashi_shanghai",
    "gcjs_shanxi_shenghui"
  ]
}



zl_shenpi_dict={"zlshenpi":
['zlshenpi_anhuisheng', 'zlshenpi_beijingshi', 'zlshenpi_fujiansheng', 'zlshenpi_gansusheng', 'zlshenpi_guangdongsheng', 'zlshenpi_hebeisheng', 'zlshenpi_heilongjiangsheng'
, 'zlshenpi_henansheng', 'zlshenpi_hubeisheng', 'zlshenpi_hunansheng', 'zlshenpi_jiangxisheng', 'zlshenpi_jilinsheng', 'zlshenpi_neimenggusheng', 'zlshenpi_ningxiasheng'
, 'zlshenpi_qinghaisheng', 'zlshenpi_shanxisheng', 'zlshenpi_shanxisheng1', 'zlshenpi_sichuansheng', 'zlshenpi_xiamenshi', 'zlshenpi_xinjiangsheng', 'zlshenpi_yunnansheng'
, 'zlshenpi_zhejiangsheng']
}





zlest_diqu_dict={
    
    "zlest":['zlest_anhuisheng', 'zlest_anhuisheng1', 'zlest_enshizhou', 'zlest_ezhoushi', 'zlest_guangdongsheng', 'zlest_guangxisheng', 'zlest_guizhousheng'
    , 'zlest_henansheng', 'zlest_hubeisheng', 'zlest_hunansheng', 'zlest_jiangxisheng', 'zlest_neimenggu'
    , 'zlest_qinghaisheng', 'zlest_qinghaisheng1', 'zlest_shaoyangxian', 'zlest_shenzhenshi', 'zlest_sichuansheng', 'zlest_wuxishi', 'zlest_zhejiangsheng']
}



zlsys_diqu_dict={
    
    "zlsys"['zlsys_guangdongsheng_shenzhenshi', 'zlsys_sichuan_suiningshi', 'zlsys_sichuan_yaanshi', 'zlsys_sichuan_yibinshi', 'zlsys_yunnan_baoshanshi', 
    'zlsys_yunnan_chuxiongzhou', 'zlsys_yunnan_dalizhou', 'zlsys_yunnan_dehongzhou', 'zlsys_yunnan_diqingzhou', 'zlsys_yunnan_honghezhou', 'zlsys_yunnan_kunmingshi', 
    'zlsys_yunnan_lijiangshi', 'zlsys_yunnan_lincangshi', 'zlsys_yunnan_nujiangzhou', 'zlsys_yunnan_puershi', 'zlsys_yunnan_qujingshi', 'zlsys_yunnan_tengchongshi', 
    'zlsys_yunnan_wenshanzhou', 'zlsys_yunnan_xishuangbanna', 'zlsys_yunnan_yunnansheng', 'zlsys_yunnan_yuxishi', 'zlsys_yunnan_zhaotongshi']
}