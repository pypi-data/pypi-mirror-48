...
def out_t_gg_all(quyu,dir,conp):
    ...
    arr=quyu.split("_")
    s1,s2=arr[0],'_'.join(arr[1:])
    sql="""select  guid ,gg_name, fabu_time,coalesce(href,'暂无') as href ,ggtype,jytype,diqu, quyu,info
...
def update_t_gg(quyu,conp):
    ...
    SELECT 
    distinct on(guid)
    guid,gg_name,cdc.fabu_time,gg_href,ggtype,jytype,diqu,quyu,info,now()::timestamp(0) as create_time,page
