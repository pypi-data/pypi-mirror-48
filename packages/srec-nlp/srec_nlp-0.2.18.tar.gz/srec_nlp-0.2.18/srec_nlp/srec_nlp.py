...
try:
	...
	class FastText(NLP):
		...
		@staticmethod
		def wrap_init_model(fnc):
			def wrapper(self, *vargs, **kwargs):
				if getattr(self, 'model', None) is None:
					self.model = self._load_model(self.path)
				fnc(self, *vargs, **kwargs)
		...
		def sentiment(self, text: Union[str, List[str], IOBase, pathlib.Path]) -> Query:
			...
			:return: Query
			"""
			model = FastText._load_model(self.path)
			labels, scores = model.predict(text, 1)
			label = labels[0]
			score = scores[0]
			if label == "__label__1":
				score = 1 - score
