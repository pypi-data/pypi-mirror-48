...
class Assembly(MgModel):
    ...
    def __init__(self, mgid, **data):
        ...
        self.hmm_searches = None
        if 'HmmSearches' in self.d:
            self.hmm_searches = []
            for m in self.d['CrisprConnections']:
                self.hmm_searches.append(HmmSearch(self.mgid, db=self.db, check=self.check, **m))
    ...
    def download_genes(self, dir_to_dl=os.getcwd(), overwrite=True, dry_run=False, raise_error=True):
        if self.prodigal:
            genes = f'{self.prodigal.protein_file}.genes'
        else:
            if raise_error:
                e = f'No filtered contigs, prodigal has not been run'
                raise ValueError(e)
        ...
        return download_file(genes, dir_to_dl, dry_run=dry_run, overwrite=overwrite)
