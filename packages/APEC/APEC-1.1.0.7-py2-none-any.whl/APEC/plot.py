...
def correlation(project,
    ...
    cluster:        Clustering algorithm used in clustering.cluster_byXXX(), can be 'louvain' or 'KNN', default='louvain'.
    method:         method parameter for scipy funcion pdist, can be 'ward', 'average', 'single', default='ward'.
    metric:         metric parameter for scipy funcion pdist, default='euclidean', see scipy website for more details.
    ...
    if cell_label=='notes':
        ...
        cells_file = project+'/result/'+cluster+'_cluster_by_'+matrix_type+'.csv'
    cells_df = pandas.read_csv(cells_file, sep='\t', index_col=0)
...
#    corr = numpy.clip(corr, clip[0], clip[1])
