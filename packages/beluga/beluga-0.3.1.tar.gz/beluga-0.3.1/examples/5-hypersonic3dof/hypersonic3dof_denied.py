...
guess_maker = beluga.guess_generator('auto', start=[20000,0,2000,(-90)*pi/180], direction='forward',
                                     costate_guess = -0.1)
...
ocp_2.constant('epsilon1', 1, 'm^2/s^2')
ocp_2.constant('denied_rad', 0*pi/180, 'rad')
ocp_2.constant('upper_rad', 3*pi/180, 'rad')
ocp_2.constant('xmid', 1.5*pi/180, 'rad')
ocp_2.constant('ymid', 0.5*pi/180, 'rad')

ocp_2.constraints().path('sqrt((theta - xmid)**2 + (phi - ymid)**2)', 'rad', lower='denied_rad', upper='upper_rad', activator='epsilon1')
...
# continuation_steps_2.add_step('bisection').num_cases(5) \
#     .const('phi_f', -0.1*pi/180)

# continuation_steps_2.add_step('bisection').num_cases(10) \
#     .const('denied_rad', 0.45*pi/180)

# continuation_steps_2.add_step('bisection').num_cases(10) \
#     .const('phi_f', 0.25*pi/180)

# continuation_steps_2.add_step('bisection').num_cases(10) \
#     .const('phi_f', 0.48*pi/180)

cont_3dof = beluga.solve(ocp=ocp_2,
             method='traditional',
             bvp_algorithm=bvp_solver_2,
             steps=continuation_steps_2,
             guess_generator=guess_maker_2, autoscale=True)
...
for trajectory in final_continuation:
    plt.plot(trajectory.y[:,1], trajectory.y[:,2])
    plt.plot((trajectory.aux['const']['xmid']+trajectory.aux['const']['denied_rad']*np.cos(T)), (trajectory.aux['const']['ymid']+trajectory.aux['const']['denied_rad']*np.sin(T)))
...
trajectory = final_continuation[-1]

plt.plot(trajectory.t, np.sqrt((trajectory.aux['const']['xmid'] - trajectory.y[:,1])**2 + (trajectory.aux['const']['ymid'] - trajectory.y[:,2])**2))
plt.plot(trajectory.t, np.linspace(trajectory.aux['const']['denied_rad'], trajectory.aux['const']['denied_rad'],num=len(trajectory.t)), color='k', linestyle='--')
plt.show()
