...
def main():
    ...
    if bucket == "none":
        ...
        try:
            cmd = "which tree"
            tree_exists = subprocess.check_output(cmd, shell=True).decode("utf-8").strip()
            if "tree" not in tree_exists:
            ...
            dl_path = home+"/delete-later-"+today+"/"+bucket
