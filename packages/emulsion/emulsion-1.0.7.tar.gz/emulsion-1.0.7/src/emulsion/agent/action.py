...
class SetVarAction(ValueAction):
    ...
    def execute_action(self, unit, agents=None, **others):
        ...
        for agent in agents:
            ...
            # already a statevar, retrieve its value from the agent
            if self.parameter in agent.statevars:
                value = agent.statevars[self.parameter]
            # otherwise get its value through the model
            else:
...
class MessageAction(StringAction):
    ...
    def execute_action(self, unit, agents=None, **others):
        ...
        for agent in agents:
            ...
            print("${},@{},{},{}".format(agent.statevars.simu_id, agent.statevars.step, agent, message))
...
class LogVarsAction(StringAction):
    ...
    message was produced, the agent speaking, and the values of
    variables (in the order they were defined in the action).
    ...
    def execute_action(self, unit, agents=None, **others):
        ...
        if agents is None:
            agents = [unit]
        for agent in agents:
            message = '${},@{},{}'.format(agent.statevars.simu_id, agent.statevars.step, agent)
