from autoaf.core import model_mapper as mm
from autocti.model import arctic_params as ap
import workspace_jam.plotting.requirements as req
import workspace_jam.plotting.requirements.requirement_converter as convert

import matplotlib.pyplot as plt
import numpy as np

model_mapper = af.ModelMapper(serial=ap.SerialThreeSpecies)
...
most_likely_lr, most_probable_lr, lower_limits_lr, upper_limits_lr, lower_error_lr, upper_error_lr, error_pecision_lr = \
    req.setup_multinest_results(model_mapper=model_mapper, input_model=input_model, sigma_limit=sigma_limit,
                                name='Results/Serial_Tight_lr/')

model_rho_lr = np.array([most_probable_lr[0], most_probable_lr[1]])
model_tau_lr = np.array([most_probable_lr[2], most_probable_lr[3]])
delta_ellipticity_model_lr = convert.convert_to_ellipticity(rho=model_rho_lr, tau=model_tau_lr)

upper_rho_lr = np.array([upper_limits_lr[0], upper_limits_lr[1]])
upper_tau_lr = np.array([upper_limits_lr[2], upper_limits_lr[3]])
delta_ellipticity_upper_lr = convert.convert_to_ellipticity(rho=upper_rho_lr, tau=upper_tau_lr)

lower_rho_lr = np.array([lower_limits_lr[0], lower_limits_lr[1]])
lower_tau_lr = np.array([lower_limits_lr[2], lower_limits_lr[3]])
...
most_likely_mr, most_probable_mr, lower_limits_mr, upper_limits_mr, lower_error_mr, upper_error_mr, error_pecision_mr = \
    req.setup_multinest_results(model_mapper=model_mapper, input_model=input_model, sigma_limit=sigma_limit,
                                name='Results/Serial_Tight_mr/')

model_rho_mr = np.array([most_probable_mr[0], most_probable_mr[1]])
model_tau_mr = np.array([most_probable_mr[2], most_probable_mr[3]])
delta_ellipticity_model_mr = convert.convert_to_ellipticity(rho=model_rho_mr, tau=model_tau_mr)

upper_rho_mr = np.array([upper_limits_mr[0], upper_limits_mr[1]])
upper_tau_mr = np.array([upper_limits_mr[2], upper_limits_mr[3]])
delta_ellipticity_upper_mr = convert.convert_to_ellipticity(rho=upper_rho_mr, tau=upper_tau_mr)

lower_rho_mr = np.array([lower_limits_mr[0], lower_limits_mr[1]])
lower_tau_mr = np.array([lower_limits_mr[2], lower_limits_mr[3]])
...
most_likely_hr, most_probable_hr, lower_limits_hr, upper_limits_hr, lower_error_hr, upper_error_hr, error_pecision_hr = \
    req.setup_multinest_results(model_mapper=model_mapper, input_model=input_model, sigma_limit=sigma_limit,
                                name='Results/Serial_Tight_hr/')

model_rho_hr = np.array([most_probable_hr[0], most_probable_hr[1]])
model_tau_hr = np.array([most_probable_hr[2], most_probable_hr[3]])
delta_ellipticity_model_hr = convert.convert_to_ellipticity(rho=model_rho_hr, tau=model_tau_hr)

upper_rho_hr = np.array([upper_limits_hr[0], upper_limits_hr[1]])
upper_tau_hr = np.array([upper_limits_hr[2], upper_limits_hr[3]])
delta_ellipticity_upper_hr = convert.convert_to_ellipticity(rho=upper_rho_hr, tau=upper_tau_hr)

lower_rho_hr = np.array([lower_limits_hr[0], lower_limits_hr[1]])
lower_tau_hr = np.array([lower_limits_hr[2], lower_limits_hr[3]])
...
most_likely_x2, most_probable_x2, lower_limits_x2, upper_limits_x2, lower_error_x2, upper_error_x2, error_pecision_x2 = \
    req.setup_multinest_results(model_mapper=model_mapper, input_model=input_model, sigma_limit=sigma_limit,
                                name='Results/Serial_Tight_x2/')

model_rho_x2 = np.array([most_probable_x2[0], most_probable_x2[1]])
model_tau_x2 = np.array([most_probable_x2[2], most_probable_x2[3]])
delta_ellipticity_model_x2 = convert.convert_to_ellipticity(rho=model_rho_x2, tau=model_tau_x2)

upper_rho_x2 = np.array([upper_limits_x2[0], upper_limits_x2[1]])
upper_tau_x2 = np.array([upper_limits_x2[2], upper_limits_x2[3]])
delta_ellipticity_upper_x2 = convert.convert_to_ellipticity(rho=upper_rho_x2, tau=upper_tau_x2)

lower_rho_x2 = np.array([lower_limits_x2[0], lower_limits_x2[1]])
lower_tau_x2 = np.array([lower_limits_x2[2], lower_limits_x2[3]])
