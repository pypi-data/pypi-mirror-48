...
model_rho = np.array([most_probable[2], most_probable[3], most_probable[4]])
model_tau = np.array([most_probable[5], most_probable[6], most_probable[7]])
delta_ellipticity_model = convert.convert_to_ellipticity(rho=model_rho, tau=model_tau)

upper_rho = np.array([upper_limits[2], upper_limits[3], upper_limits[4]])
upper_tau = np.array([upper_limits[5], upper_limits[6], upper_limits[7]])
delta_ellipticity_upper = convert.convert_to_ellipticity(rho=upper_rho, tau=upper_tau)

lower_rho = np.array([lower_limits[2], lower_limits[3], lower_limits[4]])
lower_tau = np.array([lower_limits[5], lower_limits[6], lower_limits[7]])
...
print('Model most likely')
print(delta_ellipticity_model_ml)
print(np.sum(delta_ellipticity_model_ml))
print()
...
print(np.abs(np.sum(delta_ellipticity_model_ml) - np.sum(delta_ellipticity_true)))
...
print(np.abs(np.sum(delta_ellipticity_model_ml) - np.sum(delta_ellipticity_true)) / (1.1e-4))
