...
from azureml.explain.model.common.constants import ExplainParams, ExplainType
        ...
        def generate_predict_function():
            if self.model is not None and classification and not self.for_classifier_use_predict_proba:
                def model_predict_func(dataset):
                    return self.model.predict(typed_wrapper_func(dataset))
                return model_predict_func
            else:
                def user_defined_or_default_predict_func(dataset):
                    return self.function(typed_wrapper_func(dataset))
