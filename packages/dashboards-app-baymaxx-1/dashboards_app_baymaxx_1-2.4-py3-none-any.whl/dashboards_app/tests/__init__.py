...
from dashboards_app.cms_apps import Dashboards_appApp
...
class Dashboards_appTestsMixin(object):
    ...
    def create_dashboard(self, content=None, **kwargs):
        try:
        ...
        dashboard = Dashboard.objects.create(**fields)
        # save again to calculate dashboard search_data.
        dashboard.save()

        if content:
            api.add_plugin(dashboard.content, 'TextPlugin',
                           self.language, body=content)
        return dashboard

    def create_tagged_dashboards(self, num_dashboards=3, tags=('tag1', 'tag2'),
                               **kwargs):
        """Create num_dashboards Dashboards for each tag"""
        dashboards = {}
        for tag_name in tags:
            ...
            for _ in range(num_dashboards):
                ...
                tagged_dashboards.append(dashboard)
            tag_slug = tagged_dashboards[0].tags.slugs()[0]
            dashboards[tag_slug] = tagged_dashboards
        return dashboards
    ...
    def setUp(self):
        ...
        self.app_config = Dashboards_appConfig.objects.language(self.language).create(
            app_title='news_blog',
            namespace='NBNS',
            paginate_by=15,
        )
...
class Dashboards_appTestCase(CleanUpMixin, Dashboards_appTestsMixin, CMSTestCase):
    apphook_object = Dashboards_appApp
    pass


class Dashboards_appTransactionTestCase(CleanUpMixin,
                                  Dashboards_appTestsMixin,
