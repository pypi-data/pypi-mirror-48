...
class CombPeak(DispatchableComponent):
    ...
    def __init__(
        self, fuel, min_ratio=0.6, eff_min=0.3, eff_max=0.4,
    ...
    def _calc_pow(self, pow_req, hr):
        ...
        pow_base = self.min_ratio*self.size
        cond = [
            pow_req < 0,  # cannot go below min loading
            (pow_req >= 0)*(pow_req < self.size-pow_base),
            pow_req >= self.size-pow_base  # cannot go beyond max loading
        ]
        choice = [0, pow_req, self.size-pow_base]
    ...
    def _rec_pow(self, pow_rec, hr):
        ...
        pow_base = self.min_ratio*self.size
        fl_use = (
            self.fl_slp*(pow_rec+pow_base) +
            self.fl_int*self.size*(pow_rec != 0)
        )
        self.fuel.rec_fl(fl_use, hr)
    ...
    def _cost_calc(self, yr_proj, infl):
        ...
        # adjust energy usage
        self.use_tot = 8760
        self.enr_tot += 8760*self.size*self.min_ratio
        ...
        self.fuel.cost_calc(yr_proj, infl)
    ...
    def _update_init(self):
        ...
        self.fuel.set_num(self.num_case)
