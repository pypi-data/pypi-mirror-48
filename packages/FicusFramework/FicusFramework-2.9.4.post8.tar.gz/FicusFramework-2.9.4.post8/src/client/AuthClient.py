...
def oauth_access_token(client_id, client_secret):
    ...
    try:
        r = do_service(f"/oauth/token?grant_type=client_credentials&scope=select&client_id={client_id}&client_secret={client_secret}"
...
def oauth_jwt_token(user_name, user_password):
    ...
    try:
        r = do_service("/user/login", method="POST",return_type="str",data={"username": user_name, "password": user_password}, auth=None)
