...
class CodeElementGraphDataBuilder:
    ...
    def build_method_code_use_constant_field_relation(self):
        has_field_relation = CodeEntityRelationCategory.to_str(
        ...
        for node_id in self.graph_data.get_node_ids_by_label(
            ...
            word_lemma = node_properties["qualified_name"].split(" ")
            field_name = word_lemma[1]
            if len(re.findall(self.CHECK_CONSTANT_FILE_PATTERN, field_name)) == 0:

                class_node_relation_nodes = self.graph_data.get_relations(end_id=node_id,
                                                                          relation_type=has_field_relation)

                for start_class_node_id, relation_type, end_node_id in class_node_relation_nodes:
                    class_node_info = self.graph_data.get_node_info_dict(node_id=start_class_node_id)
                    class_qualified_name = class_node_info[GraphData.DEFAULT_KEY_NODE_PROPERTIES]["qualified_name"]

                    alias = class_qualified_name + "." + field_name
                    if alias not in node_properties[PropertyConstant.ALIAS]:
                        node_properties[PropertyConstant.ALIAS].append(alias)

                    simple_constant_name = class_qualified_name.split(".")[-1] + "." + field_name
                    if simple_constant_name not in node_properties[PropertyConstant.ALIAS]:
                        node_properties[PropertyConstant.ALIAS].append(simple_constant_name)
                    constant_nodes.append((node_id, simple_constant_name))
        ...
        for node_id in self.graph_data.get_node_ids_by_label(
            ...
            if "code" not in node_properties or not node_properties["code"]:
                continue

            code_text = node_properties["code"]
