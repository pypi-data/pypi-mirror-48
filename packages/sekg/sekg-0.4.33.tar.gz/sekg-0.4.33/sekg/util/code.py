...
class CodeElementNameUtil:
    ...
    def generate_aliases(self, qualified_name, include_simple_parent_name=False, **arg):
        if not qualified_name:
            return []

        simple_name = self.simplify(qualified_name)
        of_name = self.deal_with_adj(simple_name)
        ...
        part_abbreviation = []
        if "vocabulary" in arg:
            part_abbreviation = self.get_part_abbreviation(separate_name, arg["vocabulary"])
        name_list = [qualified_name, simple_name, of_name, separate_name, name_deal_number, full_abbreviation_name,
                     abbreviation_name].extend(part_abbreviation)
    ...
    def deal_with_adj(self, name):
        ...
        if len(split_name) == 2:
            return (("%s of %s") % (split_name[1], split_name[0]))
