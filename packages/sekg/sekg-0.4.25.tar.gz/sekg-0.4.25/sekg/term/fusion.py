...
from kgtools.annotation import Parallel, Cache
...
class Fusion:
    ...
    def check_synonym(self, long_term, short_term, max_dis=2, threshold=1 / 4):
        long_name = self.lemmatizer(long_term, 'NOUN')[0]
        short_name = self.lemmatizer(short_term, 'NOUN')[0]
    ...
    def check_abbr(self, long_term, short_term, stopwords=None):
        ...
        def __check_phrase_word(phrase, word, stopwords):
            ...
            if len(long_words) == 1:
                ...
                for pair in all_pairs:
                    ...
                    if long_word[0] == segment[0] or (len(long_word) > 1 and long_word[0] in set("aeiou") and segment[0] == long_word[1]):
        ...
        long_name = self.lemmatizer(long_term, 'NOUN')[0]
        short_name = self.lemmatizer(short_term, 'NOUN')[0]
        long_name, short_name = long_name.lower(), short_name.lower()
        # print(long_name, short_name)
        # print(textdistance.lcsseq(short_name, long_name))
    ...
    def merge_abbrs(self, abbr_relations, synonyms, threshold=0.5):
        ...
        for synset in synonyms:
            for c in synset.terms:
                abbr_synonyms[c] = synset

        abbr_fullname = {}
        for start, end in abbr_relations:
            if start not in abbr_synonyms:
                abbr_synonyms[start] = Synset(set([start]))
            if end not in abbr_synonyms:
                abbr_synonyms[end] = Synset(set([end]))
            start_synset = abbr_synonyms[start]
            end_synset = abbr_synonyms[end]
            if start_synset not in abbr_fullname:
                abbr_fullname[start_synset] = set()
            abbr_fullname[start_synset].add(end_synset)

        abbrs = set()
        for abbr, candidates in abbr_fullname.items():
            for full_name in candidates:
                abbrs.add(abbr + full_name)
        return abbrs

    def fuse(self, terms, stopwords=None):
        ...
        abbrs = self.merge_abbrs(abbr_relations, synonyms)
        # print("syn: ", [str(synset) for synset in synonyms])
        # print("abbr:", [str(synset) for synset in abbrs])
        ...
        def __merge(queue):
            ...
            if done:
                ...
                return __merge(synsets)
        queue = list(synonyms | abbrs)
        synsets = __merge(queue)

        processed_terms = set()
        for synset in synsets:
            processed_terms.update(synset.terms)

        for term in terms:
            if term not in processed_terms:
                synsets.append(Synset(set([term])))
        return set(synsets)


if __name__ == "__main__":
    fusion = Fusion()
    # Layer activation, Random exponential
    rs = fusion.check_synonym("Embedding Layers", "Embedding layer")
    print(rs)
    # pair, synonyms = fusion.synset()
    # with Path("synonyms_fusion/pair.json").open("w", encoding="utf-8") as f:
    #     json.dump(pair, f, indent=4)
    # with Path("synonyms_fusion/synonyms.json").open("w", encoding="utf-8") as f:
    #     json.dump(synonyms, f, indent=4)
    # fusion.synset(["VAE", "VariationalAutoencoder"])
