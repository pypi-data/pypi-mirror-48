...
class DataSet(object):
    ...
    def create(
        ...
        err, result = self.queryer.insert_data_dataset(name=name, metadata=turn_metadata(metadata))
        return err, result['data']['insert_data_dataset']['returning'][0] if not err else None
    ...
    def update(
        ...
        err, result = self.queryer.update_data_dataset(where=where, name=name, metadata=turn_metadata(metadata))
        return err, result
    ...
    def add_xs(
        ...
        for resource in resources:
            err, result = self.queryer.insert_data_dataset_xx(model_name=model_name, dataset_id=dataset['id'], id=resource['id'])
            errs.append(err)
    ...
    def remove_xs(
        ...
        err, result = self.queryer.delete_data_dataset_xx(model_name=model_name, dataset_id=dataset['id'], ids=[resource['id'] for resource in resources])
        return err, result
