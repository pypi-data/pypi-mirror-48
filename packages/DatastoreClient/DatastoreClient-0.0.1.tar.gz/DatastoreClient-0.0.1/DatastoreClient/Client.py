...
class Client():
    ...
    def __init__(
        ...
        minio_secure = True,
        graphql_path="/datastore/graphql/",
        download_path='/home/vol',
        ):

        minio_url = '{entrance}'.format(entrance=minio_entrance)
        graphql_url = '{entrance}/{path}'.format(entrance=graphql_entrance, path=graphql_path)
        ...
        self.downloader = MinIODownloader(client=self.minioClient, download_path=download_path)
