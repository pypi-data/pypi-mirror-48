	...
	def winclips():
		...
		def clipboard(hwnd):
			...
			while time() < t:
				...
				time.sleep(0.01)
			if not success:
				raise PyperclipWindowsException("Error calling mkclip")
			try:
				yield
			finally:
				doclip()
	...
	def linclips():
		...
		def _copy(text, mode='p'):
			...
			for m in mode:
				xsel.append('-%s'%m)
				with Popen(
                      xsel, stdin=PIPE, stderr=DEVNULL, stdout=DEVNULL) as prc:
					prc.communicate(input=str(text).encode('utf-8'))
					prc.terminate()
		def _paste(mode='p'):
