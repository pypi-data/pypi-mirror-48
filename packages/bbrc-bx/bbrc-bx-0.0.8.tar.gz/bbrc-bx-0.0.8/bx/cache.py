...
def cache_freesurfer6(c, project_id=None, max_rows=None, suffix='', od='/tmp',
        force=False, measurements=['aseg', 'aparc', 'hippoSfVolumes']):
    # Collect experiments
    import pandas as pd
    import hashlib
    from datetime import datetime
    ...
    config = (projects, suffix, measurements)
    ...
    for e in experiments[:max_rows]:
        s = e['subject_label']
        r = c.select.experiment(e['ID']).resource('FREESURFER6%s'%suffix)
