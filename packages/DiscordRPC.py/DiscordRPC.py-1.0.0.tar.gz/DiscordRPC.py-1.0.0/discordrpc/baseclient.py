...
class BaseClient(ABC):
    ...
    def __init__(self, client_id: str, **kwargs):
        ...
        self.update_event_loop(loop or self.get_event_loop())
        ...
        if handler:
            ...
            if not inspect.isfunction(handler):
                errors.append('Handler must be a function')
            args = inspect.getfullargspec(handler).args

            if len(args) != 2 and (inspect.ismethod(handler) and len(args) != 3):
                errors.append('Handler should only accept two arguments.')

            if errors:
                self.close_event_loop()
                raise DRPCGenericError(',\n'.join(errors))

            if self.isasync:
                if not inspect.iscoroutinefunction(handler):
                    self.close_event_loop()
                    raise ValueError('You are running async mode - your error handler should be awaitable.')
        ...
        self._events_on = bool(getattr(self, 'on_event', False))
    ...
    # Called by the asyncio loop upon error, not us.
