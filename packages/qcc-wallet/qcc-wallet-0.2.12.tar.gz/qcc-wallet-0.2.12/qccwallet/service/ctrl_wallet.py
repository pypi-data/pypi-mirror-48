...
def parser_eth_tx(cid, tx):
    ...
    if tx['input'] == '0x':
        ...
        ret['to'] = tx.pop('to')
        ret['value'] = CoinNum(str(tx.pop('value'))).set(dec, prec).group()
        ...
        if contract_type == 'ERC20':
            ...
            if method == 'transfer' and len(params) >= 2:
                ret['to'] = params[0]
                ret['value'] = CoinNum(params[1]).set(dec, prec).group()
            elif method == 'transferFrom' and len(params) >= 3:
                ret['from'] = params[0]
                ret['to'] = params[1]
                ret['value'] = CoinNum(params[2]).set(dec, prec).group()
            elif method == 'loan' and len(params) >= 3:
                ret['to'] = params[0]
                ret['value'] = CoinNum(params[1]).set(dec, prec).group()
                ret['charge'] = CoinNum(params[2]).set(dec, prec).group()
            elif method == 'repay' and len(params) >= 2:
                ret['to'] = contract_address
                ret['value'] = CoinNum(params[0]).set(dec, prec).group()
                ret['charge'] = CoinNum(params[1]).set(dec, prec).group()
