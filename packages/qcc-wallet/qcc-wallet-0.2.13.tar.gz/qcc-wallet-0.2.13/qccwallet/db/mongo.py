...
def init():
    ...
    transaction_sync = mongo_db.transaction_sync
    tx = mongo_db.tx
    # END
...
def _transaction_index():
    ...
    transaction.create_index([('from', ASCENDING), ('blockNumber', DESCENDING)])
    transaction.create_index([('to', ASCENDING), ('blockNumber', DESCENDING)])
    transaction.create_index([('to', ASCENDING),
                              ('blockNumber', DESCENDING),
                              ('transactionIndex', DESCENDING)], unique=True)


def _transaction_sync_index():
    """
    {
        "_id": "0x58a81beab5f9948114d267ee834d82928accb030",   # id即为地址
        "synced_block_number": 3,
        "updated": 0
    }
    """
    pass
