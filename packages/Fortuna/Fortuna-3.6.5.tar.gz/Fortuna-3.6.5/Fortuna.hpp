...
namespace Fortuna {
    ...
    auto d(Int sides) -> Int {
        ...
        return analytic_continuation(Fortuna::d<Int>, sides, 0);
    }
    ...
    auto dice(Int rolls, Int sides) -> Int {
        if (rolls > 0) {
            ...
            for (auto i {0}; i < rolls; ++i)
                total += Fortuna::d<Int>(sides);
            return total;
        }
        return -Fortuna::dice<Int>(-rolls, sides);
    }

    template<typename Int>
    auto dice_algo(Int rolls, Int sides) -> Int {
        if (rolls > 0) {
            std::vector<Int> the_rolls(rolls);
            std::generate_n(the_rolls.begin(), rolls, [&sides]() { return Fortuna::d<Int>(sides); });
            return std::reduce(the_rolls.cbegin(), the_rolls.cend());
        }
        return -Fortuna::dice<Int>(-rolls, sides);
    }

     auto ability_dice(int number) -> int {
        const int num { std::clamp(number, 3, 9) };
        if (num == 3) return Fortuna::dice<int>(3, 6);
        std::vector<int> the_rolls(num);
        std::generate_n(the_rolls.begin(), num, []() { return Fortuna::d<int>(6); });
    ...
    auto plus_or_minus(Integer number) -> Integer {
        return Fortuna::random_int(-number, number);
    }

    auto plus_or_minus_linear(Integer number) -> Integer {
        const auto num { std::abs(number) };
        return Fortuna::dice<Integer>(2, num + 1) - (num + 2);
    }
    ...
    auto front_gauss(Integer number) -> Integer {
        if (number > 0) {
            static const auto PI { 4 * std::atan(1) };
            const auto result { Integer(std::floor(Fortuna::gammavariate(1.0, number / PI))) };
    ...
    auto middle_gauss(Integer number) -> Integer {
        if (number > 0) {
            static const auto TAU { 8 * std::atan(1) };
            const auto result { Integer(std::floor(Fortuna::normalvariate(number / 2.0, number / TAU))) };
            return Fortuna::fuzzy_clamp(result, number);
        }
