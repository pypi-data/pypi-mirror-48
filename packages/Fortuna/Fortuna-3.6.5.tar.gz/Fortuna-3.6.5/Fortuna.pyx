...
cdef extern from "Fortuna.hpp":
    ...
    double        _min_float              "Fortuna::min_float"()
    double        _max_float              "Fortuna::max_float"()
    double        _min_below              "Fortuna::min_below"()
    double        _min_above              "Fortuna::min_above"()
...
# ZeroCool Methods #
ZeroCool = {
    "random_index": lambda x: _random_index(x),
    "front_linear": lambda x: _front_linear(x),
    "middle_linear": lambda x: _middle_linear(x),
    "back_linear": lambda x: _back_linear(x),
    "quantum_linear": lambda x: _quantum_linear(x),
    "front_gauss": lambda x: _front_gauss(x),
    "middle_gauss": lambda x: _middle_gauss(x),
    "back_gauss": lambda x: _back_gauss(x),
    "quantum_gauss": lambda x: _quantum_gauss(x),
    "front_poisson": lambda x: _front_poisson(x),
    "middle_poisson": lambda x: _middle_poisson(x),
    "back_poisson": lambda x: _back_poisson(x),
    "quantum_poisson": lambda x: _quantum_poisson(x),
    "quantum_monty": lambda x: _quantum_monty(x),
}
...
def random_sequence(sequence, zero_cool: staticmethod = random_index):
    return lambda stop=len(sequence): flatten(sequence[zero_cool(stop)])
...
def distribution(func: staticmethod, *args, num_cycles=1000000, post_processor: staticmethod = None, **kwargs):
    ...
    try:
        stat_samples = results[:min(256, num_cycles)]
