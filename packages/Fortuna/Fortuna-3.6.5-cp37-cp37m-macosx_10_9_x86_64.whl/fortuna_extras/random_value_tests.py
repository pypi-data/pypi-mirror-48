...
distribution_timer(gen_higher_order_string, f=dice, x=2, y=2, label="gen_higher_order_string(f=dice, x=1, y=3)")

distribution_timer(
    gen_higher_order_string,    # RandomValue generator
    random_int, x=-1, y=1,      # lambda arguments
    zero_cool=back_linear,      # distribution type, must follow ZeroCool spec
    range_to=-4,                # distribution limiter, must be in range [-N, N] with N = len(data)
