...
def add_args(parser: argparse.ArgumentParser,
    ...
    for key in meta_args:
        ...
        if 'dest' not in kw_args:
            kw_args['dest'] = (dest_prefix + arg_keys[-1]).lower().replace('-', '_')

        if 'metavar' not in kw_args and kw_args.get('action', '').split('_')[-1] not in ('true', 'false', ):
            kw_args['metavar'] = arg_keys[-1].split('_')[-1].upper()
...
def get_args(file_or_yaml, debug=False):
    ...
    if os.path.isfile(file_or_yaml):
        with open(file_or_yaml) as f:
            args_conf = yaml.load(f.read())
    else:
        args_conf = yaml.load(file_or_yaml)
    ...
    if enable_env:
        ...
        if arguments_cmd['env_file_'] is not None:
            env_files += arguments_cmd['env_file_'].values()
    ...
    if enable_conf:
        ...
        if arguments_cmd['conf_file_'] is not None:
            conf_files += arguments_cmd['conf_file_'].values()

        for conf_file in conf_files:
            if debug:
                print('--->', conf_file)
            if conf_file or not os.path.isfile(conf_file):
                continue
            ...
            if ext == 'json':
                ...
                arguments_conf.update(yaml.load(content))
            elif ext == 'toml':
                try:
