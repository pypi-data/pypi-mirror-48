...
class TestInstantiateSerialize(TestCase):
    def test_element_access(self):
        for class_name, element_class in Links.items():
            extracted_class = Links.get(class_name)
            self.assertEqual(element_class, extracted_class)
        for class_name, element_class in Nodes.items():
            extracted_class = Nodes.get(class_name)
            self.assertEqual(element_class, extracted_class)
    ...
    def test_serialize_links(self):
        for elem in TEST_DATA["links"]:
            repr0 = Links.instantiate(elem).asdict(json_compatible=True)
            repr1 = Links.instantiate(repr0).asdict(json_compatible=True)
            self.assertEqual(repr0, repr1)
    ...
    def test_serialize_nodes(self):
        for elem in TEST_DATA["nodes"]:
            repr0 = Nodes.instantiate(elem).asdict(json_compatible=True)
            repr1 = Nodes.instantiate(repr0).asdict(json_compatible=True)
            self.assertEqual(repr0, repr1)

    def test_instantiate_metadata(self):
        raw_metadata = TEST_DATA["metadata"]
        metadata = Metadata(**raw_metadata)
        self.assertElementUnchanged(raw_metadata, metadata)
