...
class GraphUtils:
    ...
    def visualize_using_boxplot(self, obj: object, numcols: int, release_criteria: int, ylimit: int):
      try:
        ...
        p = {str(self.last_release):'#00a1e0ff', str(self.this_release):'#7c868dff'}
        ...
        if 'entitytype' in obj.columns:
          ...
          for i, sobj in enumerate(self.sobjects):
            for appversion in [self.last_release, self.this_release]:
              ...
              sns.kdeplot(d, color=self.appversion_colors[appversion], ax=ax[i], shade=True, label=appversion, palette=p)
              ax[i].set(yticks=[])
              ax[i].set_xlabel('')
          ...
          sns.distplot(obj[obj['appVersion']==self.this_release]['EPT'], ax=ax, label=str(self.this_release))
          sns.distplot(obj[obj['appVersion']==self.last_release]['EPT'], ax=ax, label=str(self.last_release))
          plt.legend()
          ax.set_title('Bootstrap EPT')
