    ...
    def cmp_aura_flexipage(self, obj, xaxislimit: int):
      try:
        if 'entitytype' in obj.columns:
          ...
          sns.distplot(obj[obj['appVersion']==self.this_release]['EPT'], palette=self.appversion_colors, ax=ax, label=str(self.this_release))
          sns.distplot(obj[obj['appVersion']==self.last_release]['EPT'], palette=self.appversion_colors, ax=ax, label=str(self.last_release))
          plt.legend()
