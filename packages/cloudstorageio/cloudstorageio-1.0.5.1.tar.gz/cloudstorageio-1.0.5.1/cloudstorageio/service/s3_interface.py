...
from cloudstorageio.utils.exceptions import Exceptions
...
class S3Interface:
    ...
    def __init__(self, **kwargs):
        ...
        if (self._acc_secret_key and not self._acc_key) or (self._acc_key and not self._acc_secret_key):
            raise Exceptions.ConnectionRefusedError('Please provide both aws_access_key_id and aws_secret_access_key')
    ...
    def path(self):
        if self._current_bucket is None:
            raise Exceptions.ValueError("Bucket name is not set")
        if self._current_path is None:
            raise Exceptions.ValueError("Path name is not set")
    ...
    def listdir(self, path: str):
        ...
        if not self._object_exists:
            raise Exceptions.FileNotFoundError(f'No such file or dictionary: {path}')
    ...
    def remove(self, path: str):
        ...
        self._analyse_path(path)
        if not self._object_exists:
            raise Exceptions.FileNotFoundError(f"Object with path {path} does not exists")
    ...
    def read(self) -> Union[str, bytes]:
        ...
        """
        if not self._isfile:
            raise Exceptions.FileNotFoundError('No such file: {}'.format(self.path))
        ...
        if self._mode is not None and 'b' not in self._mode:
            try:
                ...
                raise Exceptions.ValueError(f"The content cannot be decoded into a string"
