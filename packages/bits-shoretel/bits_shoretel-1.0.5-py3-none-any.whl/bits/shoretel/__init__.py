...
class Shoretel(MySQL):
    ...
    def get_systemdirectorydisplay(self):
        """Return all system directory display entries."""
        return self.get_table('systemdirectorydisplay')

    def get_systemdirectorydisplay_dict(self, key='AddressID'):
        """Return a dict of Shoretel systemdirectorydisplay."""
        systemdirectorydisplay = {}
        for u in self.get_systemdirectorydisplay():
            k = u.get(key)
            if k:
                systemdirectorydisplay[k] = u
        return systemdirectorydisplay
