...
import binascii
import functools
from pysmx.crypto import hashlib as smhashlib
import importlib
import warnings
...
class SM3PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        ...
        hash = smhashlib.sm3(force_bytes(salt + password)).hexdigest()
...
class UnsaltedSM3PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        ...
        hash = smhashlib.sm3(force_bytes(password)).hexdigest()
        return 'sm3$$%s' % hash
...
class SM3KDFPasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt, iterations=None):
        ...
        if isinstance(password, str):
            hash = SM3.KDF(bytes(password, 'ascii').hex(), self.iterations)
        elif isinstance(password, (bytes, bytearray)):
            hash = SM3.KDF(password.hex(), self.iterations)
        else:
...
class SHA1PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        assert password is not None
        assert salt and '$' not in salt
        hash = smhashlib.sha1((salt + password).encode()).hexdigest()
        return "%s$%s$%s" % (self.algorithm, salt, hash)
...
class MD5PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        assert password is not None
        assert salt and '$' not in salt
        hash = smhashlib.md5((salt + password).encode()).hexdigest()
        return "%s$%s$%s" % (self.algorithm, salt, hash)
...
class UnsaltedSHA1PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        assert salt == ''
        hash = smhashlib.sha1(password.encode()).hexdigest()
        return 'sha1$$%s' % hash
...
class UnsaltedMD5PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password, salt):
        assert salt == ''
        return smhashlib.md5(password.encode()).hexdigest()
