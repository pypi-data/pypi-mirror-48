...
flags.DEFINE_integer(
    'seed', 0,
    'The seed to use when randomly sample from a set of inputs.'
...
def load_pos_neg_attr(model_pos, data_pos, model_neg, data_neg):
  """Load two sets of attributions from model_pos-data_pos and model_neg-data_neg. Filter and only return attributions of correctly classified inputs.
  ...
  for metric in FLAGS.metrics:
    ...
    if metric == 'RMCS':
      ...
      for i in range(1, NUM_RELATIVE_MODELS):
        print('MCS between', model_prefix + str(i), 'and', model_prefix +
              str(NUM_RELATIVE_MODELS))
        MCS(model_prefix + str(i), data, model_prefix +
            str(NUM_RELATIVE_MODELS), data)
    else:
