...
def compute_and_save_attr(model, data, indices, num_threads):
  ...
  with tf.Session(graph=tf.Graph()) as sess:
    ...
    def single_map(img, img_name):
      ...
      # *s is SmoothGrad
      vgs_mask = vg.GetSmoothedMask(img, feed_dict={neuron_selector: pred}, num_threads=50)
      gb_mask = gb.GetMask(img, feed_dict={neuron_selector: pred})
      gbs_mask = gb.GetSmoothedMask(img, feed_dict={neuron_selector: pred}, num_threads=50)
      ...
      igs_mask = ig.GetSmoothedMask(
          img, feed_dict={neuron_selector: pred}, x_baseline=baseline, num_threads=50)
      gc_mask = gc.GetMask(img, feed_dict={neuron_selector: pred})
      gcs_mask = gc.GetSmoothedMask(img, feed_dict={neuron_selector: pred}, num_threads=50)
