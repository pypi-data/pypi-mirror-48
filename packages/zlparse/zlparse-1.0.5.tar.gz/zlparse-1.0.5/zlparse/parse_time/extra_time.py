...
import re
import time
from zhulong.util.conf import zhulong_diqu_dict

not_ggzy_list_all = ['gcjs_anhui_huainan',
...
def not_ggzy_extime_all(page, ggstart_time, quyu):
    if quyu in not_ggzy_list_page_time:
        ...
        if strptime_transfrom_nospace(page) is not None:
            ...
            return None
    elif quyu in ['gcjs.guangdong_yangjiang',
                  'gcjs.guangxi_baise',
                  'gcjs.hunan_shenghui',
                  'gcjs.jiangxi_pingxiang',
                  'gcjs.jiangxi_nanchang',
                  'gcjs.jiangxi_shangrao',
                  'gcjs.liaoning_dalian',
                  'gcjs.ningxia_shenghui',
                  'gcjs.shandong_shenghui',
                  'gcjs.shanxi_hanzhong',
                  'gcjs.shanxi_xian',
                  'gcjs.xinjiang_atushi',
                  'gcjs.xinjiang_bole',
                  'gcjs.xinjiang_changji',
                  'gcjs.xinjiang_kashi',
                  'gcjs.xinjiang_tacheng',
                  'gcjs.xinjiang_yining',
                  'gcjs.zhejiang_shenghui',
                  'zfcg.anhui_anqing',
                  'zfcg.chongqing_chongqing',
                  'zfcg.guangxi_guigang',
                  'zfcg.jiangsu_suzhou',
                  'zfcg.jiangsu_taizhou',
                  'zfcg.neimenggu_shenghui',
                  'zfcg.shandong_dezhou',
                  'zfcg.xinjiang_wulumuqi',
                  'zfcg.zhejiang_ningbo',
                  'zfcg.zhejiang_wenzhou',
                  'qycg.baowu_ouyeelbuy_com',
                  'qycg.bidding_ceiec_com_cn',
                  'qycg.dfqcgs_dlzb_com',
                  'qycg.ec_ccccltd_cn',
                  'qycg.www_dlswzb_com',

                  ]:
        if quyu in ['gcjs.jiangsu_yangzhou', 'qycg.ec_ceec_net_cn']:
            if strptime_transfrom_nospace(page):
                return (strptime_transfrom_nospace(page))
            return (ggstart_time)
        elif strptime_transfromgg_question1(page):
            if strptime_transfromgg_question1(page) is not None:
                return strptime_transfromgg_question1(page)
...
def strptime_transfromgg_question1(page):
    # 处理ggstart_time 有问题，通过页面时间来解析出时间
    soup = BeautifulSoup(page, 'lxml')
    txt = re.sub('[^\u4E00-\u9Fa5a-zA-Z0-9:：\-\\/]', '', soup.text.strip())
    ...
    # ggstart_time时间有问题
        ...
        if quyu in ['yunnan_lijiang', 'yunnan_puer', 'yunnan_dali', 'yunnan_honghe', 'yunnan_lincang']:
            ...
            if strptime_transfrom_yue_r_n(page) is not None:
                ...
                return (ggstart_time)
        elif strptime_transfromgg_question1(page) is not None:
            return strptime_transfromgg_question1(page)
        else:
            return ggstart_time
    else:
