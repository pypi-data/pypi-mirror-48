...
if __name__ == '__main__':
    for quyu in list_all:
        ...
        # print(df.head(2))
        # 传入参数
        df['data'] = (df['page'].map(lambda x: x if x is not None else '') + '##shm##' + df['ggstart_time'].map(lambda x: x if x is not None else '') + "##shm##%s" % quyu).map(
            lambda x: extime_all(*x.split("##shm##")) if extime_all(*x.split("##shm##")) is not None else x.split("##shm##")[1])
