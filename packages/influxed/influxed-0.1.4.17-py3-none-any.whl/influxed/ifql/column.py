...
class Column(object):
    ...
    def format(self):
        if(self.call_arguments):
            return f'{self.name}({self.call_arguments[0]})'
        else:
            formated = self.name
            if(str_needs_escape(formated)):
                formated = f'"{formated}"'
            if(self.display_as):
                return f'{formated} AS {self.display_as}'
            return formated
...
class __time_column__(Column):
    ...
    @property
    def interval(self):
        return f'{self.__n__}{self.unit.value}'
    ...
    def parse_n(self, n):
        if(isinstance(n, str)):
            ...
            return n
        raise TypeError(f'n must be a str or a int not {type(n)} - {n}!')
