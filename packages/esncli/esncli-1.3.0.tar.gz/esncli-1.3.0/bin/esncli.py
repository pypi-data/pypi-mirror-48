...
class AutoconfCommand(DefaultCommand):
    ...
    def __call__(self, args):
        ...
        # https://op-admin.barcamp.awesomepaas.net/api/configurations?domain_id=5c47598e7637d10058f90865&scope=domain
        endpoint = '/api/configurations?scope=domain&domain_id=' + domain_id
        data = [{
            "name": "core",
            "keys": ["autoconf"]
        ...
        r = self.session.post(self.base_url + endpoint,
                              ...
                              verify=not self.no_verify)
        if r.status_code != 200:
            self.log.error("Can retrieve autoconf current values.")
            return False
        data = r.json()
        data[0]['configurations'][0]['value']['accounts'] = [
            {
                "identities": [
                    {
