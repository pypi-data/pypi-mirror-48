...
class BugCatcherManager(object):
    ...
    def __init__(self, project_name):
        ...
        if sys.argv[0]:
            exe_filename = os.path.abspath(sys.argv[0])
            if not os.path.exists(exe_filename):
                exe_filename = os.path.abspath(sys.argv[1])

            if os.path.isfile(exe_filename) and os.path.exists(exe_filename):
    ...
    def setup_path(self):
        ...
        path = os.path.join(os.getenv('ALLUSERSPROFILE'), BUG_CATCHER_NAME)
