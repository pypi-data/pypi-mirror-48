...
class NfAcct:
    ...
    etc_path = '/etc/nfacct'
    db_file = os.path.join(etc_path, 'db.yml')
    ...
    def __init__(self, args: Arguments):
        ...
        os.makedirs(self.etc_path, 0o755, exist_ok=True)
    ...
    def load_db(self):
        try:
            ...
            logger.critical('db file is corrupted: ' + traceback.format_exc())
            exit(-1)
        else:
            self.db = db
    ...
    def add(self):
        ...
        if self.args.proto is None and self.args.user is None:
            logger.critical('`--proto` and `--user` must supplied once at least.')
            exit(-1)
            return
        ...
        if self.args.user is not None:
            ip_args.append(f'-m owner --uid-owner {self.args.user}')
