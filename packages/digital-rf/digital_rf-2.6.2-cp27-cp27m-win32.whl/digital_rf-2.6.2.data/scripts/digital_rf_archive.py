...
def archive_subdirectory_local_local(args):
    ...
    if is_metadata:
        archive_files = glob.glob(os.path.join(
            source, channel, subdir, _metadata_file_glob))
    else:
        archive_files = glob.glob(os.path.join(
            source, channel, subdir, _rf_file_glob))
    ...
    for archive_file in archive_files:
        ...
        if ts >= startDT and ts <= endDT:
            ...
            if not is_compressed and gzip > 0 and not is_metadata:
                ...
                shutil.copyfile(archive_file, os.path.join(
...
class archive(object):
    ...
    def __init__(self, startDT, endDT, source, dest, pool=1, channels=None, is_metadata=False,
        ...
        if self._dest_type == 'local' and self._source_type == 'local':
            ...
            if self.verbose or self.check_only:
                print('Estimated GB to use = %f GB, available = %f GB' %
                      (needed_GB, avail_GB))
                if self.check_only:
                    sys.exit(-1)
    ...
    def _archive_channel_local_local(self, channel):
        ...
        file_count_list = self._pool.map(
            archive_subdirectory_local_local, args_list)

        if self.verbose:
            print('%i files backed up in channel %s' %
                  (sum(file_count_list), channel))
    ...
    def _archive_channel_local_remote(self, channel):
        ...
        file_count_list = self._pool.map(
            archive_subdirectory_local_remote, args_list)
        ...
        if self._dest_type == 'local':
            ...
            for i, this_dir in enumerate((self._top_level_dest_dir, self._metadata_dir, self._rf_dir)):
                ...
                if i == 0:
                    ...
                    cmd = 'scp -q -r %s %s' % (localDir, os.path.join(
                        self.dest, self._top_level_dest_dir))
                try:
                    subprocess.check_call(cmd.split())
                except subprocess.CalledProcessError:
    ...
    def estimate_disk_space(self):
        ...
        for channel in self.channels:
            ...
            for i, items in enumerate(subdir_list[0]):
                ...
                if self.is_metadata:
                    archive_files = glob.glob(os.path.join(
                        self.source, channel, subdir, _metadata_file_glob))
                    compress = 1.0
                else:
                    archive_files = glob.glob(os.path.join(
                        self.source, channel, subdir, _rf_file_glob))
    ...
    def _test_compression(self, testHdf5File):
        ...
        try:
            ...
            pass
        cmd = 'h5repack -i %s -o %s -f rf_data:GZIP=%i' % (
            testHdf5File, resultFile, self.gzip)
        subprocess.check_call(cmd.split())
        result = float(os.path.getsize(resultFile)) / \
            os.path.getsize(testHdf5File)
        os.remove(resultFile)
        return(result)
    ...
    def get_chan_dict_local(self):
        ...
        subdirectories = glob.glob(os.path.join(
            self.source, '*', self._sub_directory_glob))
        subdirectories.sort()
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('startDT <%s> not in expected YYYY-MM-DDTHH:MM:SS format' %
              (args.startDT))
        sys.exit(-1)
    try:
        endDT = datetime.datetime.strptime(args.endDT, '%Y-%m-%dT%H:%M:%S')
    except:
        print('endDT <%s> not in expected YYYY-MM-DDTHH:MM:SS format' %
              (args.endDT))
        sys.exit(-1)
