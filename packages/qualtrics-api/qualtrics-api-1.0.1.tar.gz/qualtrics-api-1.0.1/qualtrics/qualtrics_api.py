...
def get_parser():
    ...
    docstr = ("""Example:
                 python -m qualtrics -t <token_api> -o <organization_id> -d <datacenter_id> 
                 -s <survey_ID>""")
    parser = argparse.ArgumentParser(prog='qualtrics', description=docstr, epilog='Powered by W@xP!')
    parser.add_argument('-v', '--version', action='version', version=__version__)
    parser.add_argument('-t', '--token', type=str, help='Token obtained from Qualtrics API.',
                        nargs='+', required=True)
    parser.add_argument('-o', '--organization', type=str, help='Organization.', required=True)
...
def create_export_response(data_center_id, token, survey_id, format_out='json', questions=None):
    ...
    response = ''
    payload = {"format": format_out, "questionIds": questions, "compress": 'false'}
    try:
