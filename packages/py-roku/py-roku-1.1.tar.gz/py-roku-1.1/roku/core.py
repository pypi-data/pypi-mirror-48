...
class DeviceInfo(object):
    ...
    def __init__(self, model_name, model_number, sw_version, serial_number, user_device_name, power_mode):
    ...
    def __repr__(self):
        return ('<DeviceInfo: %s-%s, SW v%s, Ser# %s, Name %s>' %
                (self.model_name, self.model_number, self.sw_version, self.serial_number, self.user_device_name))
...
class Roku(object):
    ...
    def device_info(self):
        ...
        dinfo = DeviceInfo(
            ...
            model_number=root.find('model-number').text,
            sw_version=''.join([
                root.find('software-version').text,
                '.',
                root.find('software-build').text
            ]),
            serial_number=root.find('serial-number').text,
            user_device_name=root.find('user-device-name').text,
            power_mode=root.find('power-mode').text,
        )
        return dinfo
