...
class AbstractDataSource (object):
    ...
    def map(self, fn):
        """
        Creates a `MapDataSource` that applies a function to each mini-batch
        generated by `self`.

        A common use of `map` would be to apply post-processing to the samples
        in the mini-batch, e.g. data augmentation.

        Parameters
        ----------
        func: callable `func(*batch) -> modified_batch`
            The function to apply to each mini-batch. Must return a mini-batch
            as a tuple.

        Returns
        -------
            A `MapDataSource` instance
        """
